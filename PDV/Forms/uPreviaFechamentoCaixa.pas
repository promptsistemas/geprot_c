...
begin
  Memo1.Clear;
    ...
    begin
      ...
      Memo1.Lines.Add(' ');
      Memo1.Lines.Add('-----------------------------------------------------------------------');
      Memo1.Lines.Add('DATA ...: '+DefineTamanhoString(FormatDateTime('DD/MM/YYYY',cdsResumoVendasDT_MOV.AsDateTime),10,0)+'   Hora Impressão..:'+TimeToStr(Time));
      Memo1.Lines.Add('CAIXA...: '+DefineTamanhoString(L_CAIXA.Caption,3,0)+' Operador de Caixa..: '+L_OPERADOR.Caption);
                                       //Cabeçalho de Clientes
      Memo1.Lines.Add('-----------------------------------------------------------------------');
      Memo1.Lines.Add('  ESPÉCIE                               VL_VENDAS(R$)     ');
      Memo1.Lines.Add('-----------------------------------------------------------------------');
...
begin
  ...
  if Application.MessageBox('O Fechamento de Caixa Impossibilitará a Realização de Novas Vendas do dia.  Deseja Realmente Fechar o Caixa?','Confirmação',MB_YESNO+MB_ICONQUESTION) = idyes then
  begin
    ...
    ShowMessage('Caixa Fechado, Aplicação Será Encerrada.');
