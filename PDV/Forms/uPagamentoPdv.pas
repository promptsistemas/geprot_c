  ...
  try
    ...
    begin
      ...
      Write ( arq,('PIT|'+IntToStr(item)+'|'+DM.cdsItemPdvREFERENCIA.AsString+'|'
              ...
              +''+'|'  //33 ID CONFINS ST N
              +DM.cdsItemPdvNCM.AsString+'|'//34 NCM AN Tamanho máximo de 20 caracteres. Atualização de layout na versão 0.3.19 do DJIntegração.
...
begin
  with ACBrNFe1.NotasFiscais.Add.NFe do
   begin
     ...
     Ide.verProc   := '2.0d'; //Versão do seu sistema
...
//      Dest.IE                := '687138770110'; //NFC-e não aceita IE
      Dest.ISUF              := '';
      Dest.xNome             :=  DM.cdsPDVR_SOCIAL.AsString; //'D.J. COM. E LOCAÇÃO DE SOFTWARES LTDA - ME';
...
//      Dest.EnderDest.CEP     :=  //StrToInt(DM.cdsPDVCEP.AsString); //18270170;
      ...
      Dest.EnderDest.xMun    :=  DM.cdsPDVCIDADE.AsString; //'Tatuí';
    ...
    begin
      ...
      with Det.Add do
       begin
         ...
         Prod.xProd    :=  DM.cdsItemPdvDESCRICAO.AsString; // 'Descrição do Produto';
         Prod.NCM      :=  DM.cdsItemPdvNCM.AsString;  //'94051010'; // Tabela NCM disponível em  http://www.receita.fazenda.gov.br/Aliquotas/DownloadArqTIPI.htm
         Prod.EXTIPI   := '';
         Prod.CFOP     := '5101';
...
begin
  ...
  begin
    ...
    begin
      ShowMessage('Plano de Pagamento Inválido Para Forma de Pagamento '+cdsTipoPagamentoDESCRICAO_PAGAMENTO.AsString);
      DBLookupComboBox1.SetFocus;
      Exit;
    end;
    if (cdsTipoPagamentoID_FORMA_PAGAMENTO.AsInteger > 1) and (DM.cdsPlanoPagamentoID_PLANO_PAGAMENTO.AsInteger = 1) then
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if Application.MessageBox('Deseja Imprimir Venda?','Confirmação',MB_YESNO+MB_DEFBUTTON2) = idyes then
        begin
...
{        cdsContasReceber.Close;
    ...
    //    begin
            ...
            if cdsTipoPagamentoID_FORMA_PAGAMENTO.AsInteger = 2 then
            begin
//              if Application.MessageBox('Deseja Imprimir Convênio?','Confirmação',MB_YESNO+MB_ICONQUESTION) = idyes then
//              begin
