...
begin
  ...
  begin
    ...
    begin
      MemoCupomTEF.Lines.Add('Transação Negada');
  ...
  begin
    ...
    MemoCupomTEF.Lines.Add('Venda Concluída');
    MemoCupomTEF.Lines.Add('');
    DM.cdsNFCeNR_NFCE.Text := IntToStr(StrToIntDef(DM.cdsNFCeNR_NFCE.Text, 0) + 1);
    Application.MessageBox('Venda Concluída', 'Informação', MB_OK + MB_ICONINFORMATION);
  end
  else
...
begin
  ...
  MemoCupomTEF.Lines.Add('Imprimindo Transações/Comprovante TEF');
...
begin
  ...
  with ACBrNFe1.NotasFiscais.Add.NFe do
   begin
     ...
     begin
       Ide.dhCont := NOW;
       Ide.xJust  := 'Problemas técnicos no envio do cupom eletrônico';
