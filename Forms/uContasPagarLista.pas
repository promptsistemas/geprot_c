...
begin
  if Application.MessageBox('Deseja Realmente Baixar os Titulos Selecionados ?','Confirmação ',MB_YESNO+MB_ICONQUESTION)= idyes  then
  begin
    ...
    begin
      ...
      DBGrid1.DataSource.DataSet.GotoBookmark(DBGrid1.SelectedRows.Items[vIni]);
  ...
  //    vv          := (vv - cdsContasPagarVL_PARCELA.AsFloat);
      ...
      begin
        ...
        ShowMessage( ' Titulo Será Baixado Parcialmente, Valor do Titulo '+cdsContasPagarVL_PARCELA.AsString + ' Valor do Saldo Acumulado '+FloatToStr(y));
          ...
          cdsContasPagarNR_DOC.AsString               := nrDoc+'S';
...
begin
  ...
  begin
    cdsContasPagar.GotoBookMark(DBGrid1.SelectedRows.Items[i-1]);
...
procedure TfrmContasPagarLista.DBLookupComboBox1Enter(Sender: TObject);
begin
  cdsFornecedor.Open;
end;
...
procedure TfrmContasPagarLista.Timer1Timer(Sender: TObject);
begin
