...
type
  TfrmContasPagarTingimento = class(TForm)
    Panel1: TPanel;
    Label5: TLabel;
    Panel2: TPanel;
    Shape1: TShape;
...
var
  frmContasPagarTingimento: TfrmContasPagarTingimento;
  DiasDoMes : TDate;
  Tipo: string;
  Count : Integer;
...
uses U_LIB, UD_PESQUISA, uDM, uMenu, uProducao, uContasPagarComplementar,
  uCadEmpresa, uContasPagar, uTingimento, uConPagComplementarTingimento;

procedure TfrmContasPagarTingimento.BB_ALTERARClick(Sender: TObject);
begin
  Tipo := 'Edit';
  cdsContasPagar.Edit;
  GerenciaBotoes;
...
procedure TfrmContasPagarTingimento.BB_CANCELARClick(Sender: TObject);
begin
  if Tipo = 'Insert' then
  begin
...
procedure TfrmContasPagarTingimento.BB_DELClick(Sender: TObject);
begin
...
procedure TfrmContasPagarTingimento.BB_EXCLUIRClick(Sender: TObject);
begin
...
procedure TfrmContasPagarTingimento.BB_INCLUIRClick(Sender: TObject);
begin
  Tipo := 'Insert';
  ...
  begin
    ...
    cdsContasPagarDT_EMISSAO.AsDateTime := DM.cdsTingimentoDT_EMISSAO.AsDateTime;
...
procedure TfrmContasPagarTingimento.BB_SAIRClick(Sender: TObject);
begin
  Close;
  frmTingimento.PageControl2Change(Self);
end;

procedure TfrmContasPagarTingimento.BitBtn2Click(Sender: TObject);
begin
  if DBLookupComboBox1.Text = ''  then
  begin
    ShowMessage('Favor Informar Plano de Pagamento.');
...
procedure TfrmContasPagarTingimento.DBEdit1Exit(Sender: TObject);
begin
...
{    if cdsContasPagar.RecordCount > 0 then
    ...
    if not DM.cdsPlanoPagamentoQT_PARCELAS.IsNull then
        ...
        (DM.cdsTingimentoVL_NOTA.AsFloat / DM.cdsPlanoPagamentoQT_PARCELAS.AsInteger);
...
procedure TfrmContasPagarTingimento.DBEdit2Exit(Sender: TObject);
begin
...
procedure TfrmContasPagarTingimento.DBEdit3Exit(Sender: TObject);
begin
...
procedure TfrmContasPagarTingimento.DBEdit5Enter(Sender: TObject);
Var
...
begin
  ...
  begin
    ...
    DM.cdsDinamica.CommandText := ' SELECT CP.VL_COMPRA, SUM(CP.VL_PARCELA), '+
                                  ...
                                  ' WHERE CP.ID_PRODUCAO = '+DM.cdsTingimentoID_TINGIMENTO.AsString+
                                  ' AND CP.TP_MOV=''D'''+
    ...
    SaldoPagamento:= DM.cdsTingimentoVL_NOTA.AsFloat-(DM.cdsDinamica.Fields[1].AsFloat-vlParcelaEdit);
    cdsContasPagarVL_PARCELA.AsFloat := SaldoPagamento;
  end;

end;

procedure TfrmContasPagarTingimento.DBEdit5Exit(Sender: TObject);
begin
  ...
  begin
    ...
    begin
      ...
      cdsContasPagar.FieldByName('ID_PRODUCAO').Value         := DM.cdsTingimentoID_TINGIMENTO.AsInteger;
      cdsContasPagar.FieldByName('ID_FORNECEDOR').Value       := DM.cdsTingimentoID_FOR_TINGIMENTO.AsInteger;
      cdsContasPagar.FieldByName('ID_FORN_REM').Value         := DM.cdsTingimentoID_FORN_TINT.AsInteger;
    ...
    if cdsContasPagar.RecordCount = 0 then
      cdsContasPagar.FieldByName('NR_DOC').Value          := DM.cdsTingimentoNR_NF_TINGIMENTO.AsString +'/'+IntToStr(1)+'-'+DM.cdsPlanoPagamentoQT_PARCELAS.AsString
    else
      cdsContasPagar.FieldByName('NR_DOC').Value          := DM.cdsTingimentoNR_NF_TINGIMENTO.AsString +'/'+IntToStr(Parcelas+1)+'-'+DM.cdsPlanoPagamentoQT_PARCELAS.AsString;
      cdsContasPagar.FieldByName('VL_COMPRA').Value       := DM.cdsTingimentoVL_NOTA.AsFloat ;
      cdsContasPagar.FieldByName('ID_CLIENTE').Value      := StrToInt(DM.cdsTingimentoID_CLIENTE.AsString);
      ...
      cdsContasPagar.FieldByName('NR_NOTA_FISCAL').Value         := DM.cdsTingimentoNR_NOTA_FISCAL.AsInteger;
      cdsContasPagar.FieldByName('NR_NF_REMESSA').Value         := DM.cdsTingimentoNR_NF_TINGIMENTO.AsInteger;
      cdsContasPagar.FieldByName('ORIGEM').Value         := 'TINGIMENTO';
...
procedure TfrmContasPagarTingimento.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action := caFree;
  frmContasPagarTingimento := nil;
  iCont := 0;
end;

procedure TfrmContasPagarTingimento.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if key = vk_f4 then
  begin
    if DM.cdsTingimentoVL_POR_FORA.AsFloat > 0 then
    begin
      if frmConPagComplementarTingimento = nil then
      begin
        frmConPagComplementarTingimento := TfrmConPagComplementarTingimento.Create(Application);
        frmConPagComplementarTingimento.ShowModal;
      end;
    end
    else
    begin
      ShowMessage(' Não Exite Nota Complementar.');
    end;
  end;
end;

procedure TfrmContasPagarTingimento.FormKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
  begin
    Key := #0;
...
procedure TfrmContasPagarTingimento.FormShow(Sender: TObject);
begin
  ...
  cdsContasPagar.Params[2].AsInteger := StrToInt(DM.cdsTingimentoID_TINGIMENTO.AsString);
  cdsContasPagar.Open;
  GerenciaBotoes;
  SomaPagamentos;
  DBEdit3.SetFocus;
...
procedure TfrmContasPagarTingimento.GerarParcela;
var
i,dias: Byte;
Valor : Double;
ultimaData : Tdate;
...
//      DiasDoMes := DiasDoMes+dias;
    ...
    Valor := (StrToFloat(StringReplace(DM.cdsTingimentoVL_NOTA.Text,'.','',[rfReplaceAll])));
    ...
    cdsContasPagar.FieldByName('ID_USUARIO').Value          := frmMenu.recInformacoes.id_Usuario;
    cdsContasPagar.FieldByName('ID_LOJA').Value             := 1;
    cdsContasPagar.FieldByName('DT_EMISSAO').Value          := DM.cdsTingimentoDT_EMISSAO.AsDateTime;
    cdsContasPagar.FieldByName('DT_VENCIMENTO').Value   := IncMonth(DM.cdsTingimentoDT_EMISSAO.AsDateTime, i - 0);
 //   cdsContasPagar.FieldByName('DT_VENCIMENTO').Value   := DiasDoMes; //DM.cdsPdvDT_VENDA.AsDateTime+dias;
    cdsContasPagar.FieldByName('QT_DIAS').Value         := IncMonth(DM.cdsTingimentoDT_EMISSAO.AsDateTime, i - 0) -DM.cdsTingimentoDT_EMISSAO.AsDateTime;
    cdsContasPagar.FieldByName('QT_PARCELA').Value      := DM.cdsPlanoPagamentoQT_PARCELAS.AsString;
    if cdsContasPagar.RecordCount = 0 then
      cdsContasPagar.FieldByName('NR_DOC').Value          := DM.cdsTingimentoNR_NF_TINGIMENTO.AsString +'/'+IntToStr(1)+'-'+DM.cdsPlanoPagamentoQT_PARCELAS.AsString
    else
    ...
    cdsContasPagar.FieldByName('ID_PLANO_PAGAMENTO').Value  := DM.cdsPlanoPagamentoID_PLANO_PAGAMENTO.AsInteger;
    cdsContasPagar.FieldByName('STATUS').Value  := 'A';
    cdsContasPagar.FieldByName('NR_NOTA_FISCAL').Value         := DM.cdsTingimentoNR_NF_TINGIMENTO.AsInteger;
    cdsContasPagar.FieldByName('NR_NF_REMESSA').Value         := DM.cdsTingimentoNR_NF_TINGIMENTO.AsInteger;
    cdsContasPagar.FieldByName('ORIGEM').Value         := 'TECELAGEM';
    cdsContasPagar.FieldByName('TP_MOV').Value   :='D';
...
procedure TfrmContasPagarTingimento.GerenciaBotoes;
begin
...
procedure TfrmContasPagarTingimento.SomaPagamentos;
begin
  DM.cdsDinamica.Close;
  DM.cdsDinamica.IndexFieldNames:='';
  DM.cdsDinamica.CommandText := ' SELECT CP.VL_COMPRA, SUM(CP.VL_PARCELA), '+
...
function TfrmContasPagarTingimento.UltimoDia: integer;
var Date: TDateTime;
begin
...
procedure TfrmContasPagarTingimento.VerificaTotais;
begin
  DM.cdsDinamica.Close;
  DM.cdsDinamica.IndexFieldNames:='';
  DM.cdsDinamica.CommandText := ' SELECT CP.VL_COMPRA, SUM(CP.VL_PARCELA), '+
  ...
  if iCont = 0 then
  begin
    if DM.cdsDinamica.Fields[1].AsFloat <> DM.cdsTingimentoVL_NOTA.AsFloat then
    begin
      Totais := False;
