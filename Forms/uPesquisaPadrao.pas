...
uses uDM, uRemessaProducao, uPrecoMalharia, uTingimento, uProgramacao,
  uQuebraFios;
...
begin
  if TB_PESQUISA = 'Preço-Malharia' then
  begin
    CDS.Close;
  ...
  if TB_PESQUISA = 'Preço-Tinturaria' then
  begin
    CDS.Close;
    CDS.CommandText := ConsultaSQL +sSqlJoin +WhereParametro;
    CDS.Params.ParamByName('P').Value := '%' + AnsiUpperCase(edtPesquisa.Text) + '%';
...
begin
  ...
  if TB_PESQUISA = 'Nota Fiscal Quebra' then
  begin
     sSqlJoin := ' LEFT OUTER JOIN TB_FORNECEDOR F ON (F.ID_FORNECEDOR = RP.ID_FORNECEDOR) '+
                 ' JOIN TB_FORNECEDOR FF ON (FF.ID_FORNECEDOR = RP.ID_FOR_REMESSA) '+
                 ' JOIN TB_FORNECEDOR FT ON (FT.ID_FORNECEDOR = RP.ID_FORN_TINT)    '+
  ...
  if TB_PESQUISA = 'Quebra' then
  begin
    DM.cdsDinamica.Close;
    DM.cdsDinamica.IndexFieldNames:='';
    DM.cdsDinamica.CommandText := ' SELECT C.ID_GRUPO_EMPRESA FROM TB_CLIENTE C '+
                                  ' WHERE C.ID_CLIENTE = '+QuotedStr(frmQuebraFio.DBEdit20.Text);
    DM.cdsDinamica.Open;
  ...
  if TB_PESQUISA = 'Nota Fiscal Tingimento' then
  begin
    DM.cdsDinamica.Close;
    DM.cdsDinamica.IndexFieldNames:='';
    DM.cdsDinamica.CommandText := ' SELECT C.ID_GRUPO_EMPRESA FROM TB_CLIENTE C '+
                              ' WHERE C.ID_CLIENTE = '+QuotedStr(frmTingimento.DBEdit3.Text);
    DM.cdsDinamica.Open;

    gpEmpresa := DM.cdsDinamica.Fields[0].AsString;

     sSqlJoin :=  ' LEFT OUTER JOIN TB_FORNECEDOR F ON (F.ID_FORNECEDOR = RP.ID_FORNECEDOR)'+
                  ' JOIN TB_FORNECEDOR FF ON (FF.ID_FORNECEDOR = RP.ID_FOR_TINGIMENTO) '+
                  ' JOIN TB_FORNECEDOR FT ON (FT.ID_FORNECEDOR = RP.ID_FORN_TINT)      '+
                  ' JOIN TB_CLIENTE C ON (C.ID_CLIENTE = RP.ID_CLIENTE)                '+
  ...
  if TB_PESQUISA = 'Programacao Tinturaria' then
  begin
    DM.cdsDinamica.Close;
    DM.cdsDinamica.IndexFieldNames:='';
    DM.cdsDinamica.CommandText := ' SELECT C.ID_GRUPO_EMPRESA FROM TB_CLIENTE C '+
                              ' WHERE C.ID_CLIENTE = '+QuotedStr(frmProgramacao.DBEdit20.Text);
    DM.cdsDinamica.Open;

    gpEmpresa := DM.cdsDinamica.Fields[0].AsString;

     sSqlJoin :=  ' LEFT OUTER JOIN TB_FORNECEDOR F ON (F.ID_FORNECEDOR = RP.ID_FOR_TINTURARIA)          '+
  ...
  if TB_PESQUISA = 'Frete' then
  begin
     sSqlJoin :=  ' JOIN TB_GRUPO_EMPRESA GE ON (GE.ID_GRUPO_EMPRESA = F.ID_GRUPO)'+
