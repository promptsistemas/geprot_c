...
begin
  ...
  DM.cdsPrgProgramacaoDT_EMISSAO.AsDateTime      := cdsNotasLancadasDT_EMISSAO.AsDateTime;
...
begin
  ...
  begin
    ...
    ShowMessage('Status da Nota Fiscal Continua Como Programada. Acione o Suporte Caso Precise Alterar Status Para Não Programada.');
...
begin
  if DM.cdsItemprg.RecordCount = 0 then
  begin
   ShowMessage('Nenhum Item Lançado Para Este Lançamento. Impossivél Finalizar');
   Exit
  end;
...
begin
  ...
  begin
    ...
    if Application.MessageBox('Deseja Informar Observação?','Confirmação',MB_YESNO+MB_ICONQUESTION) = idyes then
    begin
...
begin
  ...
  begin
    ...
    QRLabel24.Caption := ' RESTAM SALDO DESTA NOTA À PROGRAMAR DE  '+IntToStr(iTotal)+' ROLOS NO TOTAL DE '+FloatToStr(iPesoRestante)+' KG' ;
  end
  else
  begin
    QRLabel24.Visible := True;
    QRLabel24.Caption := ' NÃO RESTAM SALDO DESTA NOTA À PROGRAMAR ' ;
  end;
...
begin
  ...
  DM.cdsPrgProgramacaoSTATUS.Value := 'A'; //PROGRAMADA;
...
begin
  if NOT (QtSaldo = 0) then
  begin
    if DM.cdsItemprgQUANT.AsFloat > QtSaldo then
    begin
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        ShowMessage('Quantidade Rolos Não Pode Ser Zerada.');
        DBEdit17.SetFocus;
        Exit;
      end;
...
//    BB_CANC.SetFocus;
  ...
  begin
    ...
    begin
     ...
     begin
       ShowMessage('Cliente Não Pode Ser Diferente Para Este Lote.');
...
//    BB_CANC.SetFocus;
  ...
  DM.cdsTabelaFretes.CommandText := ' SELECT  F.ID_FRETE, F.ID_GRUPO, F.ID_TRANSPORTADORA, F.VL_FRENTE, GE.NOME_DO_GRUPO, T.NOME_TRANSPORTADORA '+
                                    ' FROM TB_FRETE F '+
                                    ...
                                    ' WHERE F.ID_GRUPO = '+DM.cdsPrgProgramacaoID_GRUPO_EMPRESA.AsString;
  DM.cdsTabelaFretes.Open;
  if DM.cdsTabelaFretes.Locate('ID_TRANSPORTADORA',DM.cdsPrgProgramacaoID_TRANSPORTADORA.AsInteger,[]) then
  begin
    DM.cdsPrgProgramacaoID_TRANSPORTADORA.AsInteger := DM.cdsTabelaFretesID_TRANSPORTADORA.AsInteger;
    DM.cdsPrgProgramacaoNOME_TRANSPORTADORA.AsString:= DM.cdsTabelaFretesNOME_TRANSPORTADORA.AsString;
    DBEdit19.SetFocus;
  end
  else
  begin
...
//    BB_CANC.SetFocus;
  ...
  begin
    ...
    begin
       ...
       begin
         ShowMessage('Tinturaria Não Pode Ser Diferente Para Este Lote.');
         BB_CANCELAR.SetFocus;
         Exit;
       end
       else
...
procedure TfrmProgramacao.DBNavigator1Click(Sender: TObject;
  Button: TNavigateBtn);
begin
  GerenciaBotoes2;
end;

procedure TfrmProgramacao.DBNavigator1DblClick(Sender: TObject);
begin
  GerenciaBotoes2;
end;
...
begin
  ...
  begin
    ShowMessage('Nenhum Registro Encontrado, Lançamento Será Cancelado');
    P_NOTAS_LANCADAS.Visible := False;
...
begin
  ...
  DM.cdsDinamica.Close;
  DM.cdsDinamica.IndexFieldNames:='';
  DM.cdsDinamica.CommandText := ' SELECT SUM(IP.PESO * IP.VL_UNIT) FROM TB_ITEM_PRG IP WHERE IP.ID_PRG_TINTURARIA = '+DM.cdsPrgProgramacaoID_PRG_TINTURARIA.AsString;
  DM.cdsDinamica.Open;
  DM.cdsPrgProgramacao.Edit;
  DM.cdsPrgProgramacaoVL_NOTA.AsFloat := DM.cdsDinamica.Fields[0].AsFloat;
...
begin
  ...
  cdsArtigo.Params[4].AsInteger := DM.cdsPrgProgramacaoNR_NOTA_REMESSA.AsInteger;
  cdsArtigo.Open;
  Pesquisa(cdsArtigo,['NOME_ARTIGO','PESO','ROLOS','QT_SALDO','QT_SALDO_PESO'],['Descrição Artigo','Código','Faturado','Rolos'],[15,12,12,12,12],'','','','','Artigo','Artigo');
  DM.cdsItemprgID_ARTIGO.AsInteger         := cdsArtigoID_ARTIGO.AsInteger;
  DM.cdsItemprgNOME_ARTIGO.AsString        := cdsArtigoNOME_ARTIGO.AsString;
  DM.cdsItemprgID_CLIENTE.AsInteger        := DM.cdsPrgProgramacaoID_CLIENTE.AsInteger;
  DM.cdsItemprgID_PRG_TINTURARIA.AsInteger := DM.cdsPrgProgramacaoID_PRG_TINTURARIA.AsInteger;
  ...
  DM.cdsItemprgTUBULAR.Value               := DM.cdsArtigoTUBULAR.Value;
  DM.cdsItemprgFELPAR.Value                := DM.cdsArtigoFELPAR.Value;
  Rolos                                    := cdsArtigoROLOS.AsFloat;

  if Qt_prg = NULL then
  begin
    Qt_prg := 0;
  end;
  if QtSaldo = null then
  begin
    QtSaldo := 0;
  end;

  QtSaldo := (Rolos-Qt_prg);
...
begin
  ...
  Pesquisa(cdsCor,['DESCRICAO','CODIGO','VALOR'],['Cor','Código','Valor'],[30,6,8],'','','','','Cores','Cores');
  DM.cdsItemprgID_ARTIGO.AsInteger                    := cdsArtigoID_ARTIGO.AsInteger;
  DM.cdsItemprgID_CORES.AsInteger                     := cdsCorID_CORES.AsInteger;
  DM.cdsItemprgCODIGO.AsInteger                       := cdsCorCODIGO.AsInteger;
  DM.cdsItemprgVL_UNIT.AsFloat                        := cdsCorVALOR.AsFloat;
...
begin
  ...
  DM.cdsTabelaFretes.CommandText := ' SELECT  F.ID_FRETE, F.ID_GRUPO, F.ID_TRANSPORTADORA, F.VL_FRENTE, GE.NOME_DO_GRUPO, T.NOME_TRANSPORTADORA '+
                                    ...
                                    ' WHERE F.ID_GRUPO = '+DM.cdsPrgProgramacaoID_GRUPO_EMPRESA.AsString;
  DM.cdsTabelaFretes.Open;
  Pesquisa(DM.cdsTabelaFretes,['NOME_TRANSPORTADORA','ID_TRANSPORTADORA'],['Nome Transportadora','Código'],[40,4],'','','','','Transportadora','Transportadora');
  DM.cdsPrgProgramacaoID_TRANSPORTADORA.AsInteger := DM.cdsTabelaFretesID_TRANSPORTADORA.AsInteger;
  DM.cdsPrgProgramacaoNOME_TRANSPORTADORA.AsString:= DM.cdsTabelaFretesNOME_TRANSPORTADORA.AsString;
  DBEdit19.SetFocus;
end;
