object frmContasPagarMalharia: TfrmContasPagarMalharia
  Left = 0
  Top = 0
  BorderIcons = []
  BorderStyle = bsNone
  Caption = 'Contas a Pagar Malharia'
  ClientHeight = 508
  ClientWidth = 649
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  OldCreateOrder = False
  Position = poScreenCenter
  OnClose = FormClose
  OnKeyDown = FormKeyDown
  OnKeyPress = FormKeyPress
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object Panel1: TPanel
    Left = 0
    Top = 0
    Width = 649
    Height = 34
    Align = alTop
    Color = clBlue
    Font.Charset = ANSI_CHARSET
    Font.Color = clBlue
    Font.Height = -21
    Font.Name = 'Segoe UI Semibold'
    Font.Style = [fsBold]
    ParentBackground = False
    ParentFont = False
    TabOrder = 0
    object Label5: TLabel
      Left = 156
      Top = -1
      Width = 340
      Height = 32
      Align = alCustom
      Caption = 'CONTAS A PAGAR MALHARIA'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWhite
      Font.Height = -24
      Font.Name = 'Segoe UI'
      Font.Style = [fsBold, fsItalic]
      ParentFont = False
    end
  end
  object Panel2: TPanel
    Left = 0
    Top = 34
    Width = 649
    Height = 474
    Align = alClient
    Color = 13034239
    ParentBackground = False
    TabOrder = 1
    object Shape1: TShape
      Left = 4
      Top = 0
      Width = 642
      Height = 85
      Brush.Color = 14680063
    end
    object Label3: TLabel
      Left = 8
      Top = 3
      Width = 76
      Height = 13
      Caption = 'C'#243'digo Cliente'
      Color = 14680063
      Font.Charset = ANSI_CHARSET
      Font.Color = clBlue
      Font.Height = -11
      Font.Name = 'Segoe UI Semibold'
      Font.Style = [fsBold]
      ParentColor = False
      ParentFont = False
    end
    object Label11: TLabel
      Left = 101
      Top = 3
      Width = 69
      Height = 13
      Caption = 'Nome Cliente'
      Color = 14680063
      Font.Charset = ANSI_CHARSET
      Font.Color = clBlue
      Font.Height = -11
      Font.Name = 'Segoe UI Semibold'
      Font.Style = [fsBold]
      ParentColor = False
      ParentFont = False
    end
    object DBText3: TDBText
      Left = 8
      Top = 19
      Width = 65
      Height = 17
      Color = 14680063
      DataField = 'ID_CLIENTE'
      DataSource = DM.dsRemessaProducao
      Font.Charset = ANSI_CHARSET
      Font.Color = clGreen
      Font.Height = -11
      Font.Name = 'Segoe UI Semibold'
      Font.Style = [fsBold]
      ParentColor = False
      ParentFont = False
    end
    object DBText4: TDBText
      Left = 100
      Top = 19
      Width = 384
      Height = 17
      Color = 14680063
      DataField = 'CLIENTE'
      DataSource = DM.dsRemessaProducao
      Font.Charset = ANSI_CHARSET
      Font.Color = clGreen
      Font.Height = -11
      Font.Name = 'Segoe UI Semibold'
      Font.Style = [fsBold]
      ParentColor = False
      ParentFont = False
    end
    object Label4: TLabel
      Left = 490
      Top = 3
      Width = 69
      Height = 13
      Caption = 'Data Emiss'#227'o'
      Color = 14680063
      Font.Charset = ANSI_CHARSET
      Font.Color = clBlue
      Font.Height = -11
      Font.Name = 'Segoe UI Semibold'
      Font.Style = [fsBold]
      ParentColor = False
      ParentFont = False
    end
    object Label1: TLabel
      Left = 564
      Top = 3
      Width = 72
      Height = 13
      Caption = 'N'#186' Nota Fiscal'
      Color = 14680063
      Font.Charset = ANSI_CHARSET
      Font.Color = clBlue
      Font.Height = -11
      Font.Name = 'Segoe UI Semibold'
      Font.Style = [fsBold]
      ParentColor = False
      ParentFont = False
    end
    object Label7: TLabel
      Left = 8
      Top = 42
      Width = 72
      Height = 13
      Caption = 'Vl. Nota Fiscal'
      Color = 14680063
      Font.Charset = ANSI_CHARSET
      Font.Color = clBlue
      Font.Height = -11
      Font.Name = 'Segoe UI Semibold'
      Font.Style = [fsBold]
      ParentColor = False
      ParentFont = False
    end
    object Label8: TLabel
      Left = 198
      Top = 42
      Width = 68
      Height = 13
      Caption = 'C'#243'd.Malharia'
      Color = 14680063
      Font.Charset = ANSI_CHARSET
      Font.Color = clBlue
      Font.Height = -11
      Font.Name = 'Segoe UI Semibold'
      Font.Style = [fsBold]
      ParentColor = False
      ParentFont = False
    end
    object Label20: TLabel
      Left = 281
      Top = 42
      Width = 44
      Height = 13
      Caption = 'Malharia'
      Color = 14680063
      Font.Charset = ANSI_CHARSET
      Font.Color = clBlue
      Font.Height = -11
      Font.Name = 'Segoe UI Semibold'
      Font.Style = [fsBold]
      ParentColor = False
      ParentFont = False
    end
    object Label21: TLabel
      Left = 88
      Top = 42
      Width = 78
      Height = 13
      Caption = 'N'#186' NF Remessa'
      Color = 14680063
      Font.Charset = ANSI_CHARSET
      Font.Color = clBlue
      Font.Height = -11
      Font.Name = 'Segoe UI Semibold'
      Font.Style = [fsBold]
      ParentColor = False
      ParentFont = False
    end
    object DBText5: TDBText
      Left = 490
      Top = 19
      Width = 65
      Height = 17
      Color = 14680063
      DataField = 'DT_EMISSAO'
      DataSource = DM.dsRemessaProducao
      Font.Charset = ANSI_CHARSET
      Font.Color = clGreen
      Font.Height = -11
      Font.Name = 'Segoe UI Semibold'
      Font.Style = [fsBold]
      ParentColor = False
      ParentFont = False
    end
    object DBText6: TDBText
      Left = 564
      Top = 19
      Width = 65
      Height = 17
      Color = 14680063
      DataField = 'NR_NOTA_FISCAL'
      DataSource = DM.dsRemessaProducao
      Font.Charset = ANSI_CHARSET
      Font.Color = clGreen
      Font.Height = -11
      Font.Name = 'Segoe UI Semibold'
      Font.Style = [fsBold]
      ParentColor = False
      ParentFont = False
    end
    object DBText7: TDBText
      Left = 8
      Top = 59
      Width = 65
      Height = 17
      Color = 14680063
      DataField = 'VL_NOTA'
      DataSource = DM.dsRemessaProducao
      Font.Charset = ANSI_CHARSET
      Font.Color = clGreen
      Font.Height = -11
      Font.Name = 'Segoe UI Semibold'
      Font.Style = [fsBold]
      ParentColor = False
      ParentFont = False
    end
    object DBText8: TDBText
      Left = 88
      Top = 59
      Width = 65
      Height = 17
      Color = 14680063
      DataField = 'NR_NOTA_FISCAL'
      DataSource = DM.dsRemessaProducao
      Font.Charset = ANSI_CHARSET
      Font.Color = clGreen
      Font.Height = -11
      Font.Name = 'Segoe UI Semibold'
      Font.Style = [fsBold]
      ParentColor = False
      ParentFont = False
    end
    object DBText9: TDBText
      Left = 197
      Top = 59
      Width = 65
      Height = 17
      Color = 14680063
      DataField = 'ID_FOR_REMESSA'
      DataSource = DM.dsRemessaProducao
      Font.Charset = ANSI_CHARSET
      Font.Color = clGreen
      Font.Height = -11
      Font.Name = 'Segoe UI Semibold'
      Font.Style = [fsBold]
      ParentColor = False
      ParentFont = False
    end
    object DBText10: TDBText
      Left = 281
      Top = 59
      Width = 355
      Height = 17
      Color = 14680063
      DataField = 'NOME_MALHARIA'
      DataSource = DM.dsRemessaProducao
      Font.Charset = ANSI_CHARSET
      Font.Color = clGreen
      Font.Height = -11
      Font.Name = 'Segoe UI Semibold'
      Font.Style = [fsBold]
      ParentColor = False
      ParentFont = False
    end
    object Shape2: TShape
      Left = 4
      Top = 120
      Width = 642
      Height = 62
      Brush.Color = 14609886
    end
    object Label9: TLabel
      Left = 221
      Top = 129
      Width = 164
      Height = 15
      Caption = 'Selecione Plano de Pagamento'
      Font.Charset = ANSI_CHARSET
      Font.Color = clBlue
      Font.Height = -12
      Font.Name = 'Segoe UI Semibold'
      Font.Style = [fsBold]
      ParentFont = False
      Visible = False
    end
    object Label12: TLabel
      Left = 8
      Top = 132
      Width = 79
      Height = 15
      Caption = 'Quant Parcelas'
      Color = 14609886
      Font.Charset = ANSI_CHARSET
      Font.Color = clBlue
      Font.Height = -12
      Font.Name = 'Segoe UI Semibold'
      Font.Style = [fsBold]
      ParentColor = False
      ParentFont = False
    end
    object BB_SAIR: TBitBtn
      Left = 273
      Top = 409
      Width = 55
      Height = 52
      Caption = '&Sair'
      Font.Charset = ANSI_CHARSET
      Font.Color = clBlue
      Font.Height = -12
      Font.Name = 'Segoe UI Semibold'
      Font.Style = []
      Glyph.Data = {
        
        F8F8FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      Layout = blGlyphTop
      NumGlyphs = 2
      ParentFont = False
      TabOrder = 0
      OnClick = BB_SAIRClick
      OnKeyDown = FormKeyDown
    end
    object Panel3: TPanel
      Left = 1
      Top = 88
      Width = 647
      Height = 30
      Color = clBlue
      Font.Charset = ANSI_CHARSET
      Font.Color = clBlue
      Font.Height = -21
      Font.Name = 'Segoe UI Semibold'
      Font.Style = [fsBold]
      ParentBackground = False
      ParentFont = False
      TabOrder = 1
      object Label6: TLabel
        Left = 155
        Top = 3
        Width = 236
        Height = 25
        Align = alCustom
        Caption = 'PLANOS DE PAGAMENTOS'
        Font.Charset = ANSI_CHARSET
        Font.Color = clYellow
        Font.Height = -19
        Font.Name = 'Segoe UI'
        Font.Style = [fsBold, fsItalic]
        ParentFont = False
      end
    end
    object DBLookupComboBox1: TDBLookupComboBox
      Left = 221
      Top = 145
      Width = 164
      Height = 25
      Color = clInfoBk
      Font.Charset = ANSI_CHARSET
      Font.Color = clNavy
      Font.Height = -13
      Font.Name = 'Segoe UI'
      Font.Style = [fsBold, fsItalic]
      KeyField = 'ID_PLANO_PAGAMENTO'
      ListField = 'DESCRICAO_PAGAMENTO'
      ListFieldIndex = 1
      ListSource = DM.dsPlanoPagamento
      ParentFont = False
      TabOrder = 2
      Visible = False
      OnKeyDown = FormKeyDown
    end
    object BitBtn2: TBitBtn
      Left = 136
      Top = 124
      Width = 500
      Height = 57
      Caption = '&Gerar Parcelas'
      Font.Charset = ANSI_CHARSET
      Font.Color = clBlue
      Font.Height = -11
      Font.Name = 'Segoe UI Semibold'
      Font.Style = [fsBold]
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      Layout = blGlyphTop
      NumGlyphs = 2
      ParentFont = False
      TabOrder = 3
      Visible = False
      OnClick = BitBtn2Click
      OnKeyDown = FormKeyDown
    end
    object Panel4: TPanel
      Left = 3
      Top = 185
      Width = 647
      Height = 30
      Color = clBlue
      Font.Charset = ANSI_CHARSET
      Font.Color = clBlue
      Font.Height = -21
      Font.Name = 'Segoe UI Semibold'
      Font.Style = [fsBold]
      ParentBackground = False
      ParentFont = False
      TabOrder = 4
      object Label13: TLabel
        Left = 155
        Top = 3
        Width = 258
        Height = 25
        Align = alCustom
        Caption = 'PAGAMENTOS REGISTRADOS'
        Font.Charset = ANSI_CHARSET
        Font.Color = clYellow
        Font.Height = -19
        Font.Name = 'Segoe UI'
        Font.Style = [fsBold, fsItalic]
        ParentFont = False
      end
    end
    object DBGrid1: TDBGrid
      Left = 4
      Top = 218
      Width = 642
      Height = 135
      Ctl3D = False
      DataSource = dsContasPagar
      FixedColor = 15532031
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit, dgTitleClick, dgTitleHotTrack]
      ParentColor = True
      ParentCtl3D = False
      ParentFont = False
      ReadOnly = True
      TabOrder = 5
      TitleFont.Charset = ANSI_CHARSET
      TitleFont.Color = clBlue
      TitleFont.Height = -11
      TitleFont.Name = 'Segoe UI'
      TitleFont.Style = []
      OnKeyDown = FormKeyDown
      Columns = <
        item
          Expanded = False
          FieldName = 'DT_EMISSAO'
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'DT_VENCIMENTO'
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'QT_DIAS'
          Width = 47
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'QT_PARCELA'
          Width = 47
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'NR_DOC'
          Width = 74
          Visible = True
        end
        item
          Color = 15332329
          Expanded = False
          FieldName = 'VL_PARCELA'
          Font.Charset = ANSI_CHARSET
          Font.Color = clNavy
          Font.Height = -11
          Font.Name = 'Segoe UI Semibold'
          Font.Style = [fsBold]
          Width = 82
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'NR_PARCELA'
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'DESCRICAO_PAGAMENTO'
          Width = 133
          Visible = True
        end>
    end
    object BB_INCLUIR: TBitBtn
      Left = 4
      Top = 409
      Width = 54
      Height = 52
      Caption = '&Incluir'
      Font.Charset = ANSI_CHARSET
      Font.Color = clBlue
      Font.Height = -12
      Font.Name = 'Segoe UI Semibold'
      Font.Style = []
      Glyph.Data = {
        
        00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
      Layout = blGlyphTop
      ParentFont = False
      TabOrder = 6
      OnClick = BB_INCLUIRClick
      OnKeyDown = FormKeyDown
    end
    object BB_ALTERAR: TBitBtn
      Left = 57
      Top = 409
      Width = 55
      Height = 52
      Caption = '&Alterar'
      Font.Charset = ANSI_CHARSET
      Font.Color = clGreen
      Font.Height = -12
      Font.Name = 'Segoe UI Semibold'
      Font.Style = []
      Glyph.Data = {
        
        00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
      Layout = blGlyphTop
      ParentFont = False
      TabOrder = 7
      OnClick = BB_ALTERARClick
      OnKeyDown = FormKeyDown
    end
    object BB_EXCLUIR: TBitBtn
      Left = 111
      Top = 409
      Width = 55
      Height = 52
      Caption = 'E&xcluir'
      Font.Charset = ANSI_CHARSET
      Font.Color = clRed
      Font.Height = -12
      Font.Name = 'Segoe UI Semibold'
      Font.Style = []
      Glyph.Data = {
        
        BF82FF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
      Layout = blGlyphTop
      ParentFont = False
      TabOrder = 8
      OnClick = BB_EXCLUIRClick
      OnKeyDown = FormKeyDown
    end
    object P_PAGAMENTOS: TPanel
      Left = 4
      Top = 355
      Width = 642
      Height = 46
      Color = clInfoBk
      Enabled = False
      ParentBackground = False
      TabOrder = 9
      object Label14: TLabel
        Left = 4
        Top = 2
        Width = 71
        Height = 15
        Caption = 'Data Emiss'#227'o'
        FocusControl = DBEdit1
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlue
        Font.Height = -12
        Font.Name = 'Segoe UI Semibold'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object Label15: TLabel
        Left = 86
        Top = 2
        Width = 79
        Height = 15
        Caption = 'Data Vencimto'
        FocusControl = DBEdit2
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlue
        Font.Height = -12
        Font.Name = 'Segoe UI Semibold'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object Label16: TLabel
        Left = 172
        Top = 2
        Width = 39
        Height = 15
        Caption = 'Qt Dias'
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlue
        Font.Height = -12
        Font.Name = 'Segoe UI Semibold'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object Label17: TLabel
        Left = 220
        Top = 2
        Width = 39
        Height = 15
        Caption = 'N'#186' Doc'
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlue
        Font.Height = -12
        Font.Name = 'Segoe UI Semibold'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object Label18: TLabel
        Left = 515
        Top = 2
        Width = 52
        Height = 15
        Caption = 'Vl Parcela'
        FocusControl = DBEdit5
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlue
        Font.Height = -12
        Font.Name = 'Segoe UI Semibold'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object DBText12: TDBText
        Left = 174
        Top = 22
        Width = 29
        Height = 17
        DataField = 'QT_DIAS'
        DataSource = dsContasPagar
        Font.Charset = ANSI_CHARSET
        Font.Color = clGreen
        Font.Height = -12
        Font.Name = 'Segoe UI Semibold'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object DBText13: TDBText
        Left = 217
        Top = 22
        Width = 65
        Height = 17
        DataField = 'NR_DOC'
        DataSource = dsContasPagar
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlue
        Font.Height = -13
        Font.Name = 'Segoe UI Semibold'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object DBText14: TDBText
        Left = 284
        Top = 22
        Width = 65
        Height = 17
        DataField = 'QT_PARCELA'
        DataSource = dsContasPagar
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlue
        Font.Height = -13
        Font.Name = 'Segoe UI Semibold'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object Label19: TLabel
        Left = 284
        Top = 2
        Width = 42
        Height = 15
        Caption = 'Qt. Parc'
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlue
        Font.Height = -12
        Font.Name = 'Segoe UI Semibold'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object Label22: TLabel
        Left = 379
        Top = 2
        Width = 98
        Height = 15
        Caption = 'Forma Pagamento'
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlue
        Font.Height = -12
        Font.Name = 'Segoe UI Semibold'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object DBText15: TDBText
        Left = 379
        Top = 22
        Width = 65
        Height = 17
        DataField = 'DESCRICAO_PAGAMENTO'
        DataSource = dsContasPagar
        Font.Charset = ANSI_CHARSET
        Font.Color = clTeal
        Font.Height = -13
        Font.Name = 'Segoe UI Semibold'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object DBEdit1: TDBEdit
        Left = 4
        Top = 21
        Width = 80
        Height = 19
        Color = clMoneyGreen
        Ctl3D = False
        DataField = 'DT_EMISSAO'
        DataSource = dsContasPagar
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlue
        Font.Height = -11
        Font.Name = 'Segoe UI Semibold'
        Font.Style = [fsBold]
        ParentCtl3D = False
        ParentFont = False
        TabOrder = 0
        OnExit = DBEdit1Exit
        OnKeyPress = FormKeyPress
      end
      object DBEdit2: TDBEdit
        Left = 85
        Top = 21
        Width = 84
        Height = 19
        Color = clMoneyGreen
        Ctl3D = False
        DataField = 'DT_VENCIMENTO'
        DataSource = dsContasPagar
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlue
        Font.Height = -11
        Font.Name = 'Segoe UI Semibold'
        Font.Style = [fsBold]
        ParentCtl3D = False
        ParentFont = False
        TabOrder = 1
        OnExit = DBEdit2Exit
        OnKeyPress = FormKeyPress
      end
      object DBEdit5: TDBEdit
        Left = 515
        Top = 21
        Width = 98
        Height = 21
        Color = 16316664
        Ctl3D = False
        DataField = 'VL_PARCELA'
        DataSource = dsContasPagar
        Font.Charset = ANSI_CHARSET
        Font.Color = clGreen
        Font.Height = -12
        Font.Name = 'Segoe UI Semibold'
        Font.Style = [fsBold]
        ParentCtl3D = False
        ParentFont = False
        TabOrder = 2
        OnEnter = DBEdit5Enter
        OnExit = DBEdit5Exit
        OnKeyPress = FormKeyPress
      end
    end
    object GB_SALDO: TGroupBox
      Left = 350
      Top = 403
      Width = 296
      Height = 68
      Caption = 'Saldo das Parcelas'
      Color = clCream
      Font.Charset = ANSI_CHARSET
      Font.Color = clGreen
      Font.Height = -13
      Font.Name = 'Segoe UI'
      Font.Style = []
      ParentBackground = False
      ParentColor = False
      ParentFont = False
      TabOrder = 10
      Visible = False
      object L_SALDO: TLabel
        Left = 7
        Top = 18
        Width = 139
        Height = 46
        Caption = 'R$ 0,00'
        Font.Charset = ANSI_CHARSET
        Font.Color = clTeal
        Font.Height = -40
        Font.Name = 'Arial Narrow'
        Font.Style = [fsBold]
        ParentFont = False
      end
    end
    object BB_CANCELAR: TBitBtn
      Left = 219
      Top = 409
      Width = 55
      Height = 52
      Caption = 'Cancela&r'
      Font.Charset = ANSI_CHARSET
      Font.Color = 10172223
      Font.Height = -12
      Font.Name = 'Segoe UI Semibold'
      Font.Style = []
      Glyph.Data = {
        
        95FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
      Layout = blGlyphTop
      ParentFont = False
      TabOrder = 11
      OnClick = BB_CANCELARClick
      OnKeyDown = FormKeyDown
    end
    object BB_DEL: TBitBtn
      Left = 165
      Top = 409
      Width = 55
      Height = 52
      Caption = 'Del Item'
      Font.Charset = ANSI_CHARSET
      Font.Color = clRed
      Font.Height = -12
      Font.Name = 'Segoe UI Semibold'
      Font.Style = []
      Glyph.Data = {
        
        29EAFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
      Layout = blGlyphTop
      ParentFont = False
      TabOrder = 12
      OnClick = BB_DELClick
      OnKeyDown = FormKeyDown
    end
    object DBEdit3: TDBEdit
      Left = 8
      Top = 149
      Width = 98
      Height = 21
      Color = 16316664
      Ctl3D = False
      DataField = 'QT_PARCELAS'
      DataSource = DM.dsPlanoPagamento
      Font.Charset = ANSI_CHARSET
      Font.Color = clGreen
      Font.Height = -12
      Font.Name = 'Segoe UI Semibold'
      Font.Style = [fsBold]
      ParentCtl3D = False
      ParentFont = False
      TabOrder = 13
      OnExit = DBEdit3Exit
      OnKeyDown = FormKeyDown
      OnKeyPress = FormKeyPress
    end
  end
  object dspContasPagar: TDataSetProvider
    DataSet = qContasPagar
    Options = [poIncFieldProps, poAllowMultiRecordUpdates, poAutoRefresh, poAllowCommandText, poUseQuoteChar]
    Left = 232
    Top = 274
  end
  object cdsContasPagar: TClientDataSet
    Aggregates = <>
    Params = <
      item
        DataType = ftUnknown
        Name = 'ID_CLIENTE'
        ParamType = ptInput
      end
      item
        DataType = ftUnknown
        Name = 'DT_EMISSAO'
        ParamType = ptInput
      end
      item
        DataType = ftUnknown
        Name = 'ID_FORNECEDOR'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Name = 'NR_NF'
        ParamType = ptInput
      end>
    ProviderName = 'dspContasPagar'
    Left = 260
    Top = 273
    object cdsContasPagarID_CONTAS_PAGAR: TIntegerField
      FieldName = 'ID_CONTAS_PAGAR'
      Required = True
    end
    object cdsContasPagarID_CLIENTE: TIntegerField
      FieldName = 'ID_CLIENTE'
      Required = True
    end
    object cdsContasPagarID_PLANO_PAGAMENTO: TIntegerField
      FieldName = 'ID_PLANO_PAGAMENTO'
      Required = True
    end
    object cdsContasPagarID_USUARIO: TIntegerField
      FieldName = 'ID_USUARIO'
      Required = True
    end
    object cdsContasPagarID_LOJA: TIntegerField
      FieldName = 'ID_LOJA'
      Required = True
    end
    object cdsContasPagarDT_EMISSAO: TDateField
      DisplayLabel = 'Data Emiss'#227'o'
      FieldName = 'DT_EMISSAO'
      EditMask = '##/##/####'
    end
    object cdsContasPagarDT_VENCIMENTO: TDateField
      DisplayLabel = 'Data Vencimento'
      FieldName = 'DT_VENCIMENTO'
      EditMask = '##/##/####'
    end
    object cdsContasPagarDT_PAGAMENTO: TDateField
      DisplayLabel = 'Data Pagamento'
      FieldName = 'DT_PAGAMENTO'
      EditMask = '##/##/####'
    end
    object cdsContasPagarQT_DIAS: TIntegerField
      DisplayLabel = 'Qt Dias'
      FieldName = 'QT_DIAS'
    end
    object cdsContasPagarQT_DIAS_ATRAZO: TIntegerField
      DisplayLabel = 'Atrazo'
      FieldName = 'QT_DIAS_ATRAZO'
    end
    object cdsContasPagarQT_PARCELA: TIntegerField
      DisplayLabel = 'Qt Parc'
      FieldName = 'QT_PARCELA'
    end
    object cdsContasPagarNR_DOC: TStringField
      DisplayLabel = 'N'#186' Doc'
      FieldName = 'NR_DOC'
      Size = 15
    end
    object cdsContasPagarVL_COMPRA: TFMTBCDField
      DisplayLabel = 'Vl.Nota'
      FieldName = 'VL_COMPRA'
      DisplayFormat = '###,###0.000'
      EditFormat = '###,###0.000'
      Precision = 18
      Size = 2
    end
    object cdsContasPagarVL_PARCELA: TFMTBCDField
      DisplayLabel = 'Vl Parcela'
      FieldName = 'VL_PARCELA'
      DisplayFormat = '###,###0.000'
      EditFormat = '###,###0.000'
      Precision = 18
      Size = 2
    end
    object cdsContasPagarNR_PARCELA: TStringField
      DisplayLabel = 'N'#186' Parcela'
      FieldName = 'NR_PARCELA'
      Size = 10
    end
    object cdsContasPagarSTATUS: TStringField
      DisplayLabel = 'Status'
      FieldName = 'STATUS'
      FixedChar = True
      Size = 1
    end
    object cdsContasPagarDESCRICAO_PAGAMENTO: TStringField
      DisplayLabel = 'Forma Pagamento'
      FieldName = 'DESCRICAO_PAGAMENTO'
      ProviderFlags = []
      Size = 40
    end
    object cdsContasPagarCLIENTE: TStringField
      DisplayLabel = 'Cliente'
      FieldName = 'CLIENTE'
      ProviderFlags = []
      Size = 50
    end
    object cdsContasPagarFORNECEDOR: TStringField
      DisplayLabel = 'Fornecedor'
      FieldName = 'FORNECEDOR'
      ProviderFlags = []
      Size = 50
    end
    object cdsContasPagarFORN_REMESSA: TStringField
      DisplayLabel = 'Malharia'
      FieldName = 'FORN_REMESSA'
      ProviderFlags = []
      Size = 50
    end
    object cdsContasPagarNOME_USUARIO: TStringField
      DisplayLabel = 'Usuario'
      FieldName = 'NOME_USUARIO'
      ProviderFlags = []
      Size = 30
    end
    object cdsContasPagarEMPRESA: TStringField
      DisplayLabel = 'Empresa'
      FieldName = 'EMPRESA'
      ProviderFlags = []
      Size = 35
    end
    object cdsContasPagarID_FORNECEDOR: TIntegerField
      FieldName = 'ID_FORNECEDOR'
      Required = True
    end
    object cdsContasPagarID_FORN_REM: TIntegerField
      FieldName = 'ID_FORN_REM'
      Required = True
    end
    object cdsContasPagarNR_NOTA_FISCAL: TIntegerField
      FieldName = 'NR_NOTA_FISCAL'
    end
    object cdsContasPagarNR_NF_REMESSA: TIntegerField
      FieldName = 'NR_NF_REMESSA'
    end
    object cdsContasPagarTP_MOV: TStringField
      FieldName = 'TP_MOV'
      FixedChar = True
      Size = 1
    end
    object cdsContasPagarORIGEM: TStringField
      FieldName = 'ORIGEM'
    end
    object cdsContasPagarID_PRODUCAO: TIntegerField
      FieldName = 'ID_PRODUCAO'
    end
  end
  object dsContasPagar: TDataSource
    DataSet = cdsContasPagar
    Left = 288
    Top = 274
  end
  object qContasPagar: TSQLDataSet
    CommandText = 
      'SELECT'#13#10'    CP.ID_CONTAS_PAGAR    ,'#13#10'    CP.ID_PRODUCAO        ,' +
      #13#10'    CP.ID_CLIENTE         ,'#13#10'    CP.ID_PLANO_PAGAMENTO ,'#13#10'    ' +
      'CP.ID_USUARIO         ,'#13#10'    CP.ID_LOJA            ,'#13#10'    CP.ID_' +
      'FORNECEDOR,'#13#10'    CP.ID_FORN_REM,'#13#10'    CP.DT_EMISSAO         ,'#13#10' ' +
      '   CP.DT_VENCIMENTO      ,'#13#10'    CP.DT_PAGAMENTO       ,'#13#10'    CP.' +
      'QT_DIAS            ,'#13#10'    CP.QT_DIAS_ATRAZO     ,'#13#10'    CP.QT_PAR' +
      'CELA         ,'#13#10'    CP.NR_DOC             ,'#13#10'    CP.VL_COMPRA   ' +
      '       ,'#13#10'    CP.VL_PARCELA         ,'#13#10'    CP.NR_PARCELA        ' +
      ' ,'#13#10'    CP.STATUS             ,'#13#10'    CP.NR_NOTA_FISCAL,'#13#10'    CP.' +
      'NR_NF_REMESSA,'#13#10'    CP.ORIGEM,'#13#10'    CP.TP_MOV,'#13#10'    CP.ORIGEM,'#13#10 +
      '    PP.DESCRICAO_PAGAMENTO,'#13#10'    C.R_SOCIAL AS CLIENTE ,'#13#10'    F.' +
      'NOME AS FORNECEDOR  ,'#13#10'    FR.NOME AS FORN_REMESSA,'#13#10'    U.NOME_' +
      'USUARIO         ,'#13#10'    L.R_SOCIAL AS EMPRESA'#13#10'FROM TB_CONTAS_PAG' +
      'AR CP'#13#10'JOIN TB_PLANO_PAGAMENTO PP ON (PP.ID_PLANO_PAGAMENTO = CP' +
      '.ID_PLANO_PAGAMENTO)'#13#10'JOIN TB_CLIENTE C ON (C.ID_CLIENTE = CP.ID' +
      '_CLIENTE)'#13#10'JOIN TB_FORNECEDOR F ON (F.ID_FORNECEDOR = CP.ID_FORN' +
      'ECEDOR)'#13#10'JOIN TB_FORNECEDOR FR ON (FR.ID_FORNECEDOR = CP.ID_FORN' +
      '_REM)'#13#10'JOIN TB_USUARIO U ON (U.ID_USUARIO = CP.ID_USUARIO)'#13#10'JOIN' +
      ' TB_LOJAS L ON (L.ID_LOJA = CP.ID_LOJA)'#13#10'WHERE CP.ID_CLIENTE = :' +
      'ID_CLIENTE'#13#10'AND CP.DT_EMISSAO   = :DT_EMISSAO'#13#10'AND CP.ID_FORNECE' +
      'DOR  = :ID_FORNECEDOR'#13#10'AND CP.nr_nota_fiscal = :NR_NF'#13#10'AND CP.TP' +
      '_MOV = '#39'D'#39#13#10'ORDER BY CP.DT_VENCIMENTO'
    MaxBlobSize = -1
    Params = <
      item
        DataType = ftUnknown
        Name = 'ID_CLIENTE'
        ParamType = ptInput
      end
      item
        DataType = ftUnknown
        Name = 'DT_EMISSAO'
        ParamType = ptInput
      end
      item
        DataType = ftUnknown
        Name = 'ID_FORNECEDOR'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Name = 'NR_NF'
        ParamType = ptInput
      end>
    SQLConnection = DM.conexao
    Left = 204
    Top = 274
    object qContasPagarID_CONTAS_PAGAR: TIntegerField
      FieldName = 'ID_CONTAS_PAGAR'
      Required = True
    end
    object qContasPagarID_CLIENTE: TIntegerField
      FieldName = 'ID_CLIENTE'
      Required = True
    end
    object qContasPagarID_PLANO_PAGAMENTO: TIntegerField
      FieldName = 'ID_PLANO_PAGAMENTO'
      Required = True
    end
    object qContasPagarID_USUARIO: TIntegerField
      FieldName = 'ID_USUARIO'
      Required = True
    end
    object qContasPagarID_LOJA: TIntegerField
      FieldName = 'ID_LOJA'
      Required = True
    end
    object qContasPagarDT_EMISSAO: TDateField
      FieldName = 'DT_EMISSAO'
    end
    object qContasPagarDT_VENCIMENTO: TDateField
      FieldName = 'DT_VENCIMENTO'
    end
    object qContasPagarDT_PAGAMENTO: TDateField
      FieldName = 'DT_PAGAMENTO'
    end
    object qContasPagarQT_DIAS: TIntegerField
      FieldName = 'QT_DIAS'
    end
    object qContasPagarQT_DIAS_ATRAZO: TIntegerField
      FieldName = 'QT_DIAS_ATRAZO'
    end
    object qContasPagarQT_PARCELA: TIntegerField
      FieldName = 'QT_PARCELA'
    end
    object qContasPagarNR_DOC: TStringField
      FieldName = 'NR_DOC'
      Size = 15
    end
    object qContasPagarVL_COMPRA: TFMTBCDField
      FieldName = 'VL_COMPRA'
      Precision = 18
      Size = 2
    end
    object qContasPagarVL_PARCELA: TFMTBCDField
      FieldName = 'VL_PARCELA'
      Precision = 18
      Size = 2
    end
    object qContasPagarNR_PARCELA: TStringField
      FieldName = 'NR_PARCELA'
      Size = 10
    end
    object qContasPagarSTATUS: TStringField
      FieldName = 'STATUS'
      FixedChar = True
      Size = 1
    end
    object qContasPagarDESCRICAO_PAGAMENTO: TStringField
      FieldName = 'DESCRICAO_PAGAMENTO'
      ProviderFlags = []
      Size = 40
    end
    object qContasPagarCLIENTE: TStringField
      FieldName = 'CLIENTE'
      ProviderFlags = []
      Size = 50
    end
    object qContasPagarFORNECEDOR: TStringField
      FieldName = 'FORNECEDOR'
      ProviderFlags = []
      Size = 50
    end
    object qContasPagarFORN_REMESSA: TStringField
      FieldName = 'FORN_REMESSA'
      ProviderFlags = []
      Size = 50
    end
    object qContasPagarNOME_USUARIO: TStringField
      FieldName = 'NOME_USUARIO'
      ProviderFlags = []
      Size = 30
    end
    object qContasPagarEMPRESA: TStringField
      FieldName = 'EMPRESA'
      ProviderFlags = []
      Size = 35
    end
    object qContasPagarID_FORNECEDOR: TIntegerField
      FieldName = 'ID_FORNECEDOR'
      Required = True
    end
    object qContasPagarID_FORN_REM: TIntegerField
      FieldName = 'ID_FORN_REM'
      Required = True
    end
    object qContasPagarNR_NOTA_FISCAL: TIntegerField
      FieldName = 'NR_NOTA_FISCAL'
    end
    object qContasPagarNR_NF_REMESSA: TIntegerField
      FieldName = 'NR_NF_REMESSA'
    end
    object qContasPagarTP_MOV: TStringField
      FieldName = 'TP_MOV'
      FixedChar = True
      Size = 1
    end
    object qContasPagarORIGEM: TStringField
      FieldName = 'ORIGEM'
    end
    object qContasPagarID_PRODUCAO: TIntegerField
      FieldName = 'ID_PRODUCAO'
    end
  end
end
