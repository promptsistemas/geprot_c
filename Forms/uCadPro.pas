  ...
  begin
    if (DM.cdsProdutoTIPO_ALIQ.Value = 'T') and (DM.cdsProdutoALIQUOTA.AsFloat = 17) then begin
      DBComboBox1.Text          := 'T- 17,00 % - Tributado';
        ...
        DBComboBox1.Text := 'F - Substituição - Substituição';
        ...
        DBComboBox1.Text := 'N - Não Incidência - Não Incidência';
    end;
...
{  if DM.cdsProdutoCD_BARRA.IsNull then
  begin
    if Application.MessageBox('Você não Informou o Cód.Barra do Produto.     '+#13+
                              'Deseja que o Sistema Gere o Código Automático ?',
    ...
    begin
      ...
      ShowMessage('Código de Barra Gerado Com Sucesso !!!');
    end
    else
    begin
      ShowMessage('Código de Barra não Gerado.');
      DBEdit16.SetFocus;
    end;
  end;
  P_BOTOES.SetFocus;}
...
begin
  ...
  Pesquisa(DM.cdsFornecedor,['ID_FORNECEDOR','NOME'],['Cód.Fornecedor','Nome do Fornecedor '],[10,40],'','','','','Fornecedor','Fornecedor');
...
begin
  ...
  begin
    ShowMessage('Campo "Cod.Forn" de Preenchimento Obrigatório.');
    DBEdit6.SetFocus;
  end;
end;
...
begin
  ...
  begin
    if Application.MessageBox('Campo "Valor Custo" vai ser Zero?','Confirmação!!!!!!',MB_YESNO+MB_ICONQUESTION)= idyes then
    begin
      DBEdit12.SetFocus;
    end
    else
...
begin
  ...
  begin
    Application.MessageBox('Preço de Venda 1 igual ao Preço de Custo.','ATENÇÃO!!!!!!',MB_OK+MB_ICONEXCLAMATION);
    DBEdit13.SetFocus;
  end;
  if DBEdit13.Text = '' then
  begin
    Application.MessageBox('Preço de Venda 1 Vazio favor informar Preço.','ATENÇÃO!!!!!!',MB_OK+MB_ICONEXCLAMATION);
    DBEdit13.SetFocus;
  end;

  if DBEdit13.Text = '0,00' then
  begin
    if Application.MessageBox('Campo "Valor Venda 1 " vai ser Zero?','Confirmação!!!!!!',MB_YESNO+MB_ICONQUESTION)= idyes then
    begin
      DBEdit13.SetFocus;
    end
    else
...
begin
  ...
  begin
    Application.MessageBox('Campo "Desconto" não pode ser Vazio.','ATENÇÃO!!!!!!',MB_OK+MB_ICONEXCLAMATION);
    DBEdit21.SetFocus;
  end;
end;
...
{  if DM.cdsProdutoVL_VENDA_NF.AsFloat < DM.cdsProdutoVL_CUSTO.AsFloat then
  begin
    if Application.MessageBox('Preço para Nota Fiscal Ficou Menor que o Preço de Custo. Deseja Continuar?','Atenção',MB_YESNO+MB_ICONQUESTION)= idyes then
    begin
      DBEdit21.SetFocus;
    end
    else
...
begin
   ...
   begin
     ...
     begin
       ...
       begin
         ShowMessage('Cód.Barra Já Cadastrado Para o Produto.'+#13+
                     ' '+DM.cdsDinamica.Fields[1].AsString+'  Código '+DM.cdsDinamica.Fields[2].AsString+'-'+DM.cdsDinamica.Fields[3].AsString);
...
begin
    ...
    if DBComboBox1.Text = 'F - Substituição - Substituição' then
    begin
       DBComboBox1.Text := 'F - Substituição - Substituição';
    ...
    if DBComboBox1.Text = 'N - Não Incidência - Não Incidência' then
    begin
       DBComboBox1.Text := 'N - Não Incidência - Não Incidência';
...
begin
  ...
  begin
    ShowMessage(' Campo "NCM" de preenchimento Obrigatórido.');
    PageControl1.ActivePage := TabSheet1;
    DBEdit23.SetFocus;
  ...
  begin
    ShowMessage(' Campo "Origem da Mercadoria" de preenchimento Obrigatórido.');
  ...
  begin
    ShowMessage(' Campo "CST" de preenchimento Obrigatórido.');
    PageControl1.ActivePage := TabSheet2;
  ...
  if DM.cdsProdutoCD_BARRA.IsNull then
  begin
    ShowMessage(' Campo "Código de Barras" de preenchimento Obrigatórido.');
    PageControl1.ActivePage := TabSheet1;
    DBEdit16.SetFocus;
  ...
  begin
    ShowMessage(' Campo "Tipo Aliquota" de preenchimento Obrigatórido.');
    PageControl1.ActivePage := TabSheet2;
  ...
  begin
    ShowMessage(' Campo "Aliquota Fora do Estado" de preenchimento Obrigatórido.');
    PageControl1.ActivePage := TabSheet2;
    DBEdit32.SetFocus;
  ...
  begin
    ShowMessage(' Campo "CSOSN" de preenchimento Obrigatórido.');
    PageControl1.ActivePage := TabSheet2;
  ...
  begin
    ShowMessage(' Campo "CFOP Saida Dentro do Estado" de preenchimento Obrigatórido.');
  ...
  if DM.cdsProdutoCFOP_DENTRO_ESTADO.IsNull then
  begin
    ShowMessage(' Campo "CFOP Saida Fora do Estado" de preenchimento Obrigatórido.');
    PageControl1.ActivePage := TabSheet3;
...
begin
  ...
  if Application.MessageBox('Deseja Excluir Este Produto ?','Confirmação',MB_ICONQUESTION+MB_YESNO ) = idyes then
  begin
  ...
  begin
    ShowMessage('Produto Não Excluído');
  end;
    GerenciaBotoes;
...
begin
  if Application.MessageBox('Deseja Excluir Este Código ?','Confirmação',MB_ICONQUESTION+MB_YESNO ) = idyes then
  begin
    DM.cdsBarras.Delete;
  ...
  begin
    ShowMessage('Código Não Excluído');
  end;
    GerenciaBotoes2;
...
begin
  Pesquisa(DM.cdsProduto,['DESCRICAO','CD_BARRA','VL_VENDA_ATAC'],['None do Produto','Código','Vl.Venda'],
...
begin
  ...
  begin
    ShowMessage('Favor Informar Código de Barra Alternativo!');
    DBEdit36.SetFocus;
    Exit;
  end;
...
{  DM.cdsDinamica.Close;
  ...
  begin
    if Application.MessageBox('Existe(m) Produto(s) sem importar, Deseja Importar Produto(s) Agora?','Atenção',MB_OK+MB_YESNO)= idyes then
    begin
...
//  AssignFile (arq,'C:\Sintegra\Arquivos_PAF\PRODUTO.TXT');
  ...
  begin
    ...
    Write ( arq, AjustaStr ( DM.cdsProdutoID_GRUPO_PRODUTO.AsString, 6 ) ); //CAMPO 16 Cód. Grupo
...
//    qtEstoque:=ColocaTextoDir(qtEstoque,3,'12');
      ...
      Write ( arq, AjustaStr (qtEstoque,12 )); //40 Quantidade Estoque S 489 500 12 N Quantidade de estoque do produto, com 3 casas
      Write ( arq, AjustaStr ('0',3 )); //41 Prazo  Devolução. S 501 503 3 N
      Write ( arq, AjustaStr ('',7 )); //42 Cest S 504 510 7 N Obrigatório para produtos com Substituição Tributária a  partir de 01/04/2016
      Write ( arq, AjustaStr ('S',1 ));//      43 Controla Estoque S 511 511 1 N Controlar Estoque
      Write ( arq, AjustaStr ('',9 ));//      44 Código ANP S 512 520 9 N
...
begin
  If DM.cdsProdutoESTOQUE.AsInteger < 0  then // condição
  begin
  ...
  else if DM.cdsProdutoESTOQUE.AsInteger = 0  then // condição
  begin
...
begin
  ...
  begin
    Application.MessageBox('Preço de Venda 2 igual ao Preço de Custo.','ATENÇÃO!!!!!!',MB_OK+MB_ICONEXCLAMATION);
    DBEdit29.SetFocus;
  end;
  if DBEdit29.Text = '' then
  begin
    Application.MessageBox('Preço de Venda 2 Vazio favor informar Preço.','ATENÇÃO!!!!!!',MB_OK+MB_ICONEXCLAMATION);
    DBEdit29.SetFocus;
  end;

  if DBEdit29.Text = '0,00' then
  begin
    if Application.MessageBox('Campo "Valor Venda Atacado" vai ser Zero?','Confirmação!!!!!!',MB_YESNO+MB_ICONQUESTION)= idyes then
    begin
      DBEdit29.SetFocus;
    end
    else
