...
type
  TfrmRemessaProducao = class(TForm)
    Panel1: TPanel;
    DBText1: TDBText;
    Label17: TLabel;
    Label18: TLabel;
...
var
  frmRemessaProducao: TfrmRemessaProducao;
...
procedure TfrmRemessaProducao.BB_ALTERARClick(Sender: TObject);
begin
  if DM.cdsRemessaProducaoSTATUS.AsString = 'F' then
  begin
...
procedure TfrmRemessaProducao.BB_CANCClick(Sender: TObject);
begin
  DM.cdsItemRemessa.Cancel;
  SomaTotal;
  GerenciaBotoes2;

end;

procedure TfrmRemessaProducao.BB_CANCELARClick(Sender: TObject);
begin
  DM.cdsRemessaProducao.Cancel;
  GerenciaBotoes;
end;

procedure TfrmRemessaProducao.BB_EDITARClick(Sender: TObject);
begin
  if DM.cdsRemessaProducaoSTATUS.AsString = 'F' then
  begin
    ShowMessage('Nota Já Finalizada.');
...
procedure TfrmRemessaProducao.BB_EXCClick(Sender: TObject);
Var
...
begin
  ...
  begin
    ...
    NR_NOTA_FISCAL := DM.cdsItemRemessaNR_NOTA_FISCAL.AsInteger;
    ID_MATERIA     := DM.cdsItemRemessaID_MATERIA.AsInteger;
    ID_FORNECEDOR  := DM.cdsItemRemessaID_FORNECEDOR.AsInteger;
    ID_CLIENTE     := DM.cdsItemRemessaID_CLIENTE.AsInteger;
    ID_PRODUCAO    := DM.cdsItemRemessaID_PRODUCAO.AsInteger;

  if Application.MessageBox('Deseja Realmente Excluir. S/N ?','Confirmação',MB_ICONQUESTION+MB_YESNO ) = IDYES Then
  begin
    DM.cdsItemRemessa.Delete;
...
procedure TfrmRemessaProducao.BB_EXCLUIRClick(Sender: TObject);
begin
  if DM.cdsRemessaProducaoSTATUS.AsString = 'F' then
  begin
    ShowMessage('Nota Já Finalizada.');
  ...
  if DM.cdsItemRemessa.RecordCount > 0  then
  begin
  ...
  if DM.cdsRemessaProducao.RecordCount > 0 then
  begin
...
procedure TfrmRemessaProducao.BB_EXC_PAGClick(Sender: TObject);
begin
  if DM.cdsRemessaProducaoSTATUS.AsString = 'F' then
  begin
    ShowMessage('Nota Já Finalizada.');
  ...
  begin
    ShowMessage('Impossível Excluir Registro. Exite(m) Pagamento(s) Vinculados. ');
    exit;
  end;
...
procedure TfrmRemessaProducao.BB_GRAVARClick(Sender: TObject);
begin
  if Tipo = 'Insert' then
  begin
    ...
    DM.cdsRemessaProducaoID_REMESSA.AsInteger := DM.cdsDinamica.Fields[0].AsInteger + 1;

    if DM.cdsRemessaProducao.ApplyUpdates(0) <> 0 then
    begin
    ...
    begin
      DM.cdsRemessaProducao.DisableControls;
      DM.cdsRemessaProducao.ApplyUpdates(0);
//      DM.cdsRemessaProducao.Refresh;
      DM.cdsRemessaProducao.EnableControls;
      GerenCiaBotoes;
      GerenciaBotoes2;
      if DM.cdsRemessaProducao.RecordCount > 0 then
      begin
...
procedure TfrmRemessaProducao.BB_INCLUIRClick(Sender: TObject);
begin
  if DM.cdsRemessaProducaoSTATUS.AsString = 'F' then
  begin
    ShowMessage('Nota Já Finalizada.');
...
procedure TfrmRemessaProducao.BB_INC_PAGClick(Sender: TObject);
begin
  if DM.cdsRemessaProducaoSTATUS.AsString = 'F' then
  begin
    ShowMessage('Nota Já Finalizada.');
  ...
  if frmContasPagarMalharia = nil then
  begin
    frmContasPagarMalharia := TfrmContasPagarMalharia.Create(Application);
    frmContasPagarMalharia.ShowModal;
  end;

end;

procedure TfrmRemessaProducao.BB_NOVOClick(Sender: TObject);
begin
  PageControl1.ActivePage := TabSheet1;
  Tipo := 'Insert';

  DM.cdsRemessaProducao.Insert;
  DM.cdsRemessaProducaoSTATUS.Value := 'A';
  DM.cdsRemessaProducaoQUANT_NF.AsFloat := 0;
  DM.cdsRemessaProducaoQUANT_FATURADO.AsFloat := 0;
  DM.cdsRemessaProducaoQUANT_SALDO.AsFloat := 0;
  DM.cdsRemessaProducaoVL_NOTA.AsFloat := 0;
  DM.cdsRemessaProducaoVL_POR_FORA.AsFloat := 0;
  DM.cdsRemessaProducaoVL_TOTAL_NF.AsFloat := 0;
  DM.cdsRemessaProducaoNF_QUEBRA.AsString := 'N';
  cdsItemNotaFiscal.Close;
...
procedure TfrmRemessaProducao.BB_PESQUISARClick(Sender: TObject);
begin
  frmPesquisaPadrao := TfrmPesquisaPadrao.Create(self);
  frmPesquisaPadrao.TB_PESQUISA := 'Nota Fiscal Malharia';
  frmPesquisaPadrao.CDS := DM.cdsRemessaProducao;
...
{  Pesquisa(DM.cdsRemessaProducao,['NOME_MALHARIA','NR_NOTA_FISCAL','CLIENTE','DT_EMISSAO'],['Malharia','Nº Nota','Cliente','Data Emissão'],
...
procedure TfrmRemessaProducao.BB_SAIRClick(Sender: TObject);
begin
  Close;
end;

procedure TfrmRemessaProducao.BB_SALVARClick(Sender: TObject);
begin
  DM.cdsItemRemessa.Cancel;
  SomaTotal;
  GerenciaBotoes2;
...
procedure TfrmRemessaProducao.BB_VOL_PAGClick(Sender: TObject);
begin
  TabSheet4.PageControl.ActivePage:=TabSheet3;
end;

procedure TfrmRemessaProducao.BB_CONCLUIRClick(Sender: TObject);
begin
  ...
  begin
    ...
    begin
      DM.cdsRemessaProducao.Open;
      DM.cdsRemessaProducao.Edit;
      DM.cdsRemessaProducaoSTATUS.Value := 'F';
      DM.cdsRemessaProducao.ApplyUpdates(0);
      DM.cdsRemessaProducao.Refresh;
...
procedure TfrmRemessaProducao.BB_CONFIRMARClick(Sender: TObject);
Var
 idEmpresa : string;
begin
  ...
  DM.cdsItemRemessaID_ARTIGO.AsInteger        := cdsDinamicaFiosID_ARTIGO.AsInteger;
  DM.cdsItemRemessaID_CLIENTE.AsInteger       := cdsDinamicaFiosID_CLIENTE.AsInteger;
  DM.cdsItemRemessaID_FORNECEDOR.AsInteger    := cdsDinamicaFiosID_FORNCEDOR.AsInteger;
  DM.cdsItemRemessaID_FORN_MALHARIA.AsInteger := DM.cdsRemessaProducaoID_FOR_REMESSA.AsInteger;
  DM.cdsItemRemessaID_MATERIA.AsInteger       := cdsDinamicaFiosID_MATERIA_PRIMA.AsInteger;
  DM.cdsItemRemessaNR_NOTA_FISCAL.AsInteger   := cdsDinamicaFiosNR_NOTA_FISCAL.AsInteger;
  DM.cdsItemRemessaNR_NF_REMESSA.AsInteger    := cdsDinamicaFiosNR_NF_REMESSA.AsInteger;
  DM.cdsItemRemessaID_PRODUCAO.AsInteger      := cdsDinamicaFiosID_PRODUCAO.AsInteger;
  ...
  DM.cdsDinamica.CommandText := ' SELECT IP.VL_ARTIGO FROM TB_ITEM_PRECO_MALHARIA IP '+
                                ...
                                ' WHERE M.ID_MALHARIA = '+DM.cdsRemessaProducaoID_FOR_REMESSA.AsString+
                                ' AND M.ID_GRUPO_EMPRESA = '+idEmpresa+
                                ' AND IP.ID_ARTIGO =       '+DM.cdsItemRemessaID_ARTIGO.AsString;
  DM.cdsDinamica.Open;

  DM.cdsItemRemessaVL_UNIT_REAL.AsFloat := DM.cdsDinamica.Fields[0].AsFloat;

  P_SALDO_FIOS.Visible := False;
  DBEdit17.SetFocus;
end;

procedure TfrmRemessaProducao.B_CANCELARClick(Sender: TObject);
begin
 cdsDinamicaFios.Close;
 P_SALDO_FIOS.Visible := False;
 DBEdit14.SelectAll;
...
procedure TfrmRemessaProducao.DBEdit12Exit(Sender: TObject);
begin
  DM.cdsItemRemessaVL_TOTAL.AsFloat := (DM.cdsItemRemessaQUANT_FATURADO.AsFloat*DM.cdsItemRemessaVL_UNIT.AsFloat);
end;

procedure TfrmRemessaProducao.DBEdit13Enter(Sender: TObject);
begin
  if NOT(DM.cdsItemRemessaQUANT_FATURADO.IsNull) then
  begin
    DM.cdsItemRemessaVL_TOTAL_REAL.AsFloat := (DM.cdsItemRemessaQUANT_FATURADO.AsFloat*DM.cdsItemRemessaVL_UNIT_REAL.AsFloat)-
                                              (DM.cdsItemRemessaVL_TOTAL.AsFloat);

  end;

end;

procedure TfrmRemessaProducao.DBEdit13Exit(Sender: TObject);
begin
  ...
  begin
    ...
    begin
      ...
      DM.cdsDinamica.Open;
      DM.cdsItemRemessaID_ITEM_REMESSA.AsInteger := DM.cdsDinamica.Fields[0].AsInteger + 1;

      if DM.cdsItemRemessa.ApplyUpdates(0) <> 0 then
      begin
      ...
      begin
        DM.cdsItemRemessa.ApplyUpdates(0);
        cdsAtuSaldoFios.Params[0].AsInteger := DM.cdsItemRemessaNR_NOTA_FISCAL.AsInteger;
        cdsAtuSaldoFios.Params[1].AsInteger := DM.cdsItemRemessaID_MATERIA.AsInteger;
        cdsAtuSaldoFios.Params[2].AsInteger := DM.cdsItemRemessaID_FORNECEDOR.AsInteger;
        cdsAtuSaldoFios.Params[3].AsInteger := DM.cdsItemRemessaID_CLIENTE.AsInteger;
        cdsAtuSaldoFios.Params[4].AsInteger := DM.cdsItemRemessaID_PRODUCAO.AsInteger;
        cdsAtuSaldoFios.Execute;
        cdsItemProducao.Close;
        cdsItemProducao.Params[0].AsInteger :=  DM.cdsItemRemessaID_MATERIA.AsInteger;
        cdsItemProducao.Params[1].AsInteger :=  DM.cdsRemessaProducaoID_CLIENTE.AsInteger;
        cdsItemProducao.Open;
...
procedure TfrmRemessaProducao.DBEdit14Exit(Sender: TObject);
Var
 icount : string;
...
{  DM.cdsArtigo.Close;
  ...
  begin
    DM.cdsItemRemessaID_ARTIGO.AsInteger  := DM.cdsArtigoID_ARTIGO.AsInteger;
    DM.cdsItemRemessaNOME_ARTIGO.AsString := DM.cdsArtigoNOME_ARTIGO.AsString;
    //Edit17.SetFocus;
    P_SALDO_FIOS.Visible := True;
    cdsItemProducao.Close;

    cdsVerifcaSaldoFios.Params[0].AsInteger := DM.cdsRemessaProducaoID_CLIENTE.AsInteger;
    cdsVerifcaSaldoFios.Params[1].AsInteger := DM.cdsItemRemessaID_ARTIGO.AsInteger;
    cdsVerifcaSaldoFios.Params[2].AsInteger := DM.cdsRemessaProducaoID_FOR_REMESSA.AsInteger;
    cdsVerifcaSaldoFios.Execute;



    cdsDinamicaFios.Close;
    cdsDinamicaFios.CommandText :=  ' SELECT D.ID_PRODUCAO, P.NR_NOTA_FISCAL, P.NR_NF_REMESSA, D.ID_MATERIA_PRIMA, M.NOME_MATERIA, '+
                                    ' D.QUANT_FATURADO, D.ID_FORNCEDOR, F.NOME, D.ID_CLIENTE, C.R_SOCIAL, D.ID_ARTIGO, A.NOME_ARTIGO,'+
                                    ' D.QUANT_PROD, D.SALDO, D.QT_ARTIGO FROM DINAMICA_FIOS D '+
                                    ' LEFT OUTER JOIN TB_FORNECEDOR F ON (F.ID_FORNECEDOR = D.ID_FORNCEDOR) '+
                                    ' LEFT OUTER JOIN TB_CLIENTE C ON (C.ID_CLIENTE = D.ID_CLIENTE) '+
                                    ' LEFT OUTER JOIN TB_ARTIGO A ON (A.ID_ARTIGO = D.ID_ARTIGO)    '+
                                    ' LEFT OUTER JOIN TB_MATERIA_PRIMA M ON (M.ID_MATERIA = D.ID_MATERIA_PRIMA)'+
                                    ' LEFT OUTER JOIN TB_PRODUCAO P ON (P.ID_PRODUCAO = D.ID_PRODUCAO)         '+
                                    ' WHERE D.SALDO >0 '+
                                    ' AND D.ID_FORNCEDOR  = '+DM.cdsRemessaProducaoID_FOR_REMESSA.AsString+
                                    ' AND D.ID_CLIENTE = '+DM.cdsRemessaProducaoID_CLIENTE.AsString;
...
procedure TfrmRemessaProducao.DBEdit14KeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
  begin
    Key := #0;
...
procedure TfrmRemessaProducao.DBEdit16Exit(Sender: TObject);
begin
  if DM.cdsRemessaProducaoNR_NF_REMESSA.AsInteger = DM.cdsRemessaProducaoNR_NOTA_FISCAL.AsInteger then
  begin
    ShowMessage('Nº Nota Fiscal Remessa não pode ser igual ao Nº da Nota Fiscal' );
    DBEdit16.SetFocus;
    exit
  end;
...
procedure TfrmRemessaProducao.DBEdit17Exit(Sender: TObject);
begin
  if DM.cdsItemRemessaQUANT_FATURADO.AsFloat > sSaldo then
  begin
    ShowMessage('Saldo de Fio Para a Quantidade Informada Indisponivél.'+#13+' Saldo Disponivél  : '+QuotedStr(FormatFloat('###,###0.000',sSaldo)));
    if Application.MessageBox('Deseja Informar Nova Quantidade ?','Confirmação',MB_ICONQUESTION+MB_YESNO ) = idyes then
    begin
      DBEdit17.SetFocus;
      Exit;
    end
...
procedure TfrmRemessaProducao.DBEdit19Exit(Sender: TObject);
begin
  if DBEdit19.Text = '' Then
  begin
//    BB_CANC.SetFocus;
  ...
  begin
    DM.cdsRemessaProducaoID_FORN_TINT.AsInteger := DM.cdsFornecedorID_FORNECEDOR.AsInteger;
    DM.cdsRemessaProducaoTITURARIA.AsString     := DM.cdsFornecedorNOME.AsString;
    DBEdit16.SetFocus;
  end
  else
  begin
...
procedure TfrmRemessaProducao.DBEdit19KeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
  begin
    Key := #0;
...
procedure TfrmRemessaProducao.DBEdit20Exit(Sender: TObject);
begin
  if DBEdit20.Text = '' Then
  begin
//    BB_CANC.SetFocus;
  ...
  begin
    DM.cdsRemessaProducaoID_CLIENTE.AsInteger := DM.cdsClienteID_CLIENTE.AsInteger;
    DM.cdsRemessaProducaoCLIENTE.AsString     := DM.cdsClienteR_SOCIAL.AsString;
    DBEdit19.SetFocus;
  end
  else
  begin
...
procedure TfrmRemessaProducao.DBEdit20KeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
  begin
    Key := #0;
...
procedure TfrmRemessaProducao.DBEdit2Exit(Sender: TObject);
begin
...
//    BB_CANC.SetFocus;
  ...
  begin
    DM.cdsRemessaProducaoID_FORNECEDOR.AsInteger         := DM.cdsProducaoID_FORNCEDOR.AsInteger;
    DM.cdsRemessaProducaoFORNECEDOR.AsString             := DM.cdsProducaoFORNECEDOR.AsString;
    DM.cdsRemessaProducaoID_PRODUCAO.AsInteger           := DM.cdsProducaoID_PRODUCAO.AsInteger;
    DM.cdsRemessaProducaoNR_NOTA_FISCAL.AsInteger        := DM.cdsProducaoNR_NOTA_FISCAL.AsInteger;
    DM.cdsRemessaProducaoID_CLIENTE.AsInteger            := DM.cdsProducaoID_CLIENTE.AsInteger;
    DM.cdsRemessaProducaoCLIENTE.AsString                := DM.cdsProducaoCLIENTE.AsString;
    DM.cdsRemessaProducaoFORNECEDOR.AsString             := DM.cdsProducaoFORNECEDOR.AsString;
    DM.cdsRemessaProducaoDT_EMISSAO.AsDateTime           := DM.cdsProducaoDT_EMISSAO.AsDateTime;
    DM.cdsRemessaProducaoQUANT.AsFloat                   := DM.cdsProducaoQUANT.AsFloat;
    DM.cdsRemessaProducaoQUANT_NF.AsFloat                := DM.cdsProducaoQUANT.AsFloat;
    DM.cdsRemessaProducaoVL_NOTA.AsFloat                 := DM.cdsProducaoVL_NOTA.AsFloat;
    DM.cdsRemessaProducaoVL_POR_FORA.AsFloat             := DM.cdsProducaoVL_POR_FORA.AsFloat;
    DM.cdsRemessaProducaoVL_TOTAL.AsFloat                := DM.cdsProducaoVL_TOTAL.AsFloat;
    DBEDIT3.SetFocus;
  end
  else
  begin
...
procedure TfrmRemessaProducao.DBEdit2KeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
  begin
    Key := #0;
...
procedure TfrmRemessaProducao.DBEdit3Exit(Sender: TObject);
begin
  if DBEdit3.Text = '' Then
  begin
//    BB_CANC.SetFocus;
  ...
  begin
    DM.cdsRemessaProducaoID_FOR_REMESSA.AsInteger := DM.cdsFornecedorID_FORNECEDOR.AsInteger;
    DM.cdsRemessaProducaoNOME_MALHARIA.AsString   := DM.cdsFornecedorNOME.AsString;
    DBEdit4.SetFocus;
  end
  else
  begin
...
procedure TfrmRemessaProducao.DBEdit3KeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
  begin
    Key := #0;
...
procedure TfrmRemessaProducao.DBGrid3KeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
  begin
    Key := #0;
...
procedure TfrmRemessaProducao.DBNavigator1Click(Sender: TObject;
  Button: TNavigateBtn);
begin
    cdsItemNotaFiscal.Close;
//    cdsItemNotaFiscal.Params[0].AsInteger := DM.cdsRemessaProducaoID_PRODUCAO.AsInteger;
  ...
  cdsContasPagar.Params[3].AsInteger := StrToInt(DM.cdsRemessaProducaoNR_NOTA_FISCAL.AsString);
...
procedure TfrmRemessaProducao.DBNavigator1DblClick(Sender: TObject);
begin
  PageControl2Change(Self);
end;

procedure TfrmRemessaProducao.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  if DM.cdsRemessaProducao.State in [dsEdit,dsInsert] then
  begin
...
procedure TfrmRemessaProducao.FormKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
  begin
    Key := #0;
...
procedure TfrmRemessaProducao.FormShow(Sender: TObject);
begin
  DM.cdsRemessaProducao.Close;
  DM.cdsRemessaProducao.Open;

  DM.cdsItemRemessa.Close;
  DM.cdsItemRemessa.Params[0].AsInteger := DM.cdsRemessaProducaoID_REMESSA.AsInteger;
  DM.cdsItemRemessa.Open;

  if DM.cdsRemessaProducao.RecordCount >0 then
  begin
    cdsItemNotaFiscal.Close;
    cdsItemNotaFiscal.Params[0].AsInteger := DM.cdsRemessaProducaoID_REMESSA.AsInteger;
    cdsItemNotaFiscal.Open;
...
procedure TfrmRemessaProducao.GerenciaBotoes;
begin
  BB_NOVO.Enabled := not (DM.cdsRemessaProducao.State in dsEditModes);
  BB_GRAVAR.Enabled := NOT BB_NOVO.Enabled;
  BB_EXCLUIR.Enabled := BB_NOVO.Enabled and not (DM.cdsRemessaProducao.IsEmpty);
...
procedure TfrmRemessaProducao.GerenciaBotoes2;
begin
  BB_INCLUIR.Enabled       := not (DM.cdsItemRemessa.State in dsEditModes);
  BB_SALVAR.Enabled        := NOT BB_INCLUIR.Enabled;
  BB_EXC.Enabled           := BB_INCLUIR.Enabled and not (DM.cdsItemRemessa.IsEmpty);
...
procedure TfrmRemessaProducao.PageControl2Change(Sender: TObject);
begin
  cdsContasPagar.Close;
...
procedure TfrmRemessaProducao.Somapagamentos;
Var
 sSql : string;
begin
  DM.cdsDinamica.Close;
  ...
  DM.cdsDinamica.CommandText := ' SELECT CP.VL_COMPRA, SUM(CP.VL_PARCELA), '+
                                ...
                                ' WHERE CP.ID_FORNECEDOR = '+DM.cdsRemessaProducaoID_FOR_REMESSA.AsString+
                                ' AND CP.NR_NOTA_FISCAL = '+DM.cdsRemessaProducaoNR_NOTA_FISCAL.AsString+
                                ' AND CP.TP_MOV=''D'''+
...
procedure TfrmRemessaProducao.SomaTotal;
begin
  DM.cdsDinamica.Close;
  DM.cdsDinamica.IndexFieldNames :='';
  DM.cdsDinamica.CommandText :=' SELECT SUM(IR.VL_TOTAL),SUM(IR.QT_ROLOS),SUM(IR.VL_TOTAL_REAL) FROM TB_ITEM_REMESSA_PRODUCAO IR'+
                               ' WHERE IR.ID_REMESSA = '+DM.cdsRemessaProducaoID_REMESSA.AsString;
  DM.cdsDinamica.Open;


  DM.cdsRemessaProducao.Edit;
  DM.cdsRemessaProducaoVL_NOTA.AsFloat       := DM.cdsDinamica.Fields[0].AsFloat;
  DM.cdsRemessaProducaoTOTAL_ROLOS.AsInteger := DM.cdsDinamica.Fields[1].AsInteger;
  DM.cdsRemessaProducaoVL_POR_FORA.AsFloat   := DM.cdsDinamica.Fields[2].AsFloat;
  DM.cdsRemessaProducaoVL_TOTAL_NF.AsFloat   := (DM.cdsRemessaProducaoVL_POR_FORA.AsFloat + DM.cdsRemessaProducaoVL_NOTA.AsFloat);
  DM.cdsRemessaProducao.ApplyUpdates(0);


end;

procedure TfrmRemessaProducao.SpeedButton1Click(Sender: TObject);
begin
{  DM.cdsProducao.Close;
  DM.cdsProducao.Open;
  Pesquisa(DM.cdsProducao,['NR_NOTA_FISCAL','FORNECEDOR'],['Nº Nota Fiscal','Fornecedor'],[10,40],'','','','','Nota Fiscal','Nota Fiscal');
    DM.cdsRemessaProducaoID_FORNCEDOR.AsInteger          := DM.cdsProducaoID_FORNCEDOR.AsInteger;
    DM.cdsRemessaProducaoFORNECEDOR.AsString             := DM.cdsProducaoFORNECEDOR.AsString;
    DM.cdsRemessaProducaoID_PRODUCAO.AsInteger           := DM.cdsProducaoID_PRODUCAO.AsInteger;
    DM.cdsRemessaProducaoNR_NOTA_FISCAL.AsInteger        := DM.cdsProducaoNR_NOTA_FISCAL.AsInteger;
    DM.cdsRemessaProducaoID_CLIENTE.AsInteger            := DM.cdsProducaoID_CLIENTE.AsInteger;
...
procedure TfrmRemessaProducao.SpeedButton2Click(Sender: TObject);
begin
  DM.cdsFornecedor.Close;
  DM.cdsFornecedor.CommandText := ' SELECT  F.ID_FORNECEDOR, F.CODIGO_MUNIC, F.DT_CADASTRO, F.NOME, F.TP_LOGRADOURO, F.ENDERECO,'+
                                  ' F.COMPLEMENTO, F.NUMERO, F.BAIRRO, F.CEP, F.TELFONE, F.FAX, F.CELULAR, F.EMAIL, F.CNPJ, F.INS_EST,'+
...
procedure TfrmRemessaProducao.SpeedButton3Click(Sender: TObject);
begin
  Close;
end;

procedure TfrmRemessaProducao.SpeedButton4Click(Sender: TObject);
begin
  DM.cdsFornecedor.Close;
  DM.cdsFornecedor.CommandText := ' SELECT  F.ID_FORNECEDOR, F.CODIGO_MUNIC, F.DT_CADASTRO, F.NOME, F.TP_LOGRADOURO, F.ENDERECO,'+
                                  ' F.COMPLEMENTO, F.NUMERO, F.BAIRRO, F.CEP, F.TELFONE, F.FAX, F.CELULAR, F.EMAIL, F.CNPJ, F.INS_EST,'+
...
procedure TfrmRemessaProducao.SpeedButton5Click(Sender: TObject);
var
 gpEmpresa : string;
begin
 { DM.cdsDinamica.Close;
 ...
 { DM.cdsArtigo.Close;
  ...
  frmPesquisaPadrao := TfrmPesquisaPadrao.Create(self);
  frmPesquisaPadrao.TB_PESQUISA := 'NfMalharia';
...
procedure TfrmRemessaProducao.SpeedButton6Click(Sender: TObject);
begin
  DM.cdsCliente.Close;
  DM.cdsCliente.Open;
...
procedure TfrmRemessaProducao.ValidacoesFinalizar;
begin
  //-verificar se existem itens na nota

  if DM.cdsItemremessa.RecordCount = 0 then
  begin
    ShowMessage('Impossível Finalizar Nota. Não Exite(m) Iten(s) Lançado(s).');
    Resultdo := True;
    Exit;
  end
  else
  ...
  if cdsContasPagar.RecordCount = 0 then
  begin
    ShowMessage('Impossível Finalizar Nota. Não Exite(m) Pagamento(s) Lançado(s).');
    Resultdo := True;
    Exit;
  end
  else
  ...
  begin
    ShowMessage('Impossível Finalizar Nota. Não Exite(m) Pagamento(s) Lançado(s) Com o Valor Menor que o Total da Nota.');
    Resultdo := True;
    DM.cdsDinamica.Close;
    Exit;
  end
...
procedure TfrmRemessaProducao.VerficaSaldoFios;
Var
 sSql : string;
begin
  sSql := ' SELECT p.* FROM TB_PRODUCAO p '+
          ' WHERE P.ID_CLIENTE = '+DM.cdsRemessaProducaoID_CLIENTE.AsString+
          ' AND P.QUANT > 0 ';
  DM.cdsDinamica.Close;
  DM.cdsDinamica.IndexFieldNames:='';
  ...
  if DM.cdsDinamica.IsEmpty then
  begin
    ShowMessage('Não Exite(m) Saldo(s) de Fio(s) Lançado(s) Para o Cliente Selecionado.');
    if Application.MessageBox('Deseja Continuar?','Confirmação',MB_YESNO+MB_ICONQUESTION) = idyes then
    begin
      BB_GRAVARClick(Self);
    end
    else
