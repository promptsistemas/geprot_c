...
begin
  if Application.MessageBox('Deseja Realmente Efetuar Baixa?','Confirmação.',MB_YESNO+MB_ICONQUESTION) =idyes then
  begin
    ...
    begin
      ShowMessage('Favor Informar um Valor Válido.');
      DBEdit12.SetFocus;
      Exit;
    end;
    ...
    begin
      if Application.MessageBox('Confirma Baixa Parcial deste Título ?','Confirmação.',MB_YESNO+MB_ICONQUESTION) = idyes then
      begin
        frmContasPagarLista.cdsContasPagar.Edit;
        ...
        cdsContasPagarOBS.AsString                  := ' Título Referente Baixa Parcial Doc '+frmContasPagarLista.cdsContasPagarNR_DOC.AsString;
