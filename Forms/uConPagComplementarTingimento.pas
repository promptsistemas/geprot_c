unit uConPagComplementarTingimento;
...
type
  TfrmConPagComplementarTingimento = class(TForm)
    Panel1: TPanel;
    Label5: TLabel;
    Panel2: TPanel;
    Shape1: TShape;
...
var
  frmConPagComplementarTingimento: TfrmConPagComplementarTingimento;
  DiasDoMes : TDate;
  Tipo: string;
  Count : Integer;
...
uses U_LIB, UD_PESQUISA, uDM, uMenu, uProducao, uContasPagar, uCadMesa,
  uTingimento, uConPagComplementarMalharia;

procedure TfrmConPagComplementarTingimento.BB_ALTERARClick(Sender: TObject);
begin
  Tipo := 'Edit';
...
procedure TfrmConPagComplementarTingimento.BB_CANCELARClick(Sender: TObject);
begin
  if Tipo = 'Insert' then
  begin
...
procedure TfrmConPagComplementarTingimento.BB_DELClick(Sender: TObject);
begin
...
procedure TfrmConPagComplementarTingimento.BB_EXCLUIRClick(Sender: TObject);
begin
...
procedure TfrmConPagComplementarTingimento.BB_INCLUIRClick(Sender: TObject);
begin
  Tipo := 'Insert';
  ...
  begin
    ...
    cdsComplementarVL_COMPRA.AsFloat            := DM.cdsTingimentoVL_POR_FORA.AsFloat;
    cdsComplementarSTATUS.AsString              := 'A';
    cdsComplementarNR_NOTA_FISCAL.AsString      := DM.cdsTingimentoNR_NOTA_FISCAL.AsString;
    cdsComplementarNR_NF_REMESSA.AsString       := DM.cdsTingimentoNR_NOTA_FISCAL.AsString;
    cdsComplementarORIGEM.AsString              := 'TINGIMENTO';
    ...
    if cdscomplementar.RecordCount = 0 then
      cdsComplementarNR_DOC.Value                 := DM.cdsTingimentoNR_NOTA_FISCAL.AsString +'/'+IntToStr(1)+'-'+cdsContasPagarQT_PARCELA.AsString+'/R'
    else
      cdsComplementarNR_DOC.Value                 := DM.cdsTingimentoNR_NOTA_FISCAL.AsString +'/'+IntToStr(Parcelas+1)+'-'+cdsContasPagarQT_PARCELA.AsString+'/R';

    GerenciaBotoes;
    DBEdit1.SetFocus;
end;

procedure TfrmConPagComplementarTingimento.BB_SAIRClick(Sender: TObject);
begin
  Close;
  frmTingimento.PageControl2Change(Self);
end;

procedure TfrmConPagComplementarTingimento.BitBtn2Click(Sender: TObject);
Var
...
begin
  ...
  begin
    ...
    SaldoPagamento:= DM.cdsTingimentoVL_POR_FORA.AsFloat-(DM.cdsDinamica.Fields[0].AsFloat+vlParcelaEdit);
    ...
    if iCont = CountReg then
      ...
      cdsComplementarVL_PARCELA.AsFloat := DM.cdsTingimentoVL_POR_FORA.AsFloat/cdsContasPagarQT_PARCELA.AsInteger;  //SaldoPagamento;
    cdsComplementar.ApplyUpdates(0);
    cdsContasPagar.Next;
  end;
...
procedure TfrmConPagComplementarTingimento.DBEdit12Enter(Sender: TObject);
begin
...
procedure TfrmConPagComplementarTingimento.DBEdit12Exit(Sender: TObject);
begin
...
procedure TfrmConPagComplementarTingimento.DBEdit1Exit(Sender: TObject);
begin
...
{    if cdsContasPagar.RecordCount > 0 then
    ...
    if not DM.cdsPlanoPagamentoQT_PARCELAS.IsNull then
        ...
        (DM.cdsTingimentoVL_POR_FORA.AsFloat / cdsContasPagarQT_PARCELA.AsInteger);
...
procedure TfrmConPagComplementarTingimento.DBEdit2Exit(Sender: TObject);
begin
...
procedure TfrmConPagComplementarTingimento.DBEdit5Enter(Sender: TObject);
Var
 vlParcelaEdit : Double;
begin
  if Tipo = 'Insert' then
...
//      cdsComplementarVL_PARCELA.AsFloat := SaldoPagamento
//    else
      cdsComplementarVL_PARCELA.AsFloat := DM.cdsTingimentoVL_POR_FORA.AsFloat/cdsContasPagarQT_PARCELA.AsInteger;
  end;
  if Tipo = 'Edit' then
  begin
    vlParcelaEdit := cdsComplementarVL_PARCELA.AsFloat;
...
procedure TfrmConPagComplementarTingimento.DBEdit5Exit(Sender: TObject);
begin
  ...
  begin
    ...
    begin
      ...
      cdsComplementarID_CONTAS_PAGAR.AsInteger     := DM.cdsDinamica.Fields[0].AsInteger+1;
...
procedure TfrmConPagComplementarTingimento.DBGrid2DblClick(Sender: TObject);
begin
  p_item.Enabled := True;
...
procedure TfrmConPagComplementarTingimento.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action := caFree;
  frmConPagComplementarTingimento := nil;
  iCont := 0;
end;

procedure TfrmConPagComplementarTingimento.FormKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
  begin
    Key := #0;
...
procedure TfrmConPagComplementarTingimento.FormShow(Sender: TObject);
begin
  ...
  cdsComplementar.Params[0].AsInteger := StrToInt(DM.cdsTingimentoID_CLIENTE.AsString);
  cdsComplementar.Params[1].AsDate    := StrToDate(DM.cdsTingimentoDT_EMISSAO.AsString);
  cdsComplementar.Params[2].AsInteger := StrToInt(DM.cdsTingimentoID_TINGIMENTO.AsString);
  cdsComplementar.Open;

  if DM.cdsTingimento.RecordCount >0 then
  begin
    DM.cdsItemRemessa.Close;
    DM.cdsItemRemessa.Params[0].AsInteger := DM.cdsTingimentoID_TINGIMENTO.AsInteger;
...
procedure TfrmConPagComplementarTingimento.GerenciaBotoes;
begin
...
procedure TfrmConPagComplementarTingimento.SomaPagamentos;
Var
...
begin
 // l := cdsComplementarVL_PARCELA.AsFloat;
  ...
  TotalFora := DM.cdsTingimentoVL_POR_FORA.AsFloat;
...
procedure TfrmConPagComplementarTingimento.SomaTotal;
begin
  DM.cdsDinamica.Close;
  DM.cdsDinamica.IndexFieldNames :='';
  DM.cdsDinamica.CommandText :=' SELECT SUM(IP.VL_TOTAL_REAL),SUM(IP.VL_TOTAL),SUM(IP.VL_TOTAL_REAL)+SUM(IP.VL_TOTAL) '+
                               ' FROM TB_ITEM_REMESSA_PRODUCAO IP'+
                               ' WHERE IP.ID_TINGIMENTO = '+DM.cdsTingimentoID_TINGIMENTO.AsString;
  DM.cdsDinamica.Open;


  DM.cdsTingimento.Edit;
  DM.cdsTingimentoVL_POR_FORA.AsFloat  := DM.cdsDinamica.Fields[0].AsFloat;
  DM.cdsTingimentoVL_NOTA.AsFloat      := DM.cdsDinamica.Fields[1].AsFloat;
  DM.cdsTingimentoVL_TOTAL_NF.AsFloat  := DM.cdsDinamica.Fields[2].AsFloat;;
  DM.cdsTingimento.ApplyUpdates(0);

end;

function TfrmConPagComplementarTingimento.UltimoDia: integer;
var Date: TDateTime;
begin
...
procedure TfrmConPagComplementarTingimento.VerificaTotais;
begin
  DM.cdsDinamica.Close;
  DM.cdsDinamica.IndexFieldNames:='';
  DM.cdsDinamica.CommandText := ' SELECT CP.VL_COMPRA, SUM(CP.VL_PARCELA), '+
                                ...
                                ' GROUP BY CP.VL_COMPRA ';
  DM.cdsDinamica.Open;
  if DM.cdsDinamica.Fields[1].AsFloat <> DM.cdsTingimentoVL_POR_FORA.AsFloat then
  begin
    Totais := False;
