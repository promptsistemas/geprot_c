object frmReativaProducao: TfrmReativaProducao
  Left = 0
  Top = 0
  BorderStyle = bsNone
  Caption = 'frmReativaProducao'
  ClientHeight = 484
  ClientWidth = 874
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  OldCreateOrder = False
  Position = poScreenCenter
  OnClose = FormClose
  OnKeyPress = FormKeyPress
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object SB_REATIVAR: TSpeedButton
    Left = 4
    Top = 443
    Width = 149
    Height = 40
    Caption = 'Reativar Nota Fiscal'
    Enabled = False
    Glyph.Data = {
      
      FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
    NumGlyphs = 2
    OnClick = SB_REATIVARClick
  end
  object Panel1: TPanel
    Left = 0
    Top = 0
    Width = 874
    Height = 41
    Align = alTop
    Color = clBlue
    Font.Charset = ANSI_CHARSET
    Font.Color = clBlue
    Font.Height = -21
    Font.Name = 'Segoe UI Semibold'
    Font.Style = [fsBold]
    ParentBackground = False
    ParentFont = False
    TabOrder = 0
    object Label17: TLabel
      Left = 8
      Top = 4
      Width = 483
      Height = 37
      Caption = 'REATIVAR ENTRADA DE NOTA FISCAL'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWhite
      Font.Height = -27
      Font.Name = 'Segoe UI'
      Font.Style = [fsBold, fsItalic]
      ParentFont = False
    end
    object SpeedButton3: TSpeedButton
      Left = 835
      Top = 1
      Width = 38
      Height = 39
      Align = alRight
      Flat = True
      Glyph.Data = {
        
        000000000000}
      OnClick = SpeedButton3Click
      ExplicitLeft = 1102
      ExplicitTop = 5
      ExplicitHeight = 36
    end
  end
  object Panel2: TPanel
    Left = 0
    Top = 41
    Width = 874
    Height = 152
    Align = alTop
    Color = clWhite
    ParentBackground = False
    TabOrder = 1
    object Label1: TLabel
      Left = 4
      Top = 1
      Width = 67
      Height = 13
      Caption = 'N'#186' Nota Fiscal'
    end
    object Label2: TLabel
      Left = 4
      Top = 41
      Width = 33
      Height = 13
      Caption = 'C'#243'digo'
      FocusControl = DBEdit1
    end
    object Label4: TLabel
      Left = 4
      Top = 81
      Width = 64
      Height = 13
      Caption = 'Data Emiss'#227'o'
      FocusControl = DBEdit4
    end
    object Label5: TLabel
      Left = 78
      Top = 81
      Width = 67
      Height = 13
      Caption = 'N'#186' Nota Fiscal'
      FocusControl = DBEdit5
    end
    object Label3: TLabel
      Left = 78
      Top = 41
      Width = 58
      Height = 13
      Caption = 'C'#243'digo Forn'
      FocusControl = DBEdit2
    end
    object Label10: TLabel
      Left = 145
      Top = 41
      Width = 85
      Height = 13
      Caption = 'Nome Fornecedor'
      FocusControl = DBEdit10
    end
    object Label6: TLabel
      Left = 147
      Top = 81
      Width = 46
      Height = 13
      Caption = 'Quant NF'
      FocusControl = DBEdit6
    end
    object Label7: TLabel
      Left = 222
      Top = 81
      Width = 67
      Height = 13
      Caption = 'Vl. Nota Fiscal'
      FocusControl = DBEdit7
    end
    object Label9: TLabel
      Left = 295
      Top = 81
      Width = 39
      Height = 13
      Caption = 'Vl. Total'
    end
    object Label8: TLabel
      Left = 375
      Top = 81
      Width = 63
      Height = 13
      Caption = 'C'#243'd.Malharia'
      FocusControl = DBEdit11
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlue
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
    end
    object Label20: TLabel
      Left = 444
      Top = 81
      Width = 40
      Height = 13
      Caption = 'Malharia'
      FocusControl = DBEdit11
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlue
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
    end
    object Label11: TLabel
      Left = 444
      Top = 41
      Width = 69
      Height = 13
      Caption = 'C'#243'digo Cliente'
      FocusControl = DBEdit3
    end
    object Label13: TLabel
      Left = 518
      Top = 41
      Width = 63
      Height = 13
      Caption = 'Nome Cliente'
      FocusControl = DBEdit11
    end
    object Label21: TLabel
      Left = 720
      Top = 81
      Width = 74
      Height = 13
      Caption = 'N'#186' NF Remessa'
      FocusControl = DBEdit5
    end
    object SpeedButton1: TSpeedButton
      Left = 320
      Top = 16
      Width = 23
      Height = 22
      Visible = False
      OnClick = SpeedButton1Click
    end
    object edtPesquisa: TEdit
      Left = 4
      Top = 15
      Width = 93
      Height = 19
      Ctl3D = False
      ParentCtl3D = False
      TabOrder = 0
      OnExit = edtPesquisaExit
      OnKeyPress = edtPesquisaKeyPress
    end
    object DBEdit1: TDBEdit
      Left = 4
      Top = 57
      Width = 72
      Height = 19
      CharCase = ecUpperCase
      Ctl3D = False
      DataField = 'ID_PRODUCAO'
      DataSource = dsReativaProducao
      ParentCtl3D = False
      ReadOnly = True
      TabOrder = 2
    end
    object DBEdit4: TDBEdit
      Left = 4
      Top = 97
      Width = 72
      Height = 19
      CharCase = ecUpperCase
      Ctl3D = False
      DataField = 'DT_EMISSAO'
      DataSource = dsReativaProducao
      ParentCtl3D = False
      ReadOnly = True
      TabOrder = 3
    end
    object DBEdit5: TDBEdit
      Left = 78
      Top = 97
      Width = 68
      Height = 19
      CharCase = ecUpperCase
      Ctl3D = False
      DataField = 'NR_NOTA_FISCAL'
      DataSource = dsReativaProducao
      ParentCtl3D = False
      ReadOnly = True
      TabOrder = 4
    end
    object DBEdit2: TDBEdit
      Left = 78
      Top = 57
      Width = 65
      Height = 19
      CharCase = ecUpperCase
      Ctl3D = False
      DataField = 'ID_FORNCEDOR'
      DataSource = dsReativaProducao
      ParentCtl3D = False
      ReadOnly = True
      TabOrder = 5
    end
    object DBEdit10: TDBEdit
      Left = 144
      Top = 57
      Width = 298
      Height = 19
      CharCase = ecUpperCase
      Ctl3D = False
      DataField = 'FORNECEDOR'
      DataSource = dsReativaProducao
      ParentCtl3D = False
      ReadOnly = True
      TabOrder = 6
    end
    object DBEdit6: TDBEdit
      Left = 147
      Top = 97
      Width = 73
      Height = 19
      CharCase = ecUpperCase
      Ctl3D = False
      DataField = 'QUANT'
      DataSource = dsReativaProducao
      ParentCtl3D = False
      ReadOnly = True
      TabOrder = 7
    end
    object DBEdit7: TDBEdit
      Left = 222
      Top = 97
      Width = 72
      Height = 19
      CharCase = ecUpperCase
      Color = clInfoBk
      Ctl3D = False
      DataField = 'VL_NOTA'
      DataSource = dsReativaProducao
      ParentCtl3D = False
      ReadOnly = True
      TabOrder = 8
    end
    object DBEdit9: TDBEdit
      Left = 295
      Top = 97
      Width = 78
      Height = 19
      CharCase = ecUpperCase
      Color = 16769476
      Ctl3D = False
      DataField = 'VL_TOTAL'
      DataSource = dsReativaProducao
      Font.Charset = ANSI_CHARSET
      Font.Color = clBlue
      Font.Height = -11
      Font.Name = 'Segoe UI Semibold'
      Font.Style = [fsBold]
      ParentCtl3D = False
      ParentFont = False
      ReadOnly = True
      TabOrder = 9
    end
    object DBEdit8: TDBEdit
      Left = 375
      Top = 97
      Width = 68
      Height = 19
      CharCase = ecUpperCase
      Ctl3D = False
      DataField = 'ID_FORN_REM'
      DataSource = dsReativaProducao
      ParentCtl3D = False
      ReadOnly = True
      TabOrder = 10
    end
    object DBEdit18: TDBEdit
      Left = 444
      Top = 97
      Width = 274
      Height = 19
      CharCase = ecUpperCase
      Ctl3D = False
      DataField = 'MALHARIA'
      DataSource = dsReativaProducao
      ParentCtl3D = False
      ReadOnly = True
      TabOrder = 11
    end
    object DBEdit3: TDBEdit
      Left = 444
      Top = 57
      Width = 73
      Height = 19
      CharCase = ecUpperCase
      Ctl3D = False
      DataField = 'ID_CLIENTE'
      DataSource = dsReativaProducao
      ParentCtl3D = False
      ReadOnly = True
      TabOrder = 12
    end
    object DBEdit11: TDBEdit
      Left = 518
      Top = 57
      Width = 283
      Height = 19
      CharCase = ecUpperCase
      Ctl3D = False
      DataField = 'CLIENTE'
      DataSource = dsReativaProducao
      ParentCtl3D = False
      ReadOnly = True
      TabOrder = 13
    end
    object DBEdit19: TDBEdit
      Left = 720
      Top = 97
      Width = 81
      Height = 19
      CharCase = ecUpperCase
      Ctl3D = False
      DataField = 'NR_NF_REMESSA'
      DataSource = dsReativaProducao
      ParentCtl3D = False
      ReadOnly = True
      TabOrder = 14
    end
    object BB_PESQUISAR: TBitBtn
      Left = 98
      Top = 14
      Width = 81
      Height = 23
      Caption = '&Pesquisar'
      Glyph.Data = {
        
        DB93A7B8B7AE9999DBDBDBDBDBDBDBDBDBDBDB9D9E9ADBDBDBDB}
      TabOrder = 1
      OnClick = BB_PESQUISARClick
    end
  end
  object Panel3: TPanel
    Left = 0
    Top = 193
    Width = 874
    Height = 41
    Align = alTop
    Color = clBlue
    Font.Charset = ANSI_CHARSET
    Font.Color = clBlue
    Font.Height = -21
    Font.Name = 'Segoe UI Semibold'
    Font.Style = [fsBold]
    ParentBackground = False
    ParentFont = False
    TabOrder = 2
    object Label12: TLabel
      Left = 8
      Top = 4
      Width = 301
      Height = 37
      Caption = 'ITENS DA NOTA FISCAL'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWhite
      Font.Height = -27
      Font.Name = 'Segoe UI'
      Font.Style = [fsBold, fsItalic]
      ParentFont = False
    end
  end
  object PageControl2: TPageControl
    Left = 0
    Top = 234
    Width = 874
    Height = 209
    ActivePage = TabSheet3
    Align = alTop
    TabOrder = 3
    object TabSheet3: TTabSheet
      Caption = 'Itens'
      object DBGrid1: TDBGrid
        Left = 0
        Top = 0
        Width = 866
        Height = 181
        Align = alClient
        Color = clBtnFace
        Ctl3D = True
        DataSource = dsItemProducao
        FixedColor = cl3DLight
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = []
        Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit, dgTitleClick, dgTitleHotTrack]
        ParentCtl3D = False
        ParentFont = False
        ParentShowHint = False
        ShowHint = False
        TabOrder = 0
        TitleFont.Charset = DEFAULT_CHARSET
        TitleFont.Color = clBlue
        TitleFont.Height = -11
        TitleFont.Name = 'Tahoma'
        TitleFont.Style = []
        Columns = <
          item
            Expanded = False
            FieldName = 'ID_MATERIA_PRIMA'
            Width = 100
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'NOME_MATERIA'
            Width = 276
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'QUANT_FATURADO'
            Width = 81
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'VL_UNIT'
            Width = 69
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'VL_TOTAL'
            Width = 103
            Visible = True
          end>
      end
    end
  end
  object dsReativaProducao: TDataSource
    DataSet = cdsReativaProducao
    Left = 712
    Top = 78
  end
  object cdsReativaProducao: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'dspReativaProducao'
    Left = 680
    Top = 78
    object cdsReativaProducaoID_PRODUCAO: TIntegerField
      FieldName = 'ID_PRODUCAO'
      Required = True
    end
    object cdsReativaProducaoID_FORNCEDOR: TIntegerField
      FieldName = 'ID_FORNCEDOR'
      Required = True
    end
    object cdsReativaProducaoID_CLIENTE: TIntegerField
      FieldName = 'ID_CLIENTE'
      Required = True
    end
    object cdsReativaProducaoDT_EMISSAO: TDateField
      FieldName = 'DT_EMISSAO'
    end
    object cdsReativaProducaoNR_NOTA_FISCAL: TIntegerField
      DisplayLabel = 'N'#186' Nota Fiscal'
      FieldName = 'NR_NOTA_FISCAL'
    end
    object cdsReativaProducaoQUANT: TFMTBCDField
      FieldName = 'QUANT'
      Precision = 18
      Size = 4
    end
    object cdsReativaProducaoSTATUS: TStringField
      FieldName = 'STATUS'
      Size = 1
    end
    object cdsReativaProducaoVL_NOTA: TFMTBCDField
      FieldName = 'VL_NOTA'
      Precision = 18
      Size = 3
    end
    object cdsReativaProducaoVL_POR_FORA: TFMTBCDField
      FieldName = 'VL_POR_FORA'
      Precision = 18
      Size = 3
    end
    object cdsReativaProducaoVL_TOTAL: TFMTBCDField
      FieldName = 'VL_TOTAL'
      Precision = 18
      Size = 3
    end
    object cdsReativaProducaoID_FORN_REM: TIntegerField
      FieldName = 'ID_FORN_REM'
      Required = True
    end
    object cdsReativaProducaoNR_NF_REMESSA: TIntegerField
      FieldName = 'NR_NF_REMESSA'
    end
    object cdsReativaProducaoVL_TOTAL_NF: TFMTBCDField
      FieldName = 'VL_TOTAL_NF'
      Precision = 18
      Size = 3
    end
    object cdsReativaProducaoVL_ACUMULADO: TFMTBCDField
      FieldName = 'VL_ACUMULADO'
      Precision = 18
      Size = 3
    end
    object cdsReativaProducaoFORNECEDOR: TStringField
      DisplayLabel = 'Fornecedor'
      FieldName = 'FORNECEDOR'
      ProviderFlags = []
      Size = 50
    end
    object cdsReativaProducaoCLIENTE: TStringField
      DisplayLabel = 'Cliente'
      FieldName = 'CLIENTE'
      ProviderFlags = []
      Size = 50
    end
    object cdsReativaProducaoMALHARIA: TStringField
      DisplayLabel = 'Malharia'
      FieldName = 'MALHARIA'
      ProviderFlags = []
      Size = 50
    end
  end
  object dspReativaProducao: TDataSetProvider
    DataSet = qReativaProducao
    Options = [poIncFieldProps, poAllowMultiRecordUpdates, poAutoRefresh, poAllowCommandText, poUseQuoteChar]
    Left = 648
    Top = 78
  end
  object qReativaProducao: TSQLQuery
    MaxBlobSize = -1
    Params = <>
    SQL.Strings = (
      'SELECT'
      ' P.ID_PRODUCAO,'
      ' P.ID_FORNCEDOR,'
      ' P.ID_CLIENTE,'
      ' P.DT_EMISSAO,'
      ' P.NR_NOTA_FISCAL,'
      ' P.QUANT,'
      ' P.STATUS,'
      ' P.VL_NOTA,'
      ' P.VL_POR_FORA,'
      ' P.VL_TOTAL, '
      ' P.ID_FORN_REM,'
      ' P.NR_NF_REMESSA,'
      ' P.VL_TOTAL_NF,'
      ' P.VL_ACUMULADO, '
      ' F.NOME AS FORNECEDOR,'
      ' C.R_SOCIAL AS CLIENTE,'
      ' FF.NOME AS MALHARIA'
      'FROM TB_PRODUCAO P'
      'JOIN TB_FORNECEDOR F ON (F.ID_FORNECEDOR = P.ID_FORNCEDOR)'
      'JOIN TB_FORNECEDOR FF ON (FF.ID_FORNECEDOR = P.ID_FORN_REM)'
      'JOIN TB_CLIENTE C ON (C.ID_CLIENTE = P.ID_CLIENTE)'
      'WHERE P.status='#39'F'#39)
    SQLConnection = DM.conexao
    Left = 616
    Top = 78
    object qReativaProducaoID_PRODUCAO: TIntegerField
      FieldName = 'ID_PRODUCAO'
      Required = True
    end
    object qReativaProducaoID_FORNCEDOR: TIntegerField
      FieldName = 'ID_FORNCEDOR'
      Required = True
    end
    object qReativaProducaoID_CLIENTE: TIntegerField
      FieldName = 'ID_CLIENTE'
      Required = True
    end
    object qReativaProducaoDT_EMISSAO: TDateField
      FieldName = 'DT_EMISSAO'
    end
    object qReativaProducaoNR_NOTA_FISCAL: TIntegerField
      FieldName = 'NR_NOTA_FISCAL'
    end
    object qReativaProducaoQUANT: TFMTBCDField
      FieldName = 'QUANT'
      Precision = 18
      Size = 4
    end
    object qReativaProducaoSTATUS: TStringField
      FieldName = 'STATUS'
      Size = 1
    end
    object qReativaProducaoVL_NOTA: TFMTBCDField
      FieldName = 'VL_NOTA'
      Precision = 18
      Size = 3
    end
    object qReativaProducaoVL_POR_FORA: TFMTBCDField
      FieldName = 'VL_POR_FORA'
      Precision = 18
      Size = 3
    end
    object qReativaProducaoVL_TOTAL: TFMTBCDField
      FieldName = 'VL_TOTAL'
      Precision = 18
      Size = 3
    end
    object qReativaProducaoID_FORN_REM: TIntegerField
      FieldName = 'ID_FORN_REM'
      Required = True
    end
    object qReativaProducaoNR_NF_REMESSA: TIntegerField
      FieldName = 'NR_NF_REMESSA'
    end
    object qReativaProducaoVL_TOTAL_NF: TFMTBCDField
      FieldName = 'VL_TOTAL_NF'
      Precision = 18
      Size = 3
    end
    object qReativaProducaoVL_ACUMULADO: TFMTBCDField
      FieldName = 'VL_ACUMULADO'
      Precision = 18
      Size = 3
    end
    object qReativaProducaoFORNECEDOR: TStringField
      FieldName = 'FORNECEDOR'
      ProviderFlags = []
      Size = 50
    end
    object qReativaProducaoCLIENTE: TStringField
      FieldName = 'CLIENTE'
      ProviderFlags = []
      Size = 50
    end
    object qReativaProducaoMALHARIA: TStringField
      FieldName = 'MALHARIA'
      ProviderFlags = []
      Size = 50
    end
  end
  object dsItemProducao: TDataSource
    DataSet = cdsItemProducao
    Left = 712
    Top = 142
  end
  object cdsItemProducao: TClientDataSet
    Aggregates = <>
    IndexFieldNames = 'ID_PRODUCAO'
    MasterFields = 'ID_PRODUCAO'
    MasterSource = dsReativaProducao
    PacketRecords = 0
    Params = <
      item
        DataType = ftInteger
        Name = 'ID_PRODUCAO'
        ParamType = ptInput
      end>
    ProviderName = 'dspItemProducao'
    Left = 680
    Top = 142
    object cdsItemProducaoID_ITEM_PRODUCAO: TIntegerField
      FieldName = 'ID_ITEM_PRODUCAO'
      Required = True
    end
    object cdsItemProducaoID_PRODUCAO: TIntegerField
      FieldName = 'ID_PRODUCAO'
    end
    object cdsItemProducaoID_MATERIA_PRIMA: TIntegerField
      DisplayLabel = 'C'#243'd.Mat-Prima'
      FieldName = 'ID_MATERIA_PRIMA'
    end
    object cdsItemProducaoQUANT_FATURADO: TFMTBCDField
      DisplayLabel = 'Quant'
      FieldName = 'QUANT_FATURADO'
      DisplayFormat = '###,###0.000'
      EditFormat = '###,###0.000'
      Precision = 18
      Size = 4
    end
    object cdsItemProducaoVL_UNIT: TFMTBCDField
      DisplayLabel = 'Vl. Unit'
      FieldName = 'VL_UNIT'
      DisplayFormat = '###,###0.00'
      EditFormat = '###,###0.00'
      Precision = 18
      Size = 2
    end
    object cdsItemProducaoVL_TOTAL: TFMTBCDField
      DisplayLabel = 'Vl. Total'
      FieldName = 'VL_TOTAL'
      DisplayFormat = '###,###0.00'
      EditFormat = '###,###0.00'
      Precision = 18
      Size = 2
    end
    object cdsItemProducaoVL_UNIT_REAL: TFMTBCDField
      FieldName = 'VL_UNIT_REAL'
      Precision = 18
      Size = 2
    end
    object cdsItemProducaoVL_TOTAL_REAL: TFMTBCDField
      FieldName = 'VL_TOTAL_REAL'
      Precision = 18
      Size = 2
    end
    object cdsItemProducaoNOME_MATERIA: TStringField
      DisplayLabel = 'Mat'#233'ria-Prima'
      FieldName = 'NOME_MATERIA'
      ProviderFlags = []
      Size = 30
    end
  end
  object dspItemProducao: TDataSetProvider
    DataSet = qItemProducao
    Options = [poIncFieldProps, poAllowMultiRecordUpdates, poAutoRefresh, poAllowCommandText, poUseQuoteChar]
    Left = 648
    Top = 142
  end
  object qItemProducao: TSQLQuery
    MaxBlobSize = -1
    Params = <
      item
        DataType = ftInteger
        Name = 'ID_PRODUCAO'
        ParamType = ptInput
      end>
    SQL.Strings = (
      'SELECT'
      ' IP.ID_ITEM_PRODUCAO,'
      ' IP.ID_PRODUCAO,'
      ' IP.ID_MATERIA_PRIMA,'
      ' IP.QUANT_FATURADO,'
      ' IP.VL_UNIT,'
      ' IP.VL_TOTAL,'
      ' IP.VL_UNIT_REAL,'
      ' IP.VL_TOTAL_REAL,'
      ' MP.NOME_MATERIA'
      'FROM TB_ITEM_PRODUCAO IP'
      
        'JOIN TB_MATERIA_PRIMA MP ON (MP.ID_MATERIA = IP.ID_MATERIA_PRIMA' +
        ')'
      'WHERE IP.ID_PRODUCAO = :ID_PRODUCAO')
    SQLConnection = DM.conexao
    Left = 616
    Top = 142
    object qItemProducaoID_ITEM_PRODUCAO: TIntegerField
      FieldName = 'ID_ITEM_PRODUCAO'
      Required = True
    end
    object qItemProducaoID_PRODUCAO: TIntegerField
      FieldName = 'ID_PRODUCAO'
    end
    object qItemProducaoID_MATERIA_PRIMA: TIntegerField
      FieldName = 'ID_MATERIA_PRIMA'
    end
    object qItemProducaoQUANT_FATURADO: TFMTBCDField
      FieldName = 'QUANT_FATURADO'
      Precision = 18
      Size = 4
    end
    object qItemProducaoVL_UNIT: TFMTBCDField
      FieldName = 'VL_UNIT'
      Precision = 18
      Size = 2
    end
    object qItemProducaoVL_TOTAL: TFMTBCDField
      FieldName = 'VL_TOTAL'
      Precision = 18
      Size = 2
    end
    object qItemProducaoVL_UNIT_REAL: TFMTBCDField
      FieldName = 'VL_UNIT_REAL'
      Precision = 18
      Size = 2
    end
    object qItemProducaoVL_TOTAL_REAL: TFMTBCDField
      FieldName = 'VL_TOTAL_REAL'
      Precision = 18
      Size = 2
    end
    object qItemProducaoNOME_MATERIA: TStringField
      FieldName = 'NOME_MATERIA'
      ProviderFlags = []
      Size = 30
    end
  end
end
