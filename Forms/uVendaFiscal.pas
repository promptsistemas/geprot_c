...
begin
  ...
  begin
    try
      ...
      Application.MessageBox( 'Não foi possivel conectar com o banco de dados Dincash.' + #13 +
                              '   Tente Novamente ou Entre em contato com suporte .',
                              'Error de Conexão',mb_ok + mb_IconError );
        Abort;
    end;
  end;
end;
...
begin
  If cdsVendaDTOCANCELADA.AsString = 'S'  then // condição
  begin
...
begin
  ...
  begin
    ShowMessage('Sem Comunicação com o Banco de Dados Dincash. Verifique as Conexões');
    Close;
  end
  else
  begin
...
procedure TfrmVendaFiscal.DBGrid4DrawColumnCell(Sender: TObject;
  ...
  State: TGridDrawState);
begin
  If cdsTBvendaDTOCANCELADA.AsString = 'S'  then // condição
  begin
