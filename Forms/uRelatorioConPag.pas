...
begin
  sSql :=' SELECT CP.ID_CONTAS_PAGAR,CP.ID_PRODUCAO,CP.ID_CLIENTE,CP.ID_PLANO_PAGAMENTO ,'+
         ...
         ' C.ID_GRUPO_EMPRESA,C.R_SOCIAL AS CLIENTE , F.NOME AS FORNECEDOR ,FR.NOME AS FORN_REMESSA,U.NOME_USUARIO,L.R_SOCIAL AS EMPRESA,'+
  ...
  begin
    ...
    begin
      ...
      cdsContasPagar.CommandText :=sSql+'WHERE CP.TP_MOV <>'+QuotedStr(sMov)+'AND CP.STATUS <>'+QuotedStr(sStatus)+'AND C.ID_GRUPO_EMPRESA = '+QuotedStr(edtCli.Text)
  ...
  begin
    ...
    begin
      cdsContasPagar.Close;
      cdsContasPagar.CommandText :=sSql+'WHERE CP.TP_MOV <>'+QuotedStr(sMov)+'AND CP.STATUS <>'+QuotedStr(sStatus)+'AND CP.ID_FORNECEDOR ='+cdsFornecedorID_FORNECEDOR.AsString+'AND C.ID_GRUPO_EMPRESA = '+QuotedStr(edtCli.Text)
      +' ORDER BY CP.DT_VENCIMENTO';
      cdsContasPagar.Open;
      if cdsContasPagar.IsEmpty then
      begin
  ...
  begin
    ...
    begin
      ...
      cdsContasPagar.CommandText :=sSql+'WHERE CP.TP_MOV <>'+QuotedStr(sMov)+'AND CP.STATUS <>'+QuotedStr(sStatus)+'AND CP.ID_FORNECEDOR ='+cdsFornecedorID_FORNECEDOR.AsString
                                     +' AND C.ID_GRUPO_EMPRESA = '+QuotedStr(edtCli.Text)
...
begin
  if CBMOV.Text = 'NF' then
  begin
    sMov := 'D';
//    ShowMessage(sMov);
  end;

  if CBMOV.Text = 'Promo' then
  begin
    sMov := 'F';
//    ShowMessage(sMov);
  end;
...
if CBMOV.Text = 'Completo' then
  ...
  DM.cdsDinamica.CommandText := ' SELECT SUM(CP.VL_PARCELA) FROM TB_CONTAS_PAGAR CP '+
                                ' JOIN tb_cliente C on (C.id_cliente = CP.id_cliente ) '+
                                ' JOIN tb_grupo_empresa G on (G.id_grupo_empresa = C.id_grupo_empresa )'+
                                ' WHERE CP.ID_FORNECEDOR = '+cdsContasPagarID_FORNECEDOR.AsString+
                                ' AND G.id_grupo_empresa = '+edtCli.Text+
                                ' AND CP.status =''A''';
...
begin
  ...
  begin
    ...
    begin
      ...
      TotPag:= TotPag + FieldByName('VL_PAGO').Value;
      TotGeral := (Total-TotPag);
      Next;
    end;
    Label10.Caption:='Total dos Registros  '+(Format('%m', [TotGeral]));
  end;
