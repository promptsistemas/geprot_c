...
uses
  ...
  Vcl.PlatformDefaultStyleActnCtrls, Vcl.ActnPopup, qrpctrls ;
...
type
  TfrmPrecoTinturaria = class(TForm)
    ...
    BB_INC_SERV: TBitBtn;
    BitBtn3: TBitBtn;
    BB_CANC_SERV: TBitBtn;
    BB_SALVA_SERV: TBitBtn;
    BB_EXC_SERV: TBitBtn;
    DBGrid3: TDBGrid;
    P_SERVICO: TPanel;
    Label24: TLabel;
    ...
    procedure BB_INC_SERVClick(Sender: TObject);
    procedure BB_CANC_SERVClick(Sender: TObject);
    procedure BB_SALVA_SERVClick(Sender: TObject);
    procedure BB_EXC_SERVClick(Sender: TObject);
...
procedure TfrmPrecoTinturaria.GerenciaBotoes4;
begin
  BB_INC_SERV.Enabled := not (DM.cdsServicoTinturaria.State in dsEditModes);
  BB_SALVA_SERV.Enabled := NOT BB_INC_SERV.Enabled;
  BB_EXC_SERV.Enabled := BB_INCLUIR.Enabled and not (DM.cdsServicoTinturaria.IsEmpty);
  BB_CANC_SERV.Enabled := BB_SALVA_SERV.Enabled;
//  BB_ALTERAR2.Enabled := BB_EXCLUIR2.Enabled;
  P_SERVICO.Enabled := not BB_INC_SERV.Enabled;
end;

procedure TfrmPrecoTinturaria.Imprimir1Click(Sender: TObject);
begin
  QR_TINTURARIA.Prepare;
  QR_TINTURARIA.Print;
end;
...
begin
  ...
  DM.cdsArtigo.Open;
  Pesquisa(DM.cdsArtigo,['NOME_ARTIGO','ID_ARTIGO'],['Descrição Artigo','Código'],[40,4],'','','','','Artigo','Artigo');
...
begin
  ...
  DM.cdsCores.CommandText := ' SELECT C.ID_CORES, C.ID_FORNECEDOR, C.CODIGO, C.DESCRICAO, F.NOME AS TINTURARIAA '+
                             ' FROM TB_CORES C '+
                             ' LEFT OUTER JOIN TB_FORNECEDOR F ON (F.ID_FORNECEDOR = C.ID_FORNECEDOR) '+
                             ' WHERE C.ID_FORNECEDOR = '+QuotedStr(DBEdit1.Text);
  DM.cdsCores.Open;

  if DM.cdsCores.RecordCount > 0 then
  begin
    Pesquisa(DM.cdsCores,['DESCRICAO','CODIGO'],['Cor','Código'],[40,6],'','','','','Cores','Cores');
  ...
  begin
    ShowMessage('Nehuma Cor Cadastrada Para Este Fornecedor.');
...
procedure TfrmPrecoTinturaria.SpeedButton7Click(Sender: TObject);
begin
  DM.cdsServico.Close;
  DM.cdsServico.Open;
  Pesquisa(DM.cdsServico,['DESCRICAO','VALOR'],['Descrição ','Valor'],[30,14],'','','','','Serviço','Serviço');
  DM.cdsServicoTinturariaID_ARTIGO.AsInteger       := DM.cdsArtigoID_ARTIGO.AsInteger;
  DM.cdsServicoTinturariaID_SERVICO.AsInteger      := DM.cdsServicoID_SERVICO.AsInteger;
  DM.cdsServicoTinturariaID_PRECO_TINTURARIA.AsInteger  := DM.cdsPrecoTinturariaID_PRECO_TINTURARIA.AsInteger;
  DM.cdsServicoTinturariaID_ARTIGO_TINTURARIA.AsInteger := DM.cdsArtigoTinturariaID_ARTIGO_TINTURARIA.AsInteger;
  DM.cdsServicoTinturariaSERVICO.AsString          := DM.cdsServicoDESCRICAO.AsString;
  DM.cdsServicoTinturariaVL_SERVICO.AsFloat        := DM.cdsServicoVALOR.AsFloat;
  DM.cdsServicoTinturariaVALOR.AsFloat             := DM.cdsServicoVALOR.AsFloat;

  BB_SALVA_SERV.SetFocus;
...
begin
  ...
  DM.cdsServicoTinturaria.Close;
  DM.cdsServicoTinturaria.Open;
  ...
  begin
    DM.cdsServicoTinturaria.Close;
    DM.cdsServicoTinturaria.Params[0].AsInteger := DM.cdsPrecoTinturariaID_PRECO_TINTURARIA.AsInteger;
    DM.cdsServicoTinturaria.Open;
  end;
...
procedure TfrmPrecoTinturaria.DBEdit19Exit(Sender: TObject);
begin
  if DBEdit19.Text <> '' then
  begin
    DM.cdsServico.Close;
    DM.cdsServico.Open;
    if DM.cdsServico.Locate('ID_SERVICO',DM.cdsServicoTinturariaID_SERVICO.AsInteger,[]) then
    begin
      DM.cdsServicoTinturariaID_ARTIGO.AsInteger       := DM.cdsArtigoID_ARTIGO.AsInteger;
      DM.cdsServicoTinturariaID_SERVICO.AsInteger      := DM.cdsServicoID_SERVICO.AsInteger;
      DM.cdsServicoTinturariaID_PRECO_TINTURARIA.AsInteger  := DM.cdsPrecoTinturariaID_PRECO_TINTURARIA.AsInteger;
  ...
  begin
    ShowMessage('Nehum Código Informado!');
    BB_CANC_SERVClick(Self);
  end;


end;

procedure TfrmPrecoTinturaria.DBEdit19KeyPress(Sender: TObject; var Key: Char);
begin
  If Key = #27 Then
   BB_CANC_SERVClick(Self);
...
begin
  ...
  if DM.cdsServicoTinturaria.State in [dsEdit,dsInsert] then
  begin
    ShowMessage('Favor Salve ou Cancele o Registro Para Sair');
    Abort;
  end
...
procedure TfrmPrecoTinturaria.BB_CANC_SERVClick(Sender: TObject);
begin
  DM.cdsServicoTinturaria.Cancel;
  GerenciaBotoes4;
end;
...
begin
  ...
  begin
    ShowMessage('Impossivél Excluir. Exite(m) Cor(es) Vinculado(s) na Tabela.');
    Exit;
  end;
...
procedure TfrmPrecoTinturaria.BB_EXC_SERVClick(Sender: TObject);
begin
  if Application.MessageBox('Deseja Realmente excluir este registro ? ','Confirmação', MB_ICONQUESTION+MB_YESNO ) = idyes then
  begin
     DM.cdsServicoTinturaria.Delete;
     DM.cdsServicoTinturaria.ApplyUpdates(0);
     GerenciaBotoes3;
     GerenciaBotoes;
     GerenciaBotoes2;
     GerenciaBotoes4;
...
begin
  ...
  frmPesquisaPadrao.TB_PESQUISA := 'Preço-Tinturaria';
  ...
  frmPesquisaPadrao.LegendaGrouBox := 'Consulta de Preço-Tinturaria';
...
begin
  if DM.cdsCoresTinturariaVALOR.IsNull then
  begin
...
procedure TfrmPrecoTinturaria.BB_SALVA_SERVClick(Sender: TObject);
begin
  if Acao2 = 'Insert' then
  begin
     DM.cdsDinamica.Close;
     DM.cdsDinamica.IndexFieldNames:='';
     DM.cdsDinamica.CommandText := ' SELECT MAX(ID_SERVICO_TINTURARIA) FROM TB_SERVICO_TINTURARIA';
     DM.cdsDinamica.Open;

     DM.cdsServicoTinturariaID_SERVICO_TINTURARIA.AsInteger   := DM.cdsDinamica.Fields[0].AsInteger+1;
     DM.cdsServicoTinturariaID_PRECO_TINTURARIA.AsInteger   := DM.cdsCoresTinturariaID_PRECO_TINTURARIA.AsInteger;
     DM.cdsServicoTinturariaID_ARTIGO_TINTURARIA.AsInteger  := DM.cdsCoresTinturariaID_ARTIGO_TINTURARIA.AsInteger;
     DM.cdsServicoTinturariaID_ARTIGO.AsInteger             := DM.cdsCoresTinturariaID_ARTIGO.AsInteger;
     DM.cdsServicoTinturaria.ApplyUpdates(0);
     GerenciaBotoes4;
     BB_INC_SERVClick(Self);
  end;
...
procedure TfrmPrecoTinturaria.BB_INC_SERVClick(Sender: TObject);
begin
  if DM.cdsPrecoTinturaria.RecordCount = 0 then
  begin
    ShowMessage('Nenhuma Tabela Cadastrada Para Inserir Registro ');
  ...
  Acao2:= 'Insert';
  DM.cdsServicoTinturaria.Open;
  DM.cdsServicoTinturaria.Insert;
  GerenciaBotoes4;
  DBEDIT19.SetFocus;
