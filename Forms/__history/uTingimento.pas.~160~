...
begin
  ...
  begin
    ...
    Pesquisa(DM.cdsDinamica,['NR_NOTA_REMESSA'],['Nº Nota Fiscal'],[20],'','','','','Nota Fiscal','Nota Fiscal');
...
begin
  ...
  if cdsContasPagar.RecordCount = 0 then
  begin
   ShowMessage('Nenhum Pagamento Lançado Para Este Lançamento. Impossivél Finalizar');
   Exit
  end;

  if DM.cdsNotaRetorno.RecordCount = 0 then
  begin
   ShowMessage('Nenhuma Nota de Retorno Lançada Para Este Lançamento. Impossivél Finalizar');
   Exit
  end;
  if DM.cdsNotaQuebra.RecordCount = 0 then
  begin
   ShowMessage('Nenhuma Nota de Quebra Lançada Para Este Lançamento. Impossivél Finalizar');
   Exit
  end;
...
begin
  if DBEdit16.Text = '' then
  begin
    ShowMessage('Favor Informa Nº NF Remessa');
    DBEdit16.SetFocus;
    Exit
  end
  else
...
begin
  if Tp_operacao = 'Insert' then
    ...
    begin
      ShowMessage(' Saldo Indiponivél Para a Quantidade Informada.');
    ...
    begin
      ShowMessage(' Quantidade Informada Indisponivél.'+#13+' Saldo Disponivél  : '+QuotedStr(FormatFloat('###,###0.000',sSaldo)));
...
begin
 dCorAlterada := cdsDinamicaFiosDESCRICAO.AsString;
  ...
  if Application.MessageBox('Confirma a Mudança de Cor ?','Confirmação',MB_ICONQUESTION+MB_YESNO ) = idyes  then
  begin
    DM.cdsDinamica.Close;
  ...
  begin
    ShowMessage('Mudança de Cor Cancelada.');
  end;
