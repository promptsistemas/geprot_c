object frmRemessaProducao: TfrmRemessaProducao
  Left = 0
  Top = 0
  BorderIcons = []
  BorderStyle = bsNone
  Caption = 'PRODU'#199#195'O'
  ClientHeight = 600
  ClientWidth = 812
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  OldCreateOrder = False
  Position = poScreenCenter
  OnClose = FormClose
  OnKeyPress = FormKeyPress
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object Panel1: TPanel
    Left = 0
    Top = 0
    Width = 812
    Height = 41
    Align = alTop
    Color = clBlue
    Font.Charset = ANSI_CHARSET
    Font.Color = clBlue
    Font.Height = -21
    Font.Name = 'Segoe UI Semibold'
    Font.Style = [fsBold]
    ParentBackground = False
    ParentFont = False
    TabOrder = 0
    object DBText1: TDBText
      Left = 647
      Top = 5
      Width = 130
      Height = 28
      DataField = 'NR_NOTA_FISCAL'
      Font.Charset = ANSI_CHARSET
      Font.Color = clLime
      Font.Height = -24
      Font.Name = 'Segoe UI'
      Font.Style = [fsBold, fsItalic]
      ParentFont = False
    end
    object Label17: TLabel
      Left = 8
      Top = 4
      Width = 454
      Height = 37
      Caption = 'ENTRADA NOTA FISCAL MALHARIA'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWhite
      Font.Height = -27
      Font.Name = 'Segoe UI'
      Font.Style = [fsBold, fsItalic]
      ParentFont = False
    end
    object Label18: TLabel
      Left = 472
      Top = 5
      Width = 162
      Height = 32
      Caption = 'N'#186' Nota Fiscal'
      Font.Charset = ANSI_CHARSET
      Font.Color = clYellow
      Font.Height = -24
      Font.Name = 'Segoe UI'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object SpeedButton3: TSpeedButton
      Left = 773
      Top = 1
      Width = 38
      Height = 39
      Align = alRight
      Flat = True
      Glyph.Data = {
        
        000000000000}
      OnClick = SpeedButton3Click
      ExplicitLeft = 1102
      ExplicitTop = 5
      ExplicitHeight = 36
    end
  end
  object DBNavigator1: TDBNavigator
    Left = 584
    Top = 43
    Width = 224
    Height = 37
    DataSource = DM.dsRemessaProducao
    VisibleButtons = [nbFirst, nbPrior, nbNext, nbLast]
    TabOrder = 1
    OnClick = DBNavigator1Click
    OnDblClick = DBNavigator1DblClick
  end
  object itemProcucao: TPanel
    Left = 0
    Top = 265
    Width = 812
    Height = 267
    TabOrder = 2
    object Bevel1: TBevel
      Left = 4
      Top = 43
      Width = 85
      Height = 218
    end
    object BB_INCLUIR: TBitBtn
      Left = 9
      Top = 51
      Width = 71
      Height = 37
      Caption = '&Incluir'
      Glyph.Data = {
        
        00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
      TabOrder = 0
      OnClick = BB_INCLUIRClick
    end
    object BB_ALTERAR: TBitBtn
      Left = 9
      Top = 87
      Width = 71
      Height = 37
      Caption = '&Alterar'
      Glyph.Data = {
        
        00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
      TabOrder = 1
      OnClick = BB_ALTERARClick
    end
    object BB_CANC: TBitBtn
      Left = 9
      Top = 159
      Width = 71
      Height = 37
      Caption = '&Cancelar'
      Glyph.Data = {
        
        00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
      TabOrder = 2
      OnClick = BB_CANCClick
    end
    object BB_EXC: TBitBtn
      Left = 9
      Top = 196
      Width = 71
      Height = 37
      Caption = '&Excluir'
      Glyph.Data = {
        
        00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
      TabOrder = 3
      OnClick = BB_EXCClick
    end
    object Panel2: TPanel
      Left = 1
      Top = 1
      Width = 810
      Height = 41
      Align = alTop
      Color = clBlue
      Font.Charset = ANSI_CHARSET
      Font.Color = clBlue
      Font.Height = -21
      Font.Name = 'Segoe UI Semibold'
      Font.Style = [fsBold]
      ParentBackground = False
      ParentFont = False
      TabOrder = 4
      object Label12: TLabel
        Left = 8
        Top = 2
        Width = 93
        Height = 37
        Caption = 'Artigos'
        Font.Charset = ANSI_CHARSET
        Font.Color = clWhite
        Font.Height = -27
        Font.Name = 'Segoe UI'
        Font.Style = [fsBold, fsItalic]
        ParentFont = False
      end
      object L_AVISO: TLabel
        Left = 483
        Top = 5
        Width = 311
        Height = 28
        Caption = 'Nota Sem Pagamentos Lan'#231'ados'
        Color = clBlue
        Font.Charset = ANSI_CHARSET
        Font.Color = clYellow
        Font.Height = -20
        Font.Name = 'Segoe UI'
        Font.Style = [fsBold, fsItalic]
        ParentColor = False
        ParentFont = False
        Visible = False
      end
    end
    object BB_SALVAR: TBitBtn
      Left = 9
      Top = 123
      Width = 71
      Height = 37
      Caption = '&Concluir'
      Glyph.Data = {
        
        C3C3C4C3C3C4C3C3C3C3C3CFC9C8B884849644459A4C4DFF00FF}
      TabOrder = 5
      OnClick = BB_SALVARClick
    end
    object PageControl2: TPageControl
      Left = 95
      Top = 43
      Width = 714
      Height = 220
      ActivePage = TabSheet3
      TabOrder = 6
      OnChange = PageControl2Change
      object TabSheet3: TTabSheet
        Caption = 'Artigos'
        object DBGrid2: TDBGrid
          Left = 1
          Top = 1
          Width = 705
          Height = 190
          Color = clBtnFace
          Ctl3D = False
          DataSource = DM.dsItemRemessa
          FixedColor = cl3DLight
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Tahoma'
          Font.Style = []
          Options = [dgTitles, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit, dgTitleClick, dgTitleHotTrack]
          ParentCtl3D = False
          ParentFont = False
          ParentShowHint = False
          ShowHint = False
          TabOrder = 0
          TitleFont.Charset = DEFAULT_CHARSET
          TitleFont.Color = clBlue
          TitleFont.Height = -11
          TitleFont.Name = 'Tahoma'
          TitleFont.Style = []
          Columns = <
            item
              Expanded = False
              FieldName = 'ID_ARTIGO'
              Width = 81
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'NOME_ARTIGO'
              Width = 145
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'QUANT_FATURADO'
              Width = 71
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'VL_UNIT'
              Width = 92
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'VL_TOTAL'
              Width = 105
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'NR_NOTA_FISCAL'
              Title.Caption = 'N'#186' Nota Fiscal'
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'NR_NF_REMESSA'
              Title.Caption = 'N'#186' NF Remessa'
              Visible = True
            end>
        end
      end
      object TabSheet4: TTabSheet
        Caption = 'Contas a Pagar'
        ImageIndex = 1
        object L_SALDO: TLabel
          Left = 354
          Top = 156
          Width = 188
          Height = 36
          Caption = 'Total R$ 0,00'
          Font.Charset = ANSI_CHARSET
          Font.Color = clTeal
          Font.Height = -31
          Font.Name = 'Arial Narrow'
          Font.Style = [fsBold]
          ParentFont = False
        end
        object DBGrid4: TDBGrid
          Left = 0
          Top = 3
          Width = 705
          Height = 155
          Ctl3D = False
          DataSource = dsContasPagar
          FixedColor = 15532031
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Tahoma'
          Font.Style = []
          Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit, dgTitleClick, dgTitleHotTrack]
          ParentColor = True
          ParentCtl3D = False
          ParentFont = False
          ReadOnly = True
          TabOrder = 0
          TitleFont.Charset = ANSI_CHARSET
          TitleFont.Color = clBlue
          TitleFont.Height = -11
          TitleFont.Name = 'Segoe UI'
          TitleFont.Style = []
          Columns = <
            item
              Expanded = False
              FieldName = 'DT_EMISSAO'
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'DT_VENCIMENTO'
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'QT_DIAS'
              Width = 47
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'QT_PARCELA'
              Width = 47
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'NR_DOC'
              Width = 74
              Visible = True
            end
            item
              Color = 15332329
              Expanded = False
              FieldName = 'VL_PARCELA'
              Font.Charset = ANSI_CHARSET
              Font.Color = clNavy
              Font.Height = -11
              Font.Name = 'Segoe UI Semibold'
              Font.Style = [fsBold]
              Width = 82
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'NR_PARCELA'
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'DESCRICAO_PAGAMENTO'
              Width = 133
              Visible = True
            end>
        end
        object BB_INC_PAG: TBitBtn
          Left = 0
          Top = 159
          Width = 124
          Height = 33
          Caption = '&Adicionar'
          Font.Charset = ANSI_CHARSET
          Font.Color = clGreen
          Font.Height = -15
          Font.Name = 'Segoe UI Semibold'
          Font.Style = [fsBold]
          Glyph.Data = {
            
            00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
          ParentFont = False
          TabOrder = 1
          OnClick = BB_INC_PAGClick
        end
        object BB_EXC_PAG: TBitBtn
          Left = 125
          Top = 159
          Width = 89
          Height = 33
          Caption = 'E&xcluir'
          Font.Charset = ANSI_CHARSET
          Font.Color = clRed
          Font.Height = -15
          Font.Name = 'Segoe UI Semibold'
          Font.Style = [fsBold]
          Glyph.Data = {
            
            00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
          ParentFont = False
          TabOrder = 2
          Visible = False
          OnClick = BB_EXC_PAGClick
        end
        object BB_VOL_PAG: TBitBtn
          Left = 214
          Top = 159
          Width = 89
          Height = 33
          Caption = '&Voltar'
          Font.Charset = ANSI_CHARSET
          Font.Color = clPurple
          Font.Height = -15
          Font.Name = 'Segoe UI Semibold'
          Font.Style = [fsBold]
          Glyph.Data = {
            
            8A1FFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
          ParentFont = False
          TabOrder = 3
          Visible = False
          OnClick = BB_VOL_PAGClick
        end
      end
    end
  end
  object DBNavigator2: TDBNavigator
    Left = 1
    Top = 248
    Width = 808
    Height = 18
    VisibleButtons = [nbFirst, nbPrior, nbNext, nbLast]
    TabOrder = 3
  end
  object p_item: TPanel
    Left = 1
    Top = 535
    Width = 811
    Height = 58
    Color = 15529196
    Enabled = False
    ParentBackground = False
    TabOrder = 4
    object Label15: TLabel
      Left = 3
      Top = 4
      Width = 59
      Height = 13
      Caption = 'C'#243'd. Artigo'
      FocusControl = DBEdit14
      Font.Charset = ANSI_CHARSET
      Font.Color = clBlue
      Font.Height = -11
      Font.Name = 'Segoe UI Semibold'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object Label16: TLabel
      Left = 81
      Top = 4
      Width = 66
      Height = 13
      Caption = 'Nome Artigo'
      FocusControl = DBEdit15
      Font.Charset = ANSI_CHARSET
      Font.Color = clBlue
      Font.Height = -11
      Font.Name = 'Segoe UI Semibold'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object Faturado: TLabel
      Left = 473
      Top = 4
      Width = 30
      Height = 13
      Caption = 'Quant'
      Font.Charset = ANSI_CHARSET
      Font.Color = clBlue
      Font.Height = -11
      Font.Name = 'Segoe UI Semibold'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object SpeedButton5: TSpeedButton
      Left = 349
      Top = -1
      Width = 23
      Height = 22
      Visible = False
      OnClick = SpeedButton5Click
    end
    object Label13: TLabel
      Left = 561
      Top = 4
      Width = 34
      Height = 13
      Caption = 'Vl.Unit'
      Font.Charset = ANSI_CHARSET
      Font.Color = clBlue
      Font.Height = -11
      Font.Name = 'Segoe UI Semibold'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object Label14: TLabel
      Left = 695
      Top = 4
      Width = 28
      Height = 13
      Caption = ' Total'
      Font.Charset = ANSI_CHARSET
      Font.Color = clBlue
      Font.Height = -11
      Font.Name = 'Segoe UI Semibold'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object Label8: TLabel
      Left = 640
      Top = 4
      Width = 44
      Height = 13
      Caption = 'Qt.Rolos'
      Font.Charset = ANSI_CHARSET
      Font.Color = clBlue
      Font.Height = -11
      Font.Name = 'Segoe UI Semibold'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object DBEdit14: TDBEdit
      Left = 3
      Top = 20
      Width = 77
      Height = 21
      DataField = 'ID_ARTIGO'
      DataSource = DM.dsItemRemessa
      TabOrder = 0
      OnExit = DBEdit14Exit
      OnKeyPress = DBEdit14KeyPress
    end
    object DBEdit15: TDBEdit
      Left = 81
      Top = 20
      Width = 392
      Height = 21
      DataField = 'NOME_ARTIGO'
      DataSource = DM.dsItemRemessa
      ReadOnly = True
      TabOrder = 5
      OnKeyPress = FormKeyPress
    end
    object DBEdit17: TDBEdit
      Left = 473
      Top = 20
      Width = 87
      Height = 21
      DataField = 'QUANT_FATURADO'
      DataSource = DM.dsItemRemessa
      TabOrder = 1
      OnExit = DBEdit17Exit
      OnKeyPress = FormKeyPress
    end
    object DBEdit12: TDBEdit
      Left = 561
      Top = 20
      Width = 78
      Height = 21
      DataField = 'VL_UNIT'
      DataSource = DM.dsItemRemessa
      TabOrder = 2
      OnExit = DBEdit12Exit
      OnKeyPress = FormKeyPress
    end
    object DBEdit13: TDBEdit
      Left = 699
      Top = 20
      Width = 109
      Height = 21
      DataField = 'VL_TOTAL'
      DataSource = DM.dsItemRemessa
      ReadOnly = True
      TabOrder = 4
      OnEnter = DBEdit13Enter
      OnExit = DBEdit13Exit
      OnKeyPress = FormKeyPress
    end
    object DBEdit18: TDBEdit
      Left = 640
      Top = 20
      Width = 59
      Height = 21
      DataField = 'QT_ROLOS'
      DataSource = DM.dsItemRemessa
      TabOrder = 3
      OnKeyPress = FormKeyPress
    end
  end
  object PageControl1: TPageControl
    Left = 1
    Top = 87
    Width = 811
    Height = 160
    ActivePage = TabSheet1
    TabOrder = 5
    object TabSheet1: TTabSheet
      Caption = 'Nota Fiscal'
      object P_PRODUCAO: TPanel
        Left = 3
        Top = -2
        Width = 810
        Height = 131
        Color = 15794175
        ParentBackground = False
        TabOrder = 0
        object Label1: TLabel
          Left = 2
          Top = 11
          Width = 33
          Height = 13
          Caption = 'C'#243'digo'
          FocusControl = DBEdit1
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlue
          Font.Height = -11
          Font.Name = 'Tahoma'
          Font.Style = []
          ParentFont = False
        end
        object Label2: TLabel
          Left = 76
          Top = 11
          Width = 67
          Height = 13
          Caption = 'N'#186' Nota Fiscal'
          FocusControl = DBEdit2
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlue
          Font.Height = -11
          Font.Name = 'Tahoma'
          Font.Style = []
          ParentFont = False
        end
        object Label4: TLabel
          Left = 2
          Top = 48
          Width = 64
          Height = 13
          Caption = 'Data Emiss'#227'o'
          FocusControl = DBEdit4
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlue
          Font.Height = -11
          Font.Name = 'Tahoma'
          Font.Style = []
          ParentFont = False
        end
        object Label7: TLabel
          Left = 688
          Top = 48
          Width = 67
          Height = 13
          Caption = 'Vl. Nota Fiscal'
          FocusControl = DBEdit7
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlue
          Font.Height = -11
          Font.Name = 'Tahoma'
          Font.Style = []
          ParentFont = False
        end
        object Label10: TLabel
          Left = 75
          Top = 88
          Width = 76
          Height = 13
          Caption = 'Nome Tinturaria'
          FocusControl = DBEdit10
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlue
          Font.Height = -11
          Font.Name = 'Tahoma'
          Font.Style = []
          ParentFont = False
        end
        object Label11: TLabel
          Left = 159
          Top = 48
          Width = 63
          Height = 13
          Caption = 'Nome Cliente'
          FocusControl = DBEdit11
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlue
          Font.Height = -11
          Font.Name = 'Tahoma'
          Font.Style = []
          ParentFont = False
        end
        object SpeedButton1: TSpeedButton
          Left = 259
          Top = -4
          Width = 23
          Height = 22
          Visible = False
          OnClick = SpeedButton1Click
        end
        object Label9: TLabel
          Left = 610
          Top = 88
          Width = 74
          Height = 13
          Caption = 'N'#186' NF Remessa'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlue
          Font.Height = -11
          Font.Name = 'Tahoma'
          Font.Style = []
          ParentFont = False
        end
        object Label3: TLabel
          Left = 159
          Top = 11
          Width = 63
          Height = 13
          Caption = 'C'#243'd.Malharia'
          FocusControl = DBEdit11
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlue
          Font.Height = -11
          Font.Name = 'Tahoma'
          Font.Style = []
          ParentFont = False
        end
        object Label5: TLabel
          Left = 228
          Top = 11
          Width = 40
          Height = 13
          Caption = 'Malharia'
          FocusControl = DBEdit11
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlue
          Font.Height = -11
          Font.Name = 'Tahoma'
          Font.Style = []
          ParentFont = False
        end
        object SpeedButton2: TSpeedButton
          Left = 329
          Top = 2
          Width = 23
          Height = 22
          Visible = False
          OnClick = SpeedButton2Click
        end
        object SpeedButton4: TSpeedButton
          Left = 278
          Top = 49
          Width = 23
          Height = 22
          Visible = False
          OnClick = SpeedButton4Click
        end
        object SpeedButton6: TSpeedButton
          Left = 199
          Top = 89
          Width = 23
          Height = 22
          Visible = False
          OnClick = SpeedButton6Click
        end
        object Label21: TLabel
          Left = 2
          Top = 88
          Width = 41
          Height = 13
          Caption = 'C'#243'd.Tint'
          FocusControl = DBEdit11
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlue
          Font.Height = -11
          Font.Name = 'Tahoma'
          Font.Style = []
          ParentFont = False
        end
        object Label22: TLabel
          Left = 75
          Top = 48
          Width = 56
          Height = 13
          Caption = 'C'#243'd.Cliente'
          FocusControl = DBEdit11
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlue
          Font.Height = -11
          Font.Name = 'Tahoma'
          Font.Style = []
          ParentFont = False
        end
        object Label6: TLabel
          Left = 719
          Top = 88
          Width = 53
          Height = 13
          Caption = 'Total Rolos'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlue
          Font.Height = -11
          Font.Name = 'Tahoma'
          Font.Style = []
          ParentFont = False
        end
        object DBEdit1: TDBEdit
          Left = 2
          Top = 24
          Width = 72
          Height = 19
          CharCase = ecUpperCase
          Ctl3D = False
          DataField = 'ID_REMESSA'
          DataSource = DM.dsRemessaProducao
          ParentCtl3D = False
          ReadOnly = True
          TabOrder = 0
          OnKeyPress = FormKeyPress
        end
        object DBEdit2: TDBEdit
          Left = 75
          Top = 24
          Width = 82
          Height = 19
          CharCase = ecUpperCase
          Ctl3D = False
          DataField = 'NR_NOTA_FISCAL'
          DataSource = DM.dsRemessaProducao
          ParentCtl3D = False
          TabOrder = 1
          OnExit = DBEdit2Exit
          OnKeyPress = DBEdit2KeyPress
        end
        object DBEdit4: TDBEdit
          Left = 2
          Top = 64
          Width = 72
          Height = 19
          CharCase = ecUpperCase
          Ctl3D = False
          DataField = 'DT_EMISSAO'
          DataSource = DM.dsRemessaProducao
          ParentCtl3D = False
          TabOrder = 3
          OnKeyPress = FormKeyPress
        end
        object DBEdit6: TDBEdit
          Left = 107
          Top = 104
          Width = 134
          Height = 19
          CharCase = ecUpperCase
          Ctl3D = False
          DataField = 'QUANT_NF'
          ParentCtl3D = False
          TabOrder = 5
          Visible = False
          OnKeyPress = FormKeyPress
        end
        object DBEdit7: TDBEdit
          Left = 688
          Top = 64
          Width = 108
          Height = 19
          CharCase = ecUpperCase
          Color = clInfoBk
          Ctl3D = False
          DataField = 'VL_NOTA'
          DataSource = DM.dsRemessaProducao
          ParentCtl3D = False
          TabOrder = 6
          OnKeyPress = FormKeyPress
        end
        object DBEdit10: TDBEdit
          Left = 75
          Top = 104
          Width = 533
          Height = 19
          CharCase = ecUpperCase
          Ctl3D = False
          DataField = 'TITURARIA'
          DataSource = DM.dsRemessaProducao
          ParentCtl3D = False
          ReadOnly = True
          TabOrder = 9
        end
        object DBEdit11: TDBEdit
          Left = 159
          Top = 64
          Width = 527
          Height = 19
          CharCase = ecUpperCase
          Ctl3D = False
          DataField = 'CLIENTE'
          DataSource = DM.dsRemessaProducao
          ParentCtl3D = False
          ReadOnly = True
          TabOrder = 8
        end
        object DBEdit9: TDBEdit
          Left = 610
          Top = 104
          Width = 107
          Height = 19
          CharCase = ecUpperCase
          Color = 16769476
          Ctl3D = False
          DataField = 'VL_TOTAL_NF'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlue
          Font.Height = -11
          Font.Name = 'Segoe UI Semibold'
          Font.Style = [fsBold]
          ParentCtl3D = False
          ParentFont = False
          ReadOnly = True
          TabOrder = 10
          Visible = False
          OnKeyPress = FormKeyPress
        end
        object DBEdit3: TDBEdit
          Left = 159
          Top = 24
          Width = 68
          Height = 19
          CharCase = ecUpperCase
          Ctl3D = False
          DataField = 'ID_FOR_REMESSA'
          DataSource = DM.dsRemessaProducao
          ParentCtl3D = False
          TabOrder = 2
          OnExit = DBEdit3Exit
          OnKeyPress = DBEdit3KeyPress
        end
        object DBEdit5: TDBEdit
          Left = 228
          Top = 24
          Width = 568
          Height = 19
          CharCase = ecUpperCase
          Ctl3D = False
          DataField = 'NOME_MALHARIA'
          DataSource = DM.dsRemessaProducao
          ParentCtl3D = False
          ReadOnly = True
          TabOrder = 11
        end
        object DBEdit16: TDBEdit
          Left = 610
          Top = 104
          Width = 108
          Height = 19
          CharCase = ecUpperCase
          Ctl3D = False
          DataField = 'NR_NF_REMESSA'
          DataSource = DM.dsRemessaProducao
          ParentCtl3D = False
          TabOrder = 12
          OnExit = DBEdit16Exit
          OnKeyPress = FormKeyPress
        end
        object DBEdit19: TDBEdit
          Left = 2
          Top = 104
          Width = 72
          Height = 19
          CharCase = ecUpperCase
          Ctl3D = False
          DataField = 'ID_FORN_TINT'
          DataSource = DM.dsRemessaProducao
          ParentCtl3D = False
          TabOrder = 7
          OnExit = DBEdit19Exit
          OnKeyPress = DBEdit19KeyPress
        end
        object DBEdit20: TDBEdit
          Left = 75
          Top = 64
          Width = 82
          Height = 19
          CharCase = ecUpperCase
          Ctl3D = False
          DataField = 'ID_CLIENTE'
          DataSource = DM.dsRemessaProducao
          ParentCtl3D = False
          TabOrder = 4
          OnExit = DBEdit20Exit
          OnKeyPress = DBEdit20KeyPress
        end
        object DBEdit8: TDBEdit
          Left = 719
          Top = 104
          Width = 77
          Height = 19
          CharCase = ecUpperCase
          Ctl3D = False
          DataField = 'TOTAL_ROLOS'
          DataSource = DM.dsRemessaProducao
          ParentCtl3D = False
          ReadOnly = True
          TabOrder = 13
          OnExit = DBEdit16Exit
          OnKeyPress = FormKeyPress
        end
      end
    end
    object TabSheet2: TTabSheet
      Caption = 'Lan'#231'adas'
      ImageIndex = 1
      object DBGrid1: TDBGrid
        Left = 3
        Top = 3
        Width = 794
        Height = 126
        Ctl3D = False
        DataSource = DM.dsRemessaProducao
        Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit, dgTitleClick, dgTitleHotTrack]
        ParentCtl3D = False
        ReadOnly = True
        TabOrder = 0
        TitleFont.Charset = DEFAULT_CHARSET
        TitleFont.Color = clWindowText
        TitleFont.Height = -11
        TitleFont.Name = 'Tahoma'
        TitleFont.Style = []
        Columns = <
          item
            Expanded = False
            FieldName = 'DT_EMISSAO'
            Width = 71
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'NR_NOTA_FISCAL'
            Width = 111
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'NOME_MALHARIA'
            Width = 169
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'CLIENTE'
            Width = 205
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'VL_NOTA'
            Visible = True
          end>
      end
    end
  end
  object P_BOTOES: TPanel
    Left = 0
    Top = 43
    Width = 584
    Height = 38
    TabOrder = 6
    object BB_NOVO: TBitBtn
      Left = 1
      Top = 0
      Width = 71
      Height = 37
      Caption = '&Incluir'
      Glyph.Data = {
        
        00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
      TabOrder = 0
      OnClick = BB_NOVOClick
    end
    object BB_EDITAR: TBitBtn
      Left = 72
      Top = 0
      Width = 71
      Height = 37
      Caption = '&Alterar'
      Glyph.Data = {
        
        00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
      TabOrder = 1
      OnClick = BB_EDITARClick
    end
    object BB_GRAVAR: TBitBtn
      Left = 143
      Top = 0
      Width = 71
      Height = 37
      Caption = '&Salvar'
      Glyph.Data = {
        
        C3C3C4C3C3C4C3C3C3C3C3CFC9C8B884849644459A4C4DFF00FF}
      TabOrder = 2
      OnClick = BB_GRAVARClick
    end
    object BB_CANCELAR: TBitBtn
      Left = 214
      Top = 0
      Width = 76
      Height = 37
      Caption = '&Cancelar'
      Glyph.Data = {
        
        00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
      TabOrder = 3
      OnClick = BB_CANCELARClick
    end
    object BB_EXCLUIR: TBitBtn
      Left = 289
      Top = 0
      Width = 71
      Height = 37
      Caption = '&Excluir'
      Glyph.Data = {
        
        00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
      TabOrder = 4
      OnClick = BB_EXCLUIRClick
    end
    object BB_PESQUISAR: TBitBtn
      Left = 360
      Top = 0
      Width = 71
      Height = 37
      Caption = '&Pesquisar'
      Glyph.Data = {
        
        DB93A7B8B7AE9999DBDBDBDBDBDBDBDBDBDBDB9D9E9ADBDBDBDB}
      TabOrder = 5
      OnClick = BB_PESQUISARClick
    end
    object BB_SAIR: TBitBtn
      Left = 505
      Top = 0
      Width = 76
      Height = 37
      Caption = 'Sai&r'
      Glyph.Data = {
        
        00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
      TabOrder = 6
      OnClick = BB_SAIRClick
    end
    object BB_CONCLUIR: TBitBtn
      Left = 431
      Top = 0
      Width = 75
      Height = 37
      Caption = '&Finalizar'
      Glyph.Data = {
        
        00FFFF00FFFF00FF0A7D12FF00FFFF00FFFF00FFFF00FFFF00FF}
      TabOrder = 7
      OnClick = BB_CONCLUIRClick
    end
  end
  object P_SALDO_FIOS: TPanel
    Left = 143
    Top = 253
    Width = 812
    Height = 363
    Color = 14807295
    ParentBackground = False
    TabOrder = 7
    Visible = False
    object Panel4: TPanel
      Left = 1
      Top = 1
      Width = 810
      Height = 41
      Align = alTop
      Color = clBlue
      Font.Charset = ANSI_CHARSET
      Font.Color = clBlue
      Font.Height = -21
      Font.Name = 'Segoe UI Semibold'
      Font.Style = [fsBold]
      ParentBackground = False
      ParentFont = False
      TabOrder = 0
      object Label23: TLabel
        Left = 302
        Top = 2
        Width = 167
        Height = 37
        Caption = 'Saldo de Fios'
        Font.Charset = ANSI_CHARSET
        Font.Color = clYellow
        Font.Height = -27
        Font.Name = 'Segoe UI'
        Font.Style = [fsBold, fsItalic]
        ParentFont = False
      end
    end
    object BB_CONFIRMAR: TBitBtn
      Left = 649
      Top = 319
      Width = 76
      Height = 37
      Caption = '&Confirmar'
      Glyph.Data = {
        
        CB805DC87D43B96424A342FF00FFFF00FFFF00FFFF00FFFF00FF}
      TabOrder = 1
      OnClick = BB_CONFIRMARClick
    end
    object B_CANCELAR: TBitBtn
      Left = 726
      Top = 319
      Width = 83
      Height = 37
      Caption = 'Cancela&r'
      Glyph.Data = {
        
        59FF2552FF0034FC0029C6FF00FFFF00FFFF00FFFF00FFFF00FF}
      TabOrder = 2
      OnClick = B_CANCELARClick
    end
    object DBGrid3: TDBGrid
      Left = 4
      Top = 48
      Width = 805
      Height = 268
      BorderStyle = bsNone
      DataSource = dsDinamicaFios
      FixedColor = clAqua
      GradientEndColor = clRed
      GradientStartColor = clMoneyGreen
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      Options = [dgTitles, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit, dgTitleClick, dgTitleHotTrack]
      ParentColor = True
      ParentFont = False
      TabOrder = 3
      TitleFont.Charset = ANSI_CHARSET
      TitleFont.Color = clBlue
      TitleFont.Height = -11
      TitleFont.Name = 'Segoe UI'
      TitleFont.Style = []
      OnKeyPress = DBGrid3KeyPress
      Columns = <
        item
          Expanded = False
          FieldName = 'NR_NOTA_FISCAL'
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'NR_NF_REMESSA'
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'NOME_MATERIA'
          Width = 119
          Visible = True
        end
        item
          Color = clCream
          Expanded = False
          FieldName = 'QUANT_FATURADO'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlue
          Font.Height = -11
          Font.Name = 'Tahoma'
          Font.Style = []
          Title.Color = clMenuHighlight
          Width = 70
          Visible = True
        end
        item
          Color = 15332329
          Expanded = False
          FieldName = 'QUANT_PROD'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = 10329599
          Font.Height = -11
          Font.Name = 'Tahoma'
          Font.Style = [fsBold]
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clBlue
          Title.Font.Height = -11
          Title.Font.Name = 'Tahoma'
          Title.Font.Style = []
          Width = 65
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'SALDO'
          Width = 67
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'NOME'
          Width = 104
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'R_SOCIAL'
          Width = 98
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'NOME_ARTIGO'
          Width = 123
          Visible = True
        end>
    end
  end
  object qItemNotaFiscal: TSQLQuery
    MaxBlobSize = -1
    Params = <
      item
        DataType = ftInteger
        Name = 'ID_PRODUCAO'
        ParamType = ptInput
      end>
    SQL.Strings = (
      'SELECT'
      ' IP.ID_ITEM_PRODUCAO,'
      ' IP.ID_PRODUCAO,'
      ' IP.ID_MATERIA_PRIMA,'
      ' IP.QUANT_FATURADO,'
      ' IP.VL_UNIT,'
      ' IP.VL_TOTAL,'
      ' MP.NOME_MATERIA,'
      ' MP.PERC_FIO'
      'FROM TB_ITEM_PRODUCAO IP'
      
        'JOIN TB_MATERIA_PRIMA MP ON (MP.ID_MATERIA = IP.ID_MATERIA_PRIMA' +
        ')'
      'WHERE IP.ID_PRODUCAO = :ID_PRODUCAO')
    SQLConnection = DM.conexao
    Left = 464
    Top = 237
    object qItemNotaFiscalID_ITEM_PRODUCAO: TIntegerField
      FieldName = 'ID_ITEM_PRODUCAO'
      Required = True
    end
    object qItemNotaFiscalID_PRODUCAO: TIntegerField
      FieldName = 'ID_PRODUCAO'
    end
    object qItemNotaFiscalID_MATERIA_PRIMA: TIntegerField
      FieldName = 'ID_MATERIA_PRIMA'
    end
    object qItemNotaFiscalQUANT_FATURADO: TFMTBCDField
      FieldName = 'QUANT_FATURADO'
      Precision = 18
      Size = 4
    end
    object qItemNotaFiscalVL_UNIT: TFMTBCDField
      FieldName = 'VL_UNIT'
      Precision = 18
      Size = 3
    end
    object qItemNotaFiscalVL_TOTAL: TFMTBCDField
      FieldName = 'VL_TOTAL'
      Precision = 18
      Size = 3
    end
    object qItemNotaFiscalNOME_MATERIA: TStringField
      FieldName = 'NOME_MATERIA'
      Size = 30
    end
    object qItemNotaFiscalPERC_FIO: TFMTBCDField
      FieldName = 'PERC_FIO'
      Precision = 18
      Size = 4
    end
  end
  object dspItemNotaFiscal: TDataSetProvider
    DataSet = qItemNotaFiscal
    Options = [poIncFieldProps, poAllowMultiRecordUpdates, poAutoRefresh, poAllowCommandText, poUseQuoteChar]
    Left = 496
    Top = 237
  end
  object cdsItemNotaFiscal: TClientDataSet
    Aggregates = <>
    Params = <
      item
        DataType = ftInteger
        Name = 'ID_PRODUCAO'
        ParamType = ptInput
      end>
    ProviderName = 'dspItemNotaFiscal'
    Left = 528
    Top = 237
    object cdsItemNotaFiscalID_ITEM_PRODUCAO: TIntegerField
      FieldName = 'ID_ITEM_PRODUCAO'
      Required = True
    end
    object cdsItemNotaFiscalID_PRODUCAO: TIntegerField
      FieldName = 'ID_PRODUCAO'
    end
    object cdsItemNotaFiscalID_MATERIA_PRIMA: TIntegerField
      DisplayLabel = 'C'#243'd.Mat'#233'ria'
      FieldName = 'ID_MATERIA_PRIMA'
    end
    object cdsItemNotaFiscalQUANT_FATURADO: TFMTBCDField
      DisplayLabel = 'Quant'
      FieldName = 'QUANT_FATURADO'
      DisplayFormat = '###,###0.000'
      EditFormat = '###,###0.000'
      Precision = 18
      Size = 4
    end
    object cdsItemNotaFiscalVL_UNIT: TFMTBCDField
      DisplayLabel = 'Vl. Unit'
      FieldName = 'VL_UNIT'
      DisplayFormat = '###,###0.000'
      EditFormat = '###,###0.000'
      Precision = 18
      Size = 3
    end
    object cdsItemNotaFiscalVL_TOTAL: TFMTBCDField
      DisplayLabel = 'Vl. Total'
      FieldName = 'VL_TOTAL'
      DisplayFormat = '###,###0.000'
      EditFormat = '###,###0.000'
      Precision = 18
      Size = 3
    end
    object cdsItemNotaFiscalNOME_MATERIA: TStringField
      DisplayLabel = 'Mat'#233'ria-Prima'
      FieldName = 'NOME_MATERIA'
      Size = 30
    end
    object cdsItemNotaFiscalPERC_FIO: TFMTBCDField
      DisplayLabel = '% Fio'
      FieldName = 'PERC_FIO'
      Precision = 18
      Size = 4
    end
  end
  object dsItemNotaFiscal: TDataSource
    DataSet = cdsItemNotaFiscal
    Left = 560
    Top = 237
  end
  object qDinamicaFios: TSQLQuery
    MaxBlobSize = -1
    Params = <>
    SQL.Strings = (
      'SELECT'
      ' D.ID_PRODUCAO,'
      ' P.NR_NOTA_FISCAL,'
      ' P.NR_NF_REMESSA,'
      ' D.ID_MATERIA_PRIMA,'
      ' M.NOME_MATERIA,'
      ' D.QUANT_FATURADO,'
      ' D.ID_FORNCEDOR,'
      ' F.NOME,'
      ' D.ID_CLIENTE,'
      ' C.R_SOCIAL,'
      ' D.ID_ARTIGO,'
      ' A.NOME_ARTIGO,'
      ' D.QUANT_PROD,'
      ' D.SALDO,'
      ' D.QT_ARTIGO'
      'FROM DINAMICA_FIOS D'
      
        'LEFT OUTER JOIN TB_FORNECEDOR F ON (F.ID_FORNECEDOR = D.ID_FORNC' +
        'EDOR)'
      'LEFT OUTER JOIN TB_CLIENTE C ON (C.ID_CLIENTE = D.ID_CLIENTE)'
      'LEFT OUTER JOIN TB_ARTIGO A ON (A.ID_ARTIGO = D.ID_ARTIGO)'
      
        'LEFT OUTER JOIN TB_MATERIA_PRIMA M ON (M.ID_MATERIA = D.ID_MATER' +
        'IA_PRIMA)'
      'LEFT OUTER JOIN TB_PRODUCAO P ON (P.ID_PRODUCAO = D.ID_PRODUCAO)'
      'WHERE D.SALDO >0')
    SQLConnection = DM.conexao
    Left = 384
    Top = 382
    object qDinamicaFiosID_PRODUCAO: TIntegerField
      FieldName = 'ID_PRODUCAO'
    end
    object qDinamicaFiosID_MATERIA_PRIMA: TIntegerField
      FieldName = 'ID_MATERIA_PRIMA'
    end
    object qDinamicaFiosNOME_MATERIA: TStringField
      FieldName = 'NOME_MATERIA'
      Size = 30
    end
    object qDinamicaFiosQUANT_FATURADO: TFMTBCDField
      FieldName = 'QUANT_FATURADO'
      Precision = 18
      Size = 3
    end
    object qDinamicaFiosID_FORNCEDOR: TIntegerField
      FieldName = 'ID_FORNCEDOR'
    end
    object qDinamicaFiosNOME: TStringField
      FieldName = 'NOME'
      Size = 50
    end
    object qDinamicaFiosID_CLIENTE: TIntegerField
      FieldName = 'ID_CLIENTE'
    end
    object qDinamicaFiosR_SOCIAL: TStringField
      FieldName = 'R_SOCIAL'
      Size = 50
    end
    object qDinamicaFiosID_ARTIGO: TIntegerField
      FieldName = 'ID_ARTIGO'
    end
    object qDinamicaFiosNOME_ARTIGO: TStringField
      FieldName = 'NOME_ARTIGO'
      Size = 30
    end
    object qDinamicaFiosNR_NOTA_FISCAL: TIntegerField
      FieldName = 'NR_NOTA_FISCAL'
    end
    object qDinamicaFiosNR_NF_REMESSA: TIntegerField
      FieldName = 'NR_NF_REMESSA'
    end
    object qDinamicaFiosQUANT_PROD: TFMTBCDField
      FieldName = 'QUANT_PROD'
      Precision = 18
      Size = 3
    end
    object qDinamicaFiosSALDO: TFMTBCDField
      FieldName = 'SALDO'
      Precision = 18
      Size = 3
    end
    object qDinamicaFiosQT_ARTIGO: TIntegerField
      FieldName = 'QT_ARTIGO'
    end
  end
  object dspDinamicaFios: TDataSetProvider
    DataSet = qDinamicaFios
    Options = [poIncFieldProps, poAllowMultiRecordUpdates, poAutoRefresh, poAllowCommandText, poUseQuoteChar]
    Left = 416
    Top = 382
  end
  object cdsDinamicaFios: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'dspDinamicaFios'
    Left = 448
    Top = 382
    object cdsDinamicaFiosID_PRODUCAO: TIntegerField
      FieldName = 'ID_PRODUCAO'
    end
    object cdsDinamicaFiosID_MATERIA_PRIMA: TIntegerField
      FieldName = 'ID_MATERIA_PRIMA'
    end
    object cdsDinamicaFiosNOME_MATERIA: TStringField
      DisplayLabel = 'Mat'#233'ria-Prima'
      FieldName = 'NOME_MATERIA'
      Size = 30
    end
    object cdsDinamicaFiosQUANT_FATURADO: TFMTBCDField
      DisplayLabel = 'Quant Fat'
      FieldName = 'QUANT_FATURADO'
      DisplayFormat = '###,###0.000'
      EditFormat = '###,###0.000'
      Precision = 18
      Size = 3
    end
    object cdsDinamicaFiosID_FORNCEDOR: TIntegerField
      FieldName = 'ID_FORNCEDOR'
    end
    object cdsDinamicaFiosNOME: TStringField
      DisplayLabel = 'Fornecedor'
      FieldName = 'NOME'
      Size = 50
    end
    object cdsDinamicaFiosID_CLIENTE: TIntegerField
      FieldName = 'ID_CLIENTE'
    end
    object cdsDinamicaFiosR_SOCIAL: TStringField
      DisplayLabel = 'Cliente'
      FieldName = 'R_SOCIAL'
      Size = 50
    end
    object cdsDinamicaFiosID_ARTIGO: TIntegerField
      FieldName = 'ID_ARTIGO'
    end
    object cdsDinamicaFiosNOME_ARTIGO: TStringField
      DisplayLabel = 'Artigo'
      FieldName = 'NOME_ARTIGO'
      Size = 30
    end
    object cdsDinamicaFiosNR_NOTA_FISCAL: TIntegerField
      DisplayLabel = 'N'#186' Nota Fiscal'
      FieldName = 'NR_NOTA_FISCAL'
    end
    object cdsDinamicaFiosNR_NF_REMESSA: TIntegerField
      DisplayLabel = 'N'#186' NF Remessa'
      FieldName = 'NR_NF_REMESSA'
    end
    object cdsDinamicaFiosQUANT_PROD: TFMTBCDField
      DisplayLabel = 'Quant Prod'
      FieldName = 'QUANT_PROD'
      DisplayFormat = '###,###0.000'
      EditFormat = '###,###0.000'
      Precision = 18
      Size = 3
    end
    object cdsDinamicaFiosSALDO: TFMTBCDField
      DisplayLabel = 'Saldo'
      FieldName = 'SALDO'
      DisplayFormat = '###,###0.000'
      EditFormat = '###,###0.000'
      Precision = 18
      Size = 3
    end
    object cdsDinamicaFiosQT_ARTIGO: TIntegerField
      FieldName = 'QT_ARTIGO'
    end
  end
  object dsDinamicaFios: TDataSource
    DataSet = cdsDinamicaFios
    Left = 480
    Top = 382
  end
  object dspVerifcaSaldoFios: TDataSetProvider
    DataSet = spVerifcaSaldoFios
    Options = [poIncFieldProps, poAllowMultiRecordUpdates, poAutoRefresh, poAllowCommandText, poUseQuoteChar]
    Left = 593
    Top = 380
  end
  object cdsVerifcaSaldoFios: TClientDataSet
    Aggregates = <>
    Params = <
      item
        DataType = ftInteger
        Precision = 4
        Name = 'PE_ID_CLIENTE'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Precision = 4
        Name = 'PE_ID_ARTIGO'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Precision = 4
        Name = 'PE_ID_FORN'
        ParamType = ptInput
      end>
    ProviderName = 'dspVerifcaSaldoFios'
    Left = 624
    Top = 380
  end
  object spVerifcaSaldoFios: TSQLStoredProc
    MaxBlobSize = -1
    Params = <
      item
        DataType = ftInteger
        Precision = 4
        Name = 'PE_ID_CLIENTE'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Precision = 4
        Name = 'PE_ID_ARTIGO'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Precision = 4
        Name = 'PE_ID_FORN'
        ParamType = ptInput
      end>
    SQLConnection = DM.conexao
    StoredProcName = 'VERICIA_SALDO_FIOS'
    Left = 563
    Top = 380
  end
  object spAtuSaldoFios: TSQLStoredProc
    MaxBlobSize = -1
    Params = <
      item
        DataType = ftInteger
        Precision = 4
        Name = 'PE_NR_NOTA_FISCAL'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Precision = 4
        Name = 'PE_ID_MATERIA'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Precision = 4
        Name = 'PE_ID_FORNECEDOR'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Precision = 4
        Name = 'PE_ID_CLIENTE'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Precision = 4
        Name = 'PE_ID_PRODUCAO'
        ParamType = ptInput
      end>
    SQLConnection = DM.conexao
    StoredProcName = 'ATU_SALDO_FIOS'
    Left = 560
    Top = 443
  end
  object cdsAtuSaldoFios: TClientDataSet
    Aggregates = <>
    Params = <
      item
        DataType = ftInteger
        Precision = 4
        Name = 'PE_NR_NOTA_FISCAL'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Precision = 4
        Name = 'PE_ID_MATERIA'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Precision = 4
        Name = 'PE_ID_FORNECEDOR'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Precision = 4
        Name = 'PE_ID_CLIENTE'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Precision = 4
        Name = 'PE_ID_PRODUCAO'
        ParamType = ptInput
      end>
    ProviderName = 'dspAtuSaldoFios'
    Left = 624
    Top = 443
  end
  object dspAtuSaldoFios: TDataSetProvider
    DataSet = spAtuSaldoFios
    Options = [poIncFieldProps, poAllowMultiRecordUpdates, poAutoRefresh, poAllowCommandText, poUseQuoteChar]
    Left = 592
    Top = 443
  end
  object cdsItemProducao: TClientDataSet
    Aggregates = <>
    Params = <
      item
        DataType = ftInteger
        Name = 'PE_ID_MATERIA_PRIMA'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Name = 'PE_ID_CLIENTE'
        ParamType = ptInput
      end>
    ProviderName = 'dspItemProducao'
    Left = 308
    Top = 430
    object cdsItemProducaoID_PRODUCAO: TIntegerField
      FieldName = 'ID_PRODUCAO'
    end
    object cdsItemProducaoID_MATERIA_PRIMA: TIntegerField
      FieldName = 'ID_MATERIA_PRIMA'
    end
    object cdsItemProducaoQUANT_FATURADO: TFMTBCDField
      FieldName = 'QUANT_FATURADO'
      Precision = 18
      Size = 4
    end
    object cdsItemProducaoQUANT_PROD: TFMTBCDField
      FieldName = 'QUANT_PROD'
      Precision = 18
      Size = 3
    end
    object cdsItemProducaoID_FORNCEDOR: TIntegerField
      FieldName = 'ID_FORNCEDOR'
      Required = True
    end
    object cdsItemProducaoID_CLIENTE: TIntegerField
      FieldName = 'ID_CLIENTE'
      Required = True
    end
  end
  object dspItemProducao: TDataSetProvider
    DataSet = qItemProducao
    Options = [poIncFieldProps, poAllowMultiRecordUpdates, poAutoRefresh, poAllowCommandText, poUseQuoteChar]
    Left = 276
    Top = 430
  end
  object dsItemProducao: TDataSource
    DataSet = cdsItemProducao
    Left = 340
    Top = 430
  end
  object qItemProducao: TSQLDataSet
    CommandText = 
      'SELECT'#13#10'     IP.ID_PRODUCAO,'#13#10'     IP.ID_MATERIA_PRIMA,'#13#10'     IP' +
      '.QUANT_FATURADO,'#13#10'     IP.QUANT_PROD,'#13#10'     P.ID_FORNCEDOR,'#13#10'   ' +
      '  P.ID_CLIENTE'#13#10'    FROM TB_ITEM_PRODUCAO IP'#13#10'    JOIN TB_PRODUC' +
      'AO P ON (P.ID_PRODUCAO = IP.ID_PRODUCAO)'#13#10'    WHERE IP.ID_MATERI' +
      'A_PRIMA = :PE_ID_MATERIA_PRIMA'#13#10'    AND P.ID_CLIENTE = :PE_ID_CL' +
      'IENTE'
    MaxBlobSize = -1
    Params = <
      item
        DataType = ftInteger
        Name = 'PE_ID_MATERIA_PRIMA'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Name = 'PE_ID_CLIENTE'
        ParamType = ptInput
      end>
    SQLConnection = DM.conexao
    Left = 248
    Top = 430
    object qItemProducaoID_PRODUCAO: TIntegerField
      FieldName = 'ID_PRODUCAO'
    end
    object qItemProducaoID_MATERIA_PRIMA: TIntegerField
      FieldName = 'ID_MATERIA_PRIMA'
    end
    object qItemProducaoQUANT_FATURADO: TFMTBCDField
      FieldName = 'QUANT_FATURADO'
      Precision = 18
      Size = 4
    end
    object qItemProducaoQUANT_PROD: TFMTBCDField
      FieldName = 'QUANT_PROD'
      Precision = 18
      Size = 3
    end
    object qItemProducaoID_FORNCEDOR: TIntegerField
      FieldName = 'ID_FORNCEDOR'
      Required = True
    end
    object qItemProducaoID_CLIENTE: TIntegerField
      FieldName = 'ID_CLIENTE'
      Required = True
    end
  end
  object qContasPagar: TSQLDataSet
    CommandText = 
      'SELECT'#13#10'    CP.ID_CONTAS_PAGAR    ,'#13#10'    CP.ID_PRODUCAO        ,' +
      #13#10'    CP.ID_CLIENTE         ,'#13#10'    CP.ID_PLANO_PAGAMENTO ,'#13#10'    ' +
      'CP.ID_USUARIO         ,'#13#10'    CP.ID_LOJA            ,'#13#10'    CP.ID_' +
      'FORNECEDOR,'#13#10'    CP.ID_FORN_REM,'#13#10'    CP.DT_EMISSAO         ,'#13#10' ' +
      '   CP.DT_VENCIMENTO      ,'#13#10'    CP.DT_PAGAMENTO       ,'#13#10'    CP.' +
      'QT_DIAS            ,'#13#10'    CP.QT_DIAS_ATRAZO     ,'#13#10'    CP.QT_PAR' +
      'CELA         ,'#13#10'    CP.NR_DOC             ,'#13#10'    CP.VL_COMPRA   ' +
      '       ,'#13#10'    CP.VL_PARCELA         ,'#13#10'    CP.NR_PARCELA        ' +
      ' ,'#13#10'    CP.STATUS             ,'#13#10'    CP.NR_NOTA_FISCAL,'#13#10'    CP.' +
      'NR_NF_REMESSA,'#13#10'    CP.ORIGEM,'#13#10'    PP.DESCRICAO_PAGAMENTO,'#13#10'   ' +
      ' C.R_SOCIAL AS CLIENTE ,'#13#10'    F.NOME AS FORNECEDOR  ,'#13#10'    FR.NO' +
      'ME AS FORN_REMESSA,'#13#10'    U.NOME_USUARIO         ,'#13#10'    L.R_SOCIA' +
      'L AS EMPRESA'#13#10'FROM TB_CONTAS_PAGAR CP'#13#10'JOIN TB_PLANO_PAGAMENTO P' +
      'P ON (PP.ID_PLANO_PAGAMENTO = CP.ID_PLANO_PAGAMENTO)'#13#10'JOIN TB_CL' +
      'IENTE C ON (C.ID_CLIENTE = CP.ID_CLIENTE)'#13#10'JOIN TB_FORNECEDOR F ' +
      'ON (F.ID_FORNECEDOR = CP.ID_FORNECEDOR)'#13#10'JOIN TB_FORNECEDOR FR O' +
      'N (FR.ID_FORNECEDOR = CP.ID_FORN_REM)'#13#10'JOIN TB_USUARIO U ON (U.I' +
      'D_USUARIO = CP.ID_USUARIO)'#13#10'JOIN TB_LOJAS L ON (L.ID_LOJA = CP.I' +
      'D_LOJA)'#13#10'WHERE CP.ID_CLIENTE = :ID_CLIENTE'#13#10'AND CP.DT_EMISSAO   ' +
      '= :DT_EMISSAO'#13#10'AND CP.ID_FORNECEDOR  = :ID_FORNECEDOR'#13#10'AND CP.nr' +
      '_nota_fiscal = :NR_NF'#13#10'AND CP.TP_MOV = '#39'D'#39#13#10'ORDER BY CP.DT_VENCI' +
      'MENTO'
    MaxBlobSize = -1
    Params = <
      item
        DataType = ftUnknown
        Name = 'ID_CLIENTE'
        ParamType = ptInput
      end
      item
        DataType = ftUnknown
        Name = 'DT_EMISSAO'
        ParamType = ptInput
      end
      item
        DataType = ftUnknown
        Name = 'ID_FORNECEDOR'
        ParamType = ptInput
      end
      item
        DataType = ftUnknown
        Name = 'NR_NF'
        ParamType = ptInput
      end>
    SQLConnection = DM.conexao
    Left = 204
    Top = 274
    object qContasPagarID_CONTAS_PAGAR: TIntegerField
      FieldName = 'ID_CONTAS_PAGAR'
      ProviderFlags = []
    end
    object qContasPagarID_CLIENTE: TIntegerField
      FieldName = 'ID_CLIENTE'
      Required = True
    end
    object qContasPagarID_PLANO_PAGAMENTO: TIntegerField
      FieldName = 'ID_PLANO_PAGAMENTO'
      Required = True
    end
    object qContasPagarID_USUARIO: TIntegerField
      FieldName = 'ID_USUARIO'
      Required = True
    end
    object qContasPagarID_LOJA: TIntegerField
      FieldName = 'ID_LOJA'
      Required = True
    end
    object qContasPagarDT_EMISSAO: TDateField
      FieldName = 'DT_EMISSAO'
    end
    object qContasPagarDT_VENCIMENTO: TDateField
      FieldName = 'DT_VENCIMENTO'
    end
    object qContasPagarDT_PAGAMENTO: TDateField
      FieldName = 'DT_PAGAMENTO'
    end
    object qContasPagarQT_DIAS: TIntegerField
      FieldName = 'QT_DIAS'
    end
    object qContasPagarQT_DIAS_ATRAZO: TIntegerField
      FieldName = 'QT_DIAS_ATRAZO'
    end
    object qContasPagarQT_PARCELA: TIntegerField
      FieldName = 'QT_PARCELA'
    end
    object qContasPagarNR_DOC: TStringField
      FieldName = 'NR_DOC'
      Size = 15
    end
    object qContasPagarVL_COMPRA: TFMTBCDField
      FieldName = 'VL_COMPRA'
      Precision = 18
      Size = 2
    end
    object qContasPagarVL_PARCELA: TFMTBCDField
      FieldName = 'VL_PARCELA'
      Precision = 18
      Size = 2
    end
    object qContasPagarNR_PARCELA: TStringField
      FieldName = 'NR_PARCELA'
      Size = 10
    end
    object qContasPagarSTATUS: TStringField
      FieldName = 'STATUS'
      FixedChar = True
      Size = 1
    end
    object qContasPagarDESCRICAO_PAGAMENTO: TStringField
      FieldName = 'DESCRICAO_PAGAMENTO'
      ProviderFlags = []
      Size = 40
    end
    object qContasPagarCLIENTE: TStringField
      FieldName = 'CLIENTE'
      ProviderFlags = []
      Size = 50
    end
    object qContasPagarFORNECEDOR: TStringField
      FieldName = 'FORNECEDOR'
      ProviderFlags = []
      Size = 50
    end
    object qContasPagarFORN_REMESSA: TStringField
      FieldName = 'FORN_REMESSA'
      ProviderFlags = []
      Size = 50
    end
    object qContasPagarNOME_USUARIO: TStringField
      FieldName = 'NOME_USUARIO'
      ProviderFlags = []
      Size = 30
    end
    object qContasPagarEMPRESA: TStringField
      FieldName = 'EMPRESA'
      ProviderFlags = []
      Size = 35
    end
    object qContasPagarID_FORNECEDOR: TIntegerField
      FieldName = 'ID_FORNECEDOR'
      Required = True
    end
    object qContasPagarID_FORN_REM: TIntegerField
      FieldName = 'ID_FORN_REM'
      Required = True
    end
    object qContasPagarNR_NOTA_FISCAL: TIntegerField
      FieldName = 'NR_NOTA_FISCAL'
    end
    object qContasPagarNR_NF_REMESSA: TIntegerField
      FieldName = 'NR_NF_REMESSA'
    end
    object qContasPagarORIGEM: TStringField
      FieldName = 'ORIGEM'
    end
    object qContasPagarID_PRODUCAO: TIntegerField
      FieldName = 'ID_PRODUCAO'
    end
  end
  object dspContasPagar: TDataSetProvider
    DataSet = qContasPagar
    Options = [poIncFieldProps, poAllowMultiRecordUpdates, poAutoRefresh, poAllowCommandText, poUseQuoteChar]
    Left = 232
    Top = 274
  end
  object cdsContasPagar: TClientDataSet
    Aggregates = <>
    Params = <
      item
        DataType = ftUnknown
        Name = 'ID_CLIENTE'
        ParamType = ptInput
      end
      item
        DataType = ftUnknown
        Name = 'DT_EMISSAO'
        ParamType = ptInput
      end
      item
        DataType = ftUnknown
        Name = 'ID_FORNECEDOR'
        ParamType = ptInput
      end
      item
        DataType = ftUnknown
        Name = 'NR_NF'
        ParamType = ptInput
      end>
    ProviderName = 'dspContasPagar'
    Left = 260
    Top = 273
    object cdsContasPagarID_CONTAS_PAGAR: TIntegerField
      FieldName = 'ID_CONTAS_PAGAR'
      ProviderFlags = []
    end
    object cdsContasPagarID_CLIENTE: TIntegerField
      FieldName = 'ID_CLIENTE'
      Required = True
    end
    object cdsContasPagarID_PLANO_PAGAMENTO: TIntegerField
      FieldName = 'ID_PLANO_PAGAMENTO'
      Required = True
    end
    object cdsContasPagarID_USUARIO: TIntegerField
      FieldName = 'ID_USUARIO'
      Required = True
    end
    object cdsContasPagarID_LOJA: TIntegerField
      FieldName = 'ID_LOJA'
      Required = True
    end
    object cdsContasPagarDT_EMISSAO: TDateField
      DisplayLabel = 'Data Emiss'#227'o'
      FieldName = 'DT_EMISSAO'
      EditMask = '##/##/####'
    end
    object cdsContasPagarDT_VENCIMENTO: TDateField
      DisplayLabel = 'Data Vencimento'
      FieldName = 'DT_VENCIMENTO'
      EditMask = '##/##/####'
    end
    object cdsContasPagarDT_PAGAMENTO: TDateField
      DisplayLabel = 'Data Pagamento'
      FieldName = 'DT_PAGAMENTO'
      EditMask = '##/##/####'
    end
    object cdsContasPagarQT_DIAS: TIntegerField
      DisplayLabel = 'Qt Dias'
      FieldName = 'QT_DIAS'
    end
    object cdsContasPagarQT_DIAS_ATRAZO: TIntegerField
      DisplayLabel = 'Atrazo'
      FieldName = 'QT_DIAS_ATRAZO'
    end
    object cdsContasPagarQT_PARCELA: TIntegerField
      DisplayLabel = 'Qt Parc'
      FieldName = 'QT_PARCELA'
    end
    object cdsContasPagarNR_DOC: TStringField
      DisplayLabel = 'N'#186' Doc'
      FieldName = 'NR_DOC'
      Size = 15
    end
    object cdsContasPagarVL_COMPRA: TFMTBCDField
      DisplayLabel = 'Vl.Nota'
      FieldName = 'VL_COMPRA'
      DisplayFormat = '###,###0.000'
      EditFormat = '###,###0.000'
      Precision = 18
      Size = 2
    end
    object cdsContasPagarVL_PARCELA: TFMTBCDField
      DisplayLabel = 'Vl Parcela'
      FieldName = 'VL_PARCELA'
      DisplayFormat = '###,###0.000'
      EditFormat = '###,###0.000'
      Precision = 18
      Size = 2
    end
    object cdsContasPagarNR_PARCELA: TStringField
      DisplayLabel = 'N'#186' Parcela'
      FieldName = 'NR_PARCELA'
      Size = 10
    end
    object cdsContasPagarSTATUS: TStringField
      DisplayLabel = 'Status'
      FieldName = 'STATUS'
      FixedChar = True
      Size = 1
    end
    object cdsContasPagarDESCRICAO_PAGAMENTO: TStringField
      DisplayLabel = 'Forma Pagamento'
      FieldName = 'DESCRICAO_PAGAMENTO'
      ProviderFlags = []
      Size = 40
    end
    object cdsContasPagarCLIENTE: TStringField
      DisplayLabel = 'Cliente'
      FieldName = 'CLIENTE'
      ProviderFlags = []
      Size = 50
    end
    object cdsContasPagarFORNECEDOR: TStringField
      DisplayLabel = 'Fornecedor'
      FieldName = 'FORNECEDOR'
      ProviderFlags = []
      Size = 50
    end
    object cdsContasPagarFORN_REMESSA: TStringField
      DisplayLabel = 'Malharia'
      FieldName = 'FORN_REMESSA'
      ProviderFlags = []
      Size = 50
    end
    object cdsContasPagarNOME_USUARIO: TStringField
      DisplayLabel = 'Usuario'
      FieldName = 'NOME_USUARIO'
      ProviderFlags = []
      Size = 30
    end
    object cdsContasPagarEMPRESA: TStringField
      DisplayLabel = 'Empresa'
      FieldName = 'EMPRESA'
      ProviderFlags = []
      Size = 35
    end
    object cdsContasPagarID_FORNECEDOR: TIntegerField
      FieldName = 'ID_FORNECEDOR'
      Required = True
    end
    object cdsContasPagarID_FORN_REM: TIntegerField
      FieldName = 'ID_FORN_REM'
      Required = True
    end
    object cdsContasPagarNR_NOTA_FISCAL: TIntegerField
      FieldName = 'NR_NOTA_FISCAL'
    end
    object cdsContasPagarNR_NF_REMESSA: TIntegerField
      FieldName = 'NR_NF_REMESSA'
    end
    object cdsContasPagarORIGEM: TStringField
      FieldName = 'ORIGEM'
    end
    object cdsContasPagarID_PRODUCAO: TIntegerField
      FieldName = 'ID_PRODUCAO'
    end
  end
  object dsContasPagar: TDataSource
    DataSet = cdsContasPagar
    Left = 288
    Top = 274
  end
end
