...
procedure TfrmContasPagar.BB_ALTERARClick(Sender: TObject);
begin
  Tipo := 'Edit';
  cdsContasPagar.Edit;
  GerenciaBotoes;
...
procedure TfrmContasPagar.BB_CANCELARClick(Sender: TObject);
begin
  if Tipo = 'Insert' then
  begin
...
procedure TfrmContasPagar.BB_DELClick(Sender: TObject);
begin
...
procedure TfrmContasPagar.BB_EXCLUIRClick(Sender: TObject);
begin
...
procedure TfrmContasPagar.BB_INCLUIRClick(Sender: TObject);
begin
  Tipo := 'Insert';
...
procedure TfrmContasPagar.BB_SAIRClick(Sender: TObject);
begin
  Close;
...
procedure TfrmContasPagar.DBEdit1Exit(Sender: TObject);
begin
...
procedure TfrmContasPagar.DBEdit2Exit(Sender: TObject);
begin
...
procedure TfrmContasPagar.DBEdit3Exit(Sender: TObject);
begin
...
procedure TfrmContasPagar.DBEdit5Enter(Sender: TObject);
Var
...
procedure TfrmContasPagar.DBEdit5Exit(Sender: TObject);
begin
  ...
  begin
    ...
    if cdsContasPagar.RecordCount = 0 then
      ...
      cdsContasPagar.FieldByName('VL_COMPRA').Value       := DM.cdsProducaoVL_NOTA.AsFloat ;
...
begin
    if key = vk_f4 then
    begin
      if frmContasPagarComplementar = nil then
      begin
        frmContasPagarComplementar := TfrmContasPagarComplementar.Create(Application);
        frmContasPagarComplementar.ShowModal;
      end;
...
procedure TfrmContasPagar.GerarParcela;
var
i,dias: Byte;
Valor : Double;
ultimaData : Tdate;
...
//      DiasDoMes := DiasDoMes+dias;
    ...
    Valor := (StrToFloat(StringReplace(DM.cdsProducaoVL_TOTAL.Text,'.','',[rfReplaceAll])));
    ...
    cdsContasPagar.FieldByName('ID_USUARIO').Value          := frmMenu.recInformacoes.id_Usuario;
    cdsContasPagar.FieldByName('ID_LOJA').Value             := 1;
    cdsContasPagar.FieldByName('DT_EMISSAO').Value          := DM.cdsProducaoDT_EMISSAO.AsDateTime;
    cdsContasPagar.FieldByName('DT_VENCIMENTO').Value   := IncMonth(DM.cdsProducaoDT_EMISSAO.AsDateTime, i - 0);
 //   cdsContasPagar.FieldByName('DT_VENCIMENTO').Value   := DiasDoMes; //DM.cdsPdvDT_VENDA.AsDateTime+dias;
    cdsContasPagar.FieldByName('QT_DIAS').Value         := IncMonth(DM.cdsProducaoDT_EMISSAO.AsDateTime, i - 0) -DM.cdsProducaoDT_EMISSAO.AsDateTime;
    cdsContasPagar.FieldByName('QT_PARCELA').Value      := DM.cdsPlanoPagamentoQT_PARCELAS.AsString;
    if cdsContasPagar.RecordCount = 0 then
      cdsContasPagar.FieldByName('NR_DOC').Value          := DM.cdsProducaoNR_NF_REMESSA.AsString +'/'+IntToStr(1)+'-'+DM.cdsPlanoPagamentoQT_PARCELAS.AsString
    else
    ...
    cdsContasPagar.FieldByName('VL_PARCELA').AsFloat    := Valor /(StrToInt(DM.cdsPlanoPagamentoQT_PARCELAS.AsString));
    cdsContasPagar.FieldByName('VL_COMPRA').Value       := DM.cdsProducaoVL_NOTA.AsFloat ;
    cdsContasPagar.FieldByName('ID_CLIENTE').Value      := StrToInt(DM.cdsProducaoID_CLIENTE.AsString);
    cdsContasPagar.FieldByName('ID_PLANO_PAGAMENTO').Value  := DM.cdsPlanoPagamentoID_PLANO_PAGAMENTO.AsInteger;
    cdsContasPagar.FieldByName('STATUS').Value  := 'A';
...
procedure TfrmContasPagar.GerenciaBotoes;
begin
...
procedure TfrmContasPagar.SomaPagamentos;
begin
  DM.cdsDinamica.Close;
  DM.cdsDinamica.IndexFieldNames:='';
  DM.cdsDinamica.CommandText := ' SELECT CP.VL_COMPRA, SUM(CP.VL_PARCELA), '+
  ...
  begin
    ...
    L_SALDO.Caption := 'R$ '+FormatFloat('###,###0.00',DM.cdsDinamica.Fields[2].AsFloat);
...
function TfrmContasPagar.UltimoDia: integer;
var Date: TDateTime;
begin
...
procedure TfrmContasPagar.VerificaTotais;
begin
  DM.cdsDinamica.Close;
  DM.cdsDinamica.IndexFieldNames:='';
  DM.cdsDinamica.CommandText := ' SELECT CP.VL_COMPRA, SUM(CP.VL_PARCELA), '+
