object frmFechamento: TfrmFechamento
  Left = 0
  Top = 0
  BorderStyle = bsNone
  Caption = 'frmFechamento'
  ClientHeight = 602
  ClientWidth = 901
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  OldCreateOrder = False
  Position = poScreenCenter
  OnClose = FormClose
  OnKeyPress = FormKeyPress
  PixelsPerInch = 96
  TextHeight = 13
  object Panel1: TPanel
    Left = 0
    Top = 0
    Width = 901
    Height = 39
    Align = alTop
    Color = clBlue
    Font.Charset = ANSI_CHARSET
    Font.Color = clBlue
    Font.Height = -21
    Font.Name = 'Segoe UI Semibold'
    Font.Style = [fsBold]
    ParentBackground = False
    ParentFont = False
    TabOrder = 0
    object Label17: TLabel
      Left = 267
      Top = 2
      Width = 315
      Height = 37
      Align = alCustom
      Caption = 'Relat'#243'rio de Fechamento'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWhite
      Font.Height = -27
      Font.Name = 'Segoe UI'
      Font.Style = [fsBold, fsItalic]
      ParentFont = False
    end
    object SpeedButton3: TSpeedButton
      Left = 862
      Top = 1
      Width = 38
      Height = 37
      Align = alRight
      Flat = True
      Glyph.Data = {
        
        000000000000}
      OnClick = SpeedButton3Click
      ExplicitLeft = 1102
      ExplicitTop = 5
      ExplicitHeight = 36
    end
  end
  object Panel2: TPanel
    Left = 0
    Top = 39
    Width = 901
    Height = 90
    Align = alTop
    TabOrder = 1
    object Label4: TLabel
      Left = 8
      Top = 38
      Width = 58
      Height = 17
      Caption = 'Tinturaria'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Segoe UI Semibold'
      Font.Style = []
      ParentFont = False
    end
    object Label2: TLabel
      Left = 8
      Top = 1
      Width = 52
      Height = 13
      Caption = 'C'#243'd.Grupo'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clNavy
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
    end
    object lblCliente: TLabel
      Left = 99
      Top = 18
      Width = 41
      Height = 13
      Caption = 'CLIENTE'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clNavy
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
    end
    object Label3: TLabel
      Left = 93
      Top = 1
      Width = 65
      Height = 13
      Caption = 'Grupo Cliente'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clNavy
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
    end
    object SpeedButton1: TSpeedButton
      Left = 67
      Top = 14
      Width = 23
      Height = 22
      Glyph.Data = {
        
        DB93A7B8B7AE9999DBDBDBDBDBDBDBDBDBDBDB9D9E9ADBDBDBDB}
      OnClick = SpeedButton1Click
    end
    object SpeedButton4: TSpeedButton
      Left = 439
      Top = 56
      Width = 35
      Height = 19
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      NumGlyphs = 2
    end
    object Label5: TLabel
      Left = 488
      Top = 59
      Width = 55
      Height = 13
      Caption = 'Per'#237'odo de:'
    end
    object Label6: TLabel
      Left = 650
      Top = 59
      Width = 7
      Height = 13
      Caption = 'A'
    end
    object L_DIAS: TLabel
      Left = 712
      Top = 7
      Width = 35
      Height = 13
      Caption = 'L_DIAS'
      Visible = False
    end
    object L_CUSTO: TLabel
      Left = 712
      Top = 32
      Width = 45
      Height = 13
      Caption = 'L_CUSTO'
      Visible = False
    end
    object L_COMPRA_MATERIA: TLabel
      Left = 560
      Top = 6
      Width = 105
      Height = 13
      Caption = 'L_COMPRA_MATERIA'
      Visible = False
    end
    object L_COMPRA_MATERIA_TOTAL: TLabel
      Left = 568
      Top = 26
      Width = 105
      Height = 13
      Caption = 'L_COMPRA_MATERIA'
      Visible = False
    end
    object L_CUSTO_TOTAL: TLabel
      Left = 795
      Top = 6
      Width = 45
      Height = 13
      Caption = 'L_CUSTO'
      Visible = False
    end
    object L_SALDO: TLabel
      Left = 398
      Top = 6
      Width = 35
      Height = 13
      Caption = 'L_DIAS'
      Visible = False
    end
    object L_PERCENTUAL: TLabel
      Left = 753
      Top = 32
      Width = 45
      Height = 13
      Caption = 'L_CUSTO'
      Visible = False
    end
    object DBLookupComboBox1: TDBLookupComboBox
      Left = 8
      Top = 56
      Width = 425
      Height = 19
      Ctl3D = False
      KeyField = 'NOME'
      ListField = 'NOME'
      ListSource = dsFornecedor
      ParentCtl3D = False
      TabOrder = 1
      OnEnter = DBLookupComboBox1Enter
      OnKeyPress = FormKeyPress
    end
    object edtCli: TEdit
      Left = 8
      Top = 15
      Width = 56
      Height = 19
      Ctl3D = False
      ParentCtl3D = False
      TabOrder = 0
      OnExit = edtCliExit
      OnKeyPress = edtCliKeyPress
    end
    object BB_LOCALIZAR: TBitBtn
      Left = 813
      Top = 28
      Width = 80
      Height = 25
      Caption = '&Localizar'
      Glyph.Data = {
        
        DB93A7B8B7AE9999DBDBDBDBDBDBDBDBDBDBDB9D9E9ADBDBDBDB}
      TabOrder = 4
      OnClick = BB_LOCALIZARClick
    end
    object BB_IMPRIMIR: TBitBtn
      Left = 813
      Top = 59
      Width = 80
      Height = 25
      Caption = '&Imprimir'
      Enabled = False
      Glyph.Data = {
        
        261E1A123B3B3B3B3B3B3B3B3B1111111112123B3B3B3B3B3B3B}
      TabOrder = 5
      OnClick = BB_IMPRIMIRClick
    end
    object ME_INI: TMaskEdit
      Left = 552
      Top = 56
      Width = 92
      Height = 19
      Ctl3D = False
      EditMask = '!99/99/0000;1;_'
      MaxLength = 10
      ParentCtl3D = False
      TabOrder = 2
      Text = '  /  /    '
      OnKeyPress = FormKeyPress
    end
    object ME_FIM: TMaskEdit
      Left = 663
      Top = 56
      Width = 93
      Height = 19
      Ctl3D = False
      EditMask = '!99/99/0000;1;_'
      MaxLength = 10
      ParentCtl3D = False
      TabOrder = 3
      Text = '  /  /    '
      OnKeyPress = FormKeyPress
    end
  end
  object DBGrid1: TDBGrid
    Left = 0
    Top = 251
    Width = 901
    Height = 100
    Align = alTop
    DataSource = dsServicos
    Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit, dgTitleClick, dgTitleHotTrack]
    ReadOnly = True
    TabOrder = 2
    TitleFont.Charset = DEFAULT_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -11
    TitleFont.Name = 'Tahoma'
    TitleFont.Style = []
    Columns = <
      item
        Expanded = False
        FieldName = 'QUANT'
        Width = 77
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'DESCRICAO'
        Width = 359
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'VALOR'
        Width = 83
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'TOTAL'
        Width = 83
        Visible = True
      end>
  end
  object DBGrid2: TDBGrid
    Left = 0
    Top = 129
    Width = 901
    Height = 96
    Align = alTop
    DataSource = dsNotas
    Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit, dgTitleClick, dgTitleHotTrack]
    ReadOnly = True
    TabOrder = 3
    TitleFont.Charset = DEFAULT_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -11
    TitleFont.Name = 'Tahoma'
    TitleFont.Style = []
    Columns = <
      item
        Expanded = False
        FieldName = 'QUANT_FATURADO'
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'VL_UNIT'
        Width = 74
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'VL_TOTAL'
        Width = 78
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'CLIENTE'
        Width = 238
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'FORNECEDOR'
        Width = 250
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'NR_NOTA_FISCAL'
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'DT_EMISSAO'
        Visible = True
      end>
  end
  object Panel3: TPanel
    Left = 0
    Top = 225
    Width = 901
    Height = 26
    Align = alTop
    Color = clBlue
    Font.Charset = ANSI_CHARSET
    Font.Color = clBlue
    Font.Height = -21
    Font.Name = 'Segoe UI Semibold'
    Font.Style = [fsBold]
    ParentBackground = False
    ParentFont = False
    TabOrder = 4
    object Label1: TLabel
      Left = 392
      Top = 0
      Width = 51
      Height = 23
      Align = alCustom
      Caption = 'Custos'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWhite
      Font.Height = -17
      Font.Name = 'Segoe UI'
      Font.Style = [fsBold, fsItalic]
      ParentFont = False
    end
  end
  object DBGrid3: TDBGrid
    Left = 0
    Top = 377
    Width = 901
    Height = 94
    Align = alTop
    DataSource = dsFaturamento
    Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit, dgTitleClick, dgTitleHotTrack]
    ReadOnly = True
    TabOrder = 5
    TitleFont.Charset = DEFAULT_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -11
    TitleFont.Name = 'Tahoma'
    TitleFont.Style = []
    Columns = <
      item
        Expanded = False
        FieldName = 'MyColumn1'
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'FORNECEDOR'
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'NR_NOTA_FISCAL'
        Width = 88
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'DT_EMISSAO'
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'VL_NOTA'
        Width = 97
        Visible = True
      end>
  end
  object Panel4: TPanel
    Left = 0
    Top = 351
    Width = 901
    Height = 26
    Align = alTop
    Color = clBlue
    Font.Charset = ANSI_CHARSET
    Font.Color = clBlue
    Font.Height = -21
    Font.Name = 'Segoe UI Semibold'
    Font.Style = [fsBold]
    ParentBackground = False
    ParentFont = False
    TabOrder = 6
    object Label7: TLabel
      Left = 336
      Top = 2
      Width = 201
      Height = 23
      Align = alCustom
      Caption = 'Rela'#231#227'o de Faturamentos'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWhite
      Font.Height = -17
      Font.Name = 'Segoe UI'
      Font.Style = [fsBold, fsItalic]
      ParentFont = False
    end
  end
  object Panel5: TPanel
    Left = 0
    Top = 471
    Width = 901
    Height = 26
    Align = alTop
    Color = clBlue
    Font.Charset = ANSI_CHARSET
    Font.Color = clBlue
    Font.Height = -21
    Font.Name = 'Segoe UI Semibold'
    Font.Style = [fsBold]
    ParentBackground = False
    ParentFont = False
    TabOrder = 7
    object Label8: TLabel
      Left = 336
      Top = 2
      Width = 135
      Height = 23
      Align = alCustom
      Caption = 'Notas de Quebra'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWhite
      Font.Height = -17
      Font.Name = 'Segoe UI'
      Font.Style = [fsBold, fsItalic]
      ParentFont = False
    end
  end
  object DBGrid4: TDBGrid
    Left = 0
    Top = 497
    Width = 901
    Height = 104
    Align = alTop
    DataSource = dsNotaQuebra
    Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit, dgTitleClick, dgTitleHotTrack]
    ReadOnly = True
    TabOrder = 8
    TitleFont.Charset = DEFAULT_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -11
    TitleFont.Name = 'Tahoma'
    TitleFont.Style = []
    Columns = <
      item
        Expanded = False
        FieldName = 'MALHARIA'
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'NR_NOTA_QUEBRA'
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'DT_EMISSAO'
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'VALOR'
        Visible = True
      end>
  end
  object PageControl1: TPageControl
    Left = 288
    Top = 199
    Width = 776
    Height = 529
    ActivePage = TabSheet1
    TabOrder = 9
    Visible = False
    object TabSheet1: TTabSheet
      Caption = 'TabSheet1'
      object QR_FECHAMENTO: TRLReport
        Left = 20
        Top = 13
        Width = 794
        Height = 1123
        Borders.Sides = sdCustom
        Borders.DrawLeft = False
        Borders.DrawTop = False
        Borders.DrawRight = False
        Borders.DrawBottom = False
        DataSource = dsNotas
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = []
        Title = 'Relat'#243'rio de Fechamento'
        object RLBand1: TRLBand
          Left = 38
          Top = 38
          Width = 718
          Height = 91
          BandType = btHeader
          Borders.Sides = sdCustom
          Borders.DrawLeft = False
          Borders.DrawTop = True
          Borders.DrawRight = False
          Borders.DrawBottom = True
          Borders.FixedBottom = True
          object RLLabel1: TRLLabel
            Left = 169
            Top = 61
            Width = 392
            Height = 24
            Caption = 'Composi'#231#227'o de Custo de Malha Acabada'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clBlack
            Font.Height = -20
            Font.Name = 'Arial'
            Font.Style = [fsBold]
            ParentFont = False
          end
          object RLSystemInfo2: TRLSystemInfo
            Left = 252
            Top = 4
            Width = 50
            Height = 13
            Info = itFullDate
            Text = ''
          end
        end
        object RLBand2: TRLBand
          Left = 38
          Top = 129
          Width = 718
          Height = 26
          Color = 14211288
          ParentColor = False
          Transparent = False
          object RLLabel2: TRLLabel
            Left = 13
            Top = 4
            Width = 43
            Height = 13
            Caption = 'Cliente :'
            Transparent = False
          end
          object RLDBText1: TRLDBText
            Left = 80
            Top = 4
            Width = 49
            Height = 16
            DataField = 'CLIENTE'
            DataSource = dsNotas
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clBlack
            Font.Height = -13
            Font.Name = 'Arial'
            Font.Style = [fsBold]
            ParentFont = False
            Text = ''
            Transparent = False
          end
          object RLLabel3: TRLLabel
            Left = 554
            Top = 4
            Width = 64
            Height = 13
            Caption = 'Quant. Total'
            Transparent = False
          end
          object RLDBText2: TRLDBText
            Left = 636
            Top = 4
            Width = 46
            Height = 16
            DataField = 'QUANT_FATURADO'
            DataSource = dsNotas
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clBlack
            Font.Height = -13
            Font.Name = 'Arial'
            Font.Style = [fsBold]
            ParentFont = False
            Text = ''
            Transparent = False
          end
        end
        object RLSubDetail1: TRLSubDetail
          Left = 38
          Top = 155
          Width = 718
          Height = 96
          DataSource = dsNotas
          object RLBand3: TRLBand
            Left = 0
            Top = 0
            Width = 718
            Height = 19
            Borders.Sides = sdCustom
            Borders.DrawLeft = False
            Borders.DrawTop = True
            Borders.DrawRight = False
            Borders.DrawBottom = True
            Borders.FixedBottom = True
            Color = 13750737
            ParentColor = False
            Transparent = False
            object RLLabel4: TRLLabel
              Left = 8
              Top = 1
              Width = 55
              Height = 13
              Caption = 'Nota Fiscal'
              Transparent = False
            end
            object RLLabel5: TRLLabel
              Left = 362
              Top = 1
              Width = 21
              Height = 13
              Caption = 'FIO'
              Transparent = False
            end
            object RLLabel6: TRLLabel
              Left = 466
              Top = 1
              Width = 59
              Height = 13
              Caption = 'Quantidade'
              Transparent = False
            end
            object RLLabel7: TRLLabel
              Left = 585
              Top = 1
              Width = 30
              Height = 13
              Caption = 'Pre'#231'o'
              Transparent = False
            end
            object RLLabel36: TRLLabel
              Left = 139
              Top = 1
              Width = 58
              Height = 13
              Caption = 'Fornecedor'
              Transparent = False
            end
          end
          object RLBand4: TRLBand
            Left = 0
            Top = 19
            Width = 718
            Height = 19
            object RLDBText3: TRLDBText
              Left = 8
              Top = 0
              Width = 92
              Height = 13
              DataField = 'NR_NOTA_FISCAL'
              DataSource = dsNotas
              Text = ''
            end
            object RLDBText4: TRLDBText
              Left = 329
              Top = 0
              Width = 84
              Height = 13
              Alignment = taCenter
              DataField = 'NOME_MATERIA'
              DataSource = dsNotas
              Text = ''
            end
            object RLDBText5: TRLDBText
              Left = 427
              Top = 0
              Width = 99
              Height = 13
              Alignment = taRightJustify
              DataField = 'QUANT_FATURADO'
              DataSource = dsNotas
              Text = ''
            end
            object RLDBText6: TRLDBText
              Left = 563
              Top = 0
              Width = 63
              Height = 13
              Alignment = taRightJustify
              DataField = 'VL_UNIT'
              DataSource = dsNotas
              Text = 'R$  '
            end
            object RLDBText8: TRLDBText
              Left = 102
              Top = 0
              Width = 208
              Height = 16
              AutoSize = False
              DataField = 'FORNECEDOR'
              DataSource = dsNotas
              Text = ''
              Transparent = False
            end
          end
          object RLBand5: TRLBand
            Left = 0
            Top = 38
            Width = 718
            Height = 27
            Color = 11053224
            ParentColor = False
            Transparent = False
            object RLDBText7: TRLDBText
              Left = 92
              Top = 8
              Width = 71
              Height = 13
              DataField = 'VL_TOTAL'
              DataSource = dsNotas
              Text = 'R$  '
              Transparent = False
            end
            object RLLabel8: TRLLabel
              Left = 8
              Top = 8
              Width = 74
              Height = 16
              Caption = 'Valor total:'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clBlack
              Font.Height = -13
              Font.Name = 'Arial'
              Font.Style = [fsBold]
              ParentFont = False
              Transparent = False
            end
          end
          object RLBand6: TRLBand
            Left = 0
            Top = 65
            Width = 718
            Height = 27
            Color = 14671839
            ParentColor = False
            Transparent = False
            object RLLabel9: TRLLabel
              Left = 8
              Top = 6
              Width = 98
              Height = 16
              Caption = 'Compra do Fio'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clBlack
              Font.Height = -13
              Font.Name = 'Arial'
              Font.Style = [fsBold]
              ParentFont = False
              Transparent = False
            end
            object RLDBText9: TRLDBText
              Left = 119
              Top = 6
              Width = 67
              Height = 13
              DataField = 'DT_EMISSAO'
              DataSource = dsNotas
              Text = ''
              Transparent = False
            end
            object RLLabel10: TRLLabel
              Left = 257
              Top = 6
              Width = 88
              Height = 16
              Caption = 'Recebimento'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clBlack
              Font.Height = -13
              Font.Name = 'Arial'
              Font.Style = [fsBold]
              ParentFont = False
              Transparent = False
            end
            object RLDBText10: TRLDBText
              Left = 353
              Top = 6
              Width = 67
              Height = 13
              DataField = 'DT_EMISSAO'
              DataSource = dsNotaQuebra
              Text = ''
              Transparent = False
            end
            object RLLabel11: TRLLabel
              Left = 465
              Top = 6
              Width = 72
              Height = 16
              Caption = 'Lead Time'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clBlack
              Font.Height = -13
              Font.Name = 'Arial'
              Font.Style = [fsBold]
              ParentFont = False
              Transparent = False
            end
            object RLL_dias: TRLLabel
              Left = 541
              Top = 6
              Width = 72
              Height = 16
              Caption = 'Lead Time'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clBlack
              Font.Height = -13
              Font.Name = 'Arial'
              Font.Style = [fsBold]
              ParentFont = False
              Transparent = False
            end
          end
        end
        object RLSubDetail2: TRLSubDetail
          Left = 38
          Top = 251
          Width = 718
          Height = 228
          DataSource = dsServicos
          object RLBand7: TRLBand
            Left = 0
            Top = 0
            Width = 718
            Height = 25
            BandType = btHeader
            Borders.Sides = sdCustom
            Borders.DrawLeft = False
            Borders.DrawTop = False
            Borders.DrawRight = False
            Borders.DrawBottom = True
            Borders.FixedBottom = True
            object RLLabel12: TRLLabel
              Left = 312
              Top = 3
              Width = 59
              Height = 18
              Caption = 'Custos'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clBlack
              Font.Height = -16
              Font.Name = 'Arial'
              Font.Style = [fsBold, fsItalic]
              ParentFont = False
            end
          end
          object RLBand8: TRLBand
            Left = 0
            Top = 25
            Width = 718
            Height = 19
            BandType = btHeader
            Borders.Sides = sdCustom
            Borders.DrawLeft = False
            Borders.DrawTop = True
            Borders.DrawRight = False
            Borders.DrawBottom = True
            Borders.FixedBottom = True
            Color = 13750737
            ParentColor = False
            Transparent = False
            object RLLabel13: TRLLabel
              Left = 23
              Top = 1
              Width = 78
              Height = 16
              Caption = 'Quantidade'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clBlack
              Font.Height = -13
              Font.Name = 'Arial'
              Font.Style = [fsBold]
              ParentFont = False
              Transparent = False
            end
            object RLLabel14: TRLLabel
              Left = 225
              Top = 1
              Width = 58
              Height = 16
              Caption = 'Servi'#231'os'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clBlack
              Font.Height = -13
              Font.Name = 'Arial'
              Font.Style = [fsBold]
              ParentFont = False
              Transparent = False
            end
            object RLLabel15: TRLLabel
              Left = 415
              Top = 1
              Width = 54
              Height = 16
              Caption = 'Unit'#225'rio'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clBlack
              Font.Height = -13
              Font.Name = 'Arial'
              Font.Style = [fsBold]
              ParentFont = False
              Transparent = False
            end
            object RLLabel16: TRLLabel
              Left = 585
              Top = 1
              Width = 36
              Height = 16
              Caption = 'Total'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clBlack
              Font.Height = -13
              Font.Name = 'Arial'
              Font.Style = [fsBold]
              ParentFont = False
              Transparent = False
            end
          end
          object RLBand9: TRLBand
            Left = 0
            Top = 44
            Width = 718
            Height = 18
            BeforePrint = RLBand9BeforePrint
            object RLDBText11: TRLDBText
              Left = 32
              Top = 1
              Width = 38
              Height = 13
              DataField = 'QUANT'
              DataSource = dsServicos
              Text = ''
            end
            object RLDBText12: TRLDBText
              Left = 204
              Top = 1
              Width = 62
              Height = 13
              DataField = 'DESCRICAO'
              DataSource = dsServicos
              Text = ''
            end
            object RLDBText13: TRLDBText
              Left = 422
              Top = 1
              Width = 36
              Height = 13
              Alignment = taRightJustify
              DataField = 'VALOR'
              DataSource = dsServicos
              Text = ''
            end
            object RLDBText14: TRLDBText
              Left = 592
              Top = 1
              Width = 35
              Height = 13
              Alignment = taRightJustify
              DataField = 'TOTAL'
              DataSource = dsServicos
              Text = ''
            end
            object RLLabel23: TRLLabel
              Left = 391
              Top = 1
              Width = 16
              Height = 13
              Caption = 'R$'
            end
            object RLLabel35: TRLLabel
              Left = 546
              Top = 1
              Width = 16
              Height = 13
              Caption = 'R$'
            end
          end
          object RLBand10: TRLBand
            Left = 0
            Top = 62
            Width = 718
            Height = 21
            BandType = btSummary
            Borders.Sides = sdCustom
            Borders.DrawLeft = False
            Borders.DrawTop = True
            Borders.DrawRight = False
            Borders.DrawBottom = False
            object RLLabel17: TRLLabel
              Left = 204
              Top = 2
              Width = 111
              Height = 16
              Caption = 'Total de Servi'#231'os'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clBlack
              Font.Height = -13
              Font.Name = 'Arial'
              Font.Style = [fsBold, fsItalic]
              ParentFont = False
            end
            object RLL_CUSTO: TRLLabel
              Left = 375
              Top = 3
              Width = 81
              Height = 16
              Alignment = taRightJustify
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clBlack
              Font.Height = -13
              Font.Name = 'Arial'
              Font.Style = [fsBold]
              ParentFont = False
            end
            object RLL_CUSTO_TOTAL: TRLLabel
              Left = 578
              Top = 3
              Width = 49
              Height = 16
              Alignment = taRightJustify
              Caption = 'R$ 0.00'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clBlack
              Font.Height = -13
              Font.Name = 'Arial'
              Font.Style = [fsBold]
              ParentFont = False
            end
          end
          object RLBand11: TRLBand
            Left = 0
            Top = 83
            Width = 718
            Height = 22
            BandType = btSummary
            Color = 14342874
            ParentColor = False
            Transparent = False
            object RLLabel18: TRLLabel
              Left = 183
              Top = 3
              Width = 162
              Height = 16
              Caption = 'Compra de Mat'#233'ria Prima'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clBlack
              Font.Height = -13
              Font.Name = 'Arial'
              Font.Style = [fsBold, fsItalic]
              ParentFont = False
              Transparent = False
            end
            object RLL_COMPRA_MATERIA: TRLLabel
              Left = 351
              Top = 3
              Width = 105
              Height = 16
              Alignment = taRightJustify
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clBlack
              Font.Height = -13
              Font.Name = 'Arial'
              Font.Style = [fsBold]
              ParentFont = False
              Transparent = False
            end
            object RLL_COMPRA_MATERIA_TOTAL: TRLLabel
              Left = 578
              Top = 3
              Width = 49
              Height = 16
              Alignment = taRightJustify
              Caption = 'R$ 0.00'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clBlack
              Font.Height = -13
              Font.Name = 'Arial'
              Font.Style = [fsBold]
              ParentFont = False
              Transparent = False
            end
          end
          object RLBand12: TRLBand
            Left = 0
            Top = 105
            Width = 718
            Height = 43
            BandType = btSummary
            object RLL_SALDO: TRLLabel
              Left = 15
              Top = 3
              Width = 49
              Height = 16
              Caption = 'R$ 0.00'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clRed
              Font.Height = -13
              Font.Name = 'Arial'
              Font.Style = [fsBold]
              ParentFont = False
            end
            object RLL_PERCENTUAL: TRLLabel
              Left = 649
              Top = 3
              Width = 25
              Height = 16
              Caption = '0 %'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clRed
              Font.Height = -13
              Font.Name = 'Arial'
              Font.Style = [fsBold]
              ParentFont = False
            end
            object RLLabel19: TRLLabel
              Left = 179
              Top = 3
              Width = 193
              Height = 16
              Caption = 'Quebra no Processo malharia'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clRed
              Font.Height = -13
              Font.Name = 'Arial'
              Font.Style = [fsBold]
              ParentFont = False
            end
            object RLL_TOTAL_CUSTO_QUERA: TRLLabel
              Left = 578
              Top = 3
              Width = 49
              Height = 16
              Alignment = taRightJustify
              Caption = 'R$ 0.00'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clRed
              Font.Height = -13
              Font.Name = 'Arial'
              Font.Style = [fsBold]
              ParentFont = False
            end
            object RLLabel20: TRLLabel
              Left = 169
              Top = 24
              Width = 207
              Height = 16
              Caption = 'Quebra no Processo estamparia'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clRed
              Font.Height = -13
              Font.Name = 'Arial'
              Font.Style = [fsBold]
              ParentFont = False
            end
            object RLL_QUEBRA: TRLLabel
              Left = 15
              Top = 24
              Width = 49
              Height = 16
              Caption = 'R$ 0.00'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clRed
              Font.Height = -13
              Font.Name = 'Arial'
              Font.Style = [fsBold]
              ParentFont = False
            end
            object RLL_VALOR: TRLLabel
              Left = 578
              Top = 24
              Width = 49
              Height = 16
              Alignment = taRightJustify
              Caption = 'R$ 0.00'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clRed
              Font.Height = -13
              Font.Name = 'Arial'
              Font.Style = [fsBold]
              ParentFont = False
            end
            object RLL_PERCENTUAL_E: TRLLabel
              Left = 649
              Top = 24
              Width = 25
              Height = 16
              Caption = '0 %'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clRed
              Font.Height = -13
              Font.Name = 'Arial'
              Font.Style = [fsBold]
              ParentFont = False
            end
          end
          object RLBand13: TRLBand
            Left = 0
            Top = 148
            Width = 718
            Height = 23
            BandType = btSummary
            Color = 12320767
            ParentColor = False
            Transparent = False
            BeforePrint = RLBand13BeforePrint
            object RLLabel21: TRLLabel
              Left = 142
              Top = 2
              Width = 229
              Height = 18
              Caption = 'Custo total da Malha Acabada'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clBlack
              Font.Height = -16
              Font.Name = 'Arial'
              Font.Style = [fsBold, fsItalic]
              ParentFont = False
              Transparent = False
            end
            object RLL_CUSTO_TOTAL_MALAHA_ACABADA: TRLLabel
              Left = 400
              Top = 4
              Width = 49
              Height = 16
              Alignment = taRightJustify
              Caption = 'R$ 0.00'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clBlack
              Font.Height = -13
              Font.Name = 'Arial'
              Font.Style = [fsBold]
              ParentFont = False
              Transparent = False
            end
            object RLL_SOMA_CUSTO_TOTAL: TRLLabel
              Left = 577
              Top = 4
              Width = 49
              Height = 16
              Alignment = taRightJustify
              Caption = 'R$ 0.00'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clBlack
              Font.Height = -13
              Font.Name = 'Arial'
              Font.Style = [fsBold]
              ParentFont = False
              Transparent = False
            end
          end
          object RLBand14: TRLBand
            Left = 0
            Top = 171
            Width = 718
            Height = 51
            BandType = btSummary
            object RLLabel24: TRLLabel
              Left = 204
              Top = 27
              Width = 183
              Height = 17
              Caption = 'Rela'#231#227'o de Faturamentos'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clBlack
              Font.Height = -15
              Font.Name = 'Arial'
              Font.Style = [fsBold, fsItalic]
              ParentFont = False
            end
          end
        end
        object RLSubDetail3: TRLSubDetail
          Left = 38
          Top = 479
          Width = 718
          Height = 44
          DataSource = dsFaturamento
          object RLBand15: TRLBand
            Left = 0
            Top = 0
            Width = 718
            Height = 21
            BandType = btHeader
            Color = 13882323
            ParentColor = False
            Transparent = False
            object RLLabel25: TRLLabel
              Left = 113
              Top = 2
              Width = 77
              Height = 16
              Caption = 'Fornecedor'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clBlack
              Font.Height = -13
              Font.Name = 'Arial'
              Font.Style = [fsBold]
              ParentFont = False
              Transparent = False
            end
            object RLLabel26: TRLLabel
              Left = 255
              Top = 2
              Width = 74
              Height = 16
              Caption = 'Nota Fiscal'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clBlack
              Font.Height = -13
              Font.Name = 'Arial'
              Font.Style = [fsBold]
              ParentFont = False
              Transparent = False
            end
            object RLLabel27: TRLLabel
              Left = 382
              Top = 2
              Width = 56
              Height = 16
              Caption = 'Emiss'#227'o'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clBlack
              Font.Height = -13
              Font.Name = 'Arial'
              Font.Style = [fsBold]
              ParentFont = False
              Transparent = False
            end
            object RLLabel28: TRLLabel
              Left = 488
              Top = 2
              Width = 59
              Height = 16
              Caption = 'Valor NF'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clBlack
              Font.Height = -13
              Font.Name = 'Arial'
              Font.Style = [fsBold]
              ParentFont = False
              Transparent = False
            end
          end
          object RLBand16: TRLBand
            Left = 0
            Top = 21
            Width = 718
            Height = 18
            object RLDBText15: TRLDBText
              Left = 113
              Top = 1
              Width = 77
              Height = 16
              AutoSize = False
              DataField = 'FORNECEDOR'
              DataSource = dsFaturamento
              Text = ''
            end
            object RLDBText16: TRLDBText
              Left = 499
              Top = 1
              Width = 48
              Height = 13
              Alignment = taRightJustify
              DataField = 'VL_NOTA'
              DataSource = dsFaturamento
              Text = ''
            end
            object RLDBText17: TRLDBText
              Left = 3
              Top = 1
              Width = 58
              Height = 13
              DataField = 'MyColumn1'
              DataSource = dsFaturamento
              Text = ''
            end
            object RLDBText18: TRLDBText
              Left = 257
              Top = 1
              Width = 92
              Height = 13
              DataField = 'NR_NOTA_FISCAL'
              DataSource = dsFaturamento
              Text = ''
            end
            object RLDBText19: TRLDBText
              Left = 370
              Top = 1
              Width = 67
              Height = 13
              DataField = 'DT_EMISSAO'
              DataSource = dsFaturamento
              Text = ''
            end
            object RLLabel22: TRLLabel
              Left = 458
              Top = 1
              Width = 16
              Height = 13
              Caption = 'R$'
            end
          end
        end
        object RLSubDetail4: TRLSubDetail
          Left = 38
          Top = 523
          Width = 718
          Height = 98
          DataSource = dsNotaQuebra
          object RLBand17: TRLBand
            Left = 0
            Top = 0
            Width = 718
            Height = 44
            BandType = btHeader
            object RLLabel30: TRLLabel
              Left = 256
              Top = 19
              Width = 106
              Height = 16
              Caption = 'Notas de Quebra'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clBlack
              Font.Height = -13
              Font.Name = 'Arial'
              Font.Style = [fsBold, fsItalic]
              ParentFont = False
            end
          end
          object RLBand18: TRLBand
            Left = 0
            Top = 44
            Width = 718
            Height = 21
            BandType = btHeader
            Color = 13882323
            ParentColor = False
            Transparent = False
            object RLLabel31: TRLLabel
              Left = 113
              Top = 2
              Width = 77
              Height = 16
              Caption = 'Fornecedor'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clBlack
              Font.Height = -13
              Font.Name = 'Arial'
              Font.Style = [fsBold]
              ParentFont = False
              Transparent = False
            end
            object RLLabel32: TRLLabel
              Left = 255
              Top = 2
              Width = 74
              Height = 16
              Caption = 'Nota Fiscal'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clBlack
              Font.Height = -13
              Font.Name = 'Arial'
              Font.Style = [fsBold]
              ParentFont = False
              Transparent = False
            end
            object RLLabel33: TRLLabel
              Left = 382
              Top = 2
              Width = 56
              Height = 16
              Caption = 'Emiss'#227'o'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clBlack
              Font.Height = -13
              Font.Name = 'Arial'
              Font.Style = [fsBold]
              ParentFont = False
              Transparent = False
            end
            object RLLabel34: TRLLabel
              Left = 488
              Top = 2
              Width = 59
              Height = 16
              Caption = 'Valor NF'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clBlack
              Font.Height = -13
              Font.Name = 'Arial'
              Font.Style = [fsBold]
              ParentFont = False
              Transparent = False
            end
          end
          object RLBand19: TRLBand
            Left = 0
            Top = 65
            Width = 718
            Height = 18
            object RLDBText20: TRLDBText
              Left = 113
              Top = 1
              Width = 77
              Height = 16
              AutoSize = False
              DataField = 'TITURARIA'
              DataSource = dsNotaQuebra
              Text = ''
            end
            object RLDBText21: TRLDBText
              Left = 492
              Top = 1
              Width = 55
              Height = 13
              Alignment = taRightJustify
              DataField = 'VALOR'
              DataSource = dsNotaQuebra
              Text = 'R$  '
            end
            object RLDBText23: TRLDBText
              Left = 257
              Top = 1
              Width = 98
              Height = 13
              DataField = 'NR_NOTA_QUEBRA'
              DataSource = dsNotaQuebra
              Text = ''
            end
            object RLDBText24: TRLDBText
              Left = 370
              Top = 1
              Width = 67
              Height = 13
              DataField = 'DT_EMISSAO'
              DataSource = dsNotaQuebra
              Text = ''
            end
          end
        end
        object RLBand20: TRLBand
          Left = 38
          Top = 621
          Width = 718
          Height = 44
          BandType = btFooter
          Borders.Sides = sdCustom
          Borders.DrawLeft = False
          Borders.DrawTop = True
          Borders.DrawRight = False
          Borders.DrawBottom = False
          object RLLabel37: TRLLabel
            Left = 8
            Top = 3
            Width = 196
            Height = 13
            Caption = 'GeProt - Gerenciador de Produ'#231#227'o T'#234'xtil'
          end
          object RLLabel38: TRLLabel
            Left = 299
            Top = 3
            Width = 114
            Height = 13
            Caption = ' Por Prompt Sistemas'#174
          end
          object RLSystemInfo1: TRLSystemInfo
            Left = 687
            Top = 3
            Width = 72
            Height = 13
            Info = itPageNumber
            Text = ''
          end
          object RLLabel39: TRLLabel
            Left = 655
            Top = 3
            Width = 25
            Height = 13
            Caption = 'Pag.'
          end
        end
      end
    end
  end
  object dsFornecedor: TDataSource
    DataSet = cdsFornecedor
    Left = 296
    Top = 42
  end
  object cdsFornecedor: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'dspFornecedor'
    Left = 264
    Top = 42
    object cdsFornecedorID_FORNECEDOR: TIntegerField
      FieldName = 'ID_FORNECEDOR'
      Required = True
    end
    object cdsFornecedorCODIGO_MUNIC: TStringField
      FieldName = 'CODIGO_MUNIC'
      Required = True
      Size = 7
    end
    object cdsFornecedorDT_CADASTRO: TDateField
      FieldName = 'DT_CADASTRO'
    end
    object cdsFornecedorNOME: TStringField
      FieldName = 'NOME'
      Size = 50
    end
    object cdsFornecedorTP_LOGRADOURO: TStringField
      FieldName = 'TP_LOGRADOURO'
      Size = 30
    end
    object cdsFornecedorENDERECO: TStringField
      FieldName = 'ENDERECO'
      Size = 50
    end
    object cdsFornecedorCOMPLEMENTO: TStringField
      FieldName = 'COMPLEMENTO'
      Size = 50
    end
    object cdsFornecedorNUMERO: TStringField
      FieldName = 'NUMERO'
      FixedChar = True
      Size = 18
    end
    object cdsFornecedorBAIRRO: TStringField
      FieldName = 'BAIRRO'
      Size = 40
    end
    object cdsFornecedorCEP: TStringField
      FieldName = 'CEP'
      Size = 10
    end
    object cdsFornecedorTELFONE: TStringField
      FieldName = 'TELFONE'
      Size = 14
    end
    object cdsFornecedorFAX: TStringField
      FieldName = 'FAX'
      Size = 14
    end
    object cdsFornecedorCELULAR: TStringField
      FieldName = 'CELULAR'
      Size = 14
    end
    object cdsFornecedorEMAIL: TStringField
      FieldName = 'EMAIL'
      Size = 60
    end
    object cdsFornecedorCNPJ: TStringField
      FieldName = 'CNPJ'
      Size = 18
    end
    object cdsFornecedorINS_EST: TStringField
      FieldName = 'INS_EST'
      Size = 14
    end
    object cdsFornecedorCONTATO: TStringField
      FieldName = 'CONTATO'
      Size = 30
    end
    object cdsFornecedorSITE: TStringField
      FieldName = 'SITE'
      Size = 70
    end
    object cdsFornecedorFANTASIA: TStringField
      FieldName = 'FANTASIA'
      Size = 30
    end
    object cdsFornecedorSITUACAO: TStringField
      FieldName = 'SITUACAO'
      Size = 30
    end
    object cdsFornecedorDT_ABERTURA: TDateField
      FieldName = 'DT_ABERTURA'
    end
    object cdsFornecedorCNAE_PRINCIPAL: TStringField
      FieldName = 'CNAE_PRINCIPAL'
      Size = 200
    end
    object cdsFornecedorCNAE_SECUNDARIO: TStringField
      FieldName = 'CNAE_SECUNDARIO'
      Size = 999
    end
    object cdsFornecedorID_CATEGORIA: TIntegerField
      FieldName = 'ID_CATEGORIA'
    end
    object cdsFornecedorCIDADE: TStringField
      FieldName = 'CIDADE'
      Size = 35
    end
    object cdsFornecedorUF: TStringField
      FieldName = 'UF'
      Size = 2
    end
    object cdsFornecedorDDD: TStringField
      FieldName = 'DDD'
      Size = 2
    end
    object cdsFornecedorNOME_CATEGORIA: TStringField
      FieldName = 'NOME_CATEGORIA'
      Size = 40
    end
  end
  object dspFornecedor: TDataSetProvider
    DataSet = qFornecedor
    Options = [poIncFieldProps, poAllowMultiRecordUpdates, poAutoRefresh, poAllowCommandText, poUseQuoteChar]
    Left = 232
    Top = 42
  end
  object qFornecedor: TSQLQuery
    MaxBlobSize = -1
    Params = <>
    SQL.Strings = (
      'SELECT'
      ' F.ID_FORNECEDOR,'
      ' F.CODIGO_MUNIC,'
      ' F.DT_CADASTRO,'
      ' F.NOME,'
      ' F.TP_LOGRADOURO,'
      ' F.ENDERECO,'
      ' F.COMPLEMENTO,'
      ' F.NUMERO,'
      ' F.BAIRRO,'
      ' F.CEP,'
      ' F.TELFONE,'
      ' F.FAX,'
      ' F.CELULAR,'
      ' F.EMAIL,'
      ' F.CNPJ,'
      ' F.INS_EST,'
      ' F.CONTATO,'
      ' F.SITE,'
      ' F.FANTASIA,'
      ' F.SITUACAO,'
      ' F.DT_ABERTURA,'
      ' F.CNAE_PRINCIPAL,'
      ' F.CNAE_SECUNDARIO,'
      ' F.ID_CATEGORIA,'
      ' M.NOME AS CIDADE,'
      ' M.UF AS UF,'
      ' M.DDD,'
      ' C.NOME_CATEGORIA'
      'FROM TB_FORNECEDOR F'
      'JOIN TB_MUNICIPIOS M ON (M.CODIGO_MUNIC = F.CODIGO_MUNIC)'
      'LEFT JOIN TB_CATEGORIA  C ON (C.ID_CATEGORIA = F.ID_CATEGORIA)'
      'WHERE F.ID_CATEGORIA = 2')
    SQLConnection = DM.conexao
    Left = 200
    Top = 42
    object qFornecedorID_FORNECEDOR: TIntegerField
      FieldName = 'ID_FORNECEDOR'
      Required = True
    end
    object qFornecedorCODIGO_MUNIC: TStringField
      FieldName = 'CODIGO_MUNIC'
      Required = True
      Size = 7
    end
    object qFornecedorDT_CADASTRO: TDateField
      FieldName = 'DT_CADASTRO'
    end
    object qFornecedorNOME: TStringField
      FieldName = 'NOME'
      Size = 50
    end
    object qFornecedorTP_LOGRADOURO: TStringField
      FieldName = 'TP_LOGRADOURO'
      Size = 30
    end
    object qFornecedorENDERECO: TStringField
      FieldName = 'ENDERECO'
      Size = 50
    end
    object qFornecedorCOMPLEMENTO: TStringField
      FieldName = 'COMPLEMENTO'
      Size = 50
    end
    object qFornecedorNUMERO: TStringField
      FieldName = 'NUMERO'
      FixedChar = True
      Size = 18
    end
    object qFornecedorBAIRRO: TStringField
      FieldName = 'BAIRRO'
      Size = 40
    end
    object qFornecedorCEP: TStringField
      FieldName = 'CEP'
      Size = 10
    end
    object qFornecedorTELFONE: TStringField
      FieldName = 'TELFONE'
      Size = 14
    end
    object qFornecedorFAX: TStringField
      FieldName = 'FAX'
      Size = 14
    end
    object qFornecedorCELULAR: TStringField
      FieldName = 'CELULAR'
      Size = 14
    end
    object qFornecedorEMAIL: TStringField
      FieldName = 'EMAIL'
      Size = 60
    end
    object qFornecedorCNPJ: TStringField
      FieldName = 'CNPJ'
      Size = 18
    end
    object qFornecedorINS_EST: TStringField
      FieldName = 'INS_EST'
      Size = 14
    end
    object qFornecedorCONTATO: TStringField
      FieldName = 'CONTATO'
      Size = 30
    end
    object qFornecedorSITE: TStringField
      FieldName = 'SITE'
      Size = 70
    end
    object qFornecedorFANTASIA: TStringField
      FieldName = 'FANTASIA'
      Size = 30
    end
    object qFornecedorSITUACAO: TStringField
      FieldName = 'SITUACAO'
      Size = 30
    end
    object qFornecedorDT_ABERTURA: TDateField
      FieldName = 'DT_ABERTURA'
    end
    object qFornecedorCNAE_PRINCIPAL: TStringField
      FieldName = 'CNAE_PRINCIPAL'
      Size = 200
    end
    object qFornecedorCNAE_SECUNDARIO: TStringField
      FieldName = 'CNAE_SECUNDARIO'
      Size = 999
    end
    object qFornecedorID_CATEGORIA: TIntegerField
      FieldName = 'ID_CATEGORIA'
    end
    object qFornecedorCIDADE: TStringField
      FieldName = 'CIDADE'
      Size = 35
    end
    object qFornecedorUF: TStringField
      FieldName = 'UF'
      Size = 2
    end
    object qFornecedorDDD: TStringField
      FieldName = 'DDD'
      Size = 2
    end
    object qFornecedorNOME_CATEGORIA: TStringField
      FieldName = 'NOME_CATEGORIA'
      Size = 40
    end
  end
  object dsServicos: TDataSource
    DataSet = cdsServicos
    Left = 592
    Top = 50
  end
  object cdsServicos: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'dspServicos'
    Left = 560
    Top = 50
    object cdsServicosID_SERVICO_TINTURARIA: TIntegerField
      FieldName = 'ID_SERVICO_TINTURARIA'
      Required = True
    end
    object cdsServicosID_SERVICO: TIntegerField
      FieldName = 'ID_SERVICO'
    end
    object cdsServicosID_PRECO_TINTURARIA: TIntegerField
      FieldName = 'ID_PRECO_TINTURARIA'
    end
    object cdsServicosID_ARTIGO_TINTURARIA: TIntegerField
      FieldName = 'ID_ARTIGO_TINTURARIA'
    end
    object cdsServicosID_ARTIGO: TIntegerField
      FieldName = 'ID_ARTIGO'
    end
    object cdsServicosVALOR: TFMTBCDField
      DisplayLabel = 'Valor'
      FieldName = 'VALOR'
      DisplayFormat = '###,###0.00'
      EditFormat = '###,###0.00'
      Precision = 18
      Size = 4
    end
    object cdsServicosDESCRICAO: TStringField
      DisplayLabel = 'Servi'#231'o'
      FieldName = 'DESCRICAO'
      Size = 50
    end
    object cdsServicosQUANT: TFMTBCDField
      DisplayLabel = 'Quant'
      FieldName = 'QUANT'
      Precision = 18
      Size = 4
    end
    object cdsServicosTOTAL: TFMTBCDField
      DisplayLabel = 'Total'
      FieldName = 'TOTAL'
      DisplayFormat = '###,###0.00'
      EditFormat = '###,###0.00'
      Precision = 18
    end
  end
  object dspServicos: TDataSetProvider
    DataSet = qServicos
    Options = [poIncFieldProps, poAllowMultiRecordUpdates, poAutoRefresh, poAllowCommandText, poUseQuoteChar]
    Left = 528
    Top = 50
  end
  object qServicos: TSQLQuery
    MaxBlobSize = -1
    Params = <>
    SQL.Strings = (
      'SELECT'
      ' ST.id_servico_tinturaria,'
      ' st.id_servico,'
      ' st.id_preco_tinturaria,'
      ' st.id_artigo_tinturaria,'
      ' st.id_artigo,'
      ' st.valor,'
      ' s.descricao,'
      ' ('
      '   SELECT IR.quant_faturado FROM tb_item_remessa_producao IR'
      
        '   JOIN tb_remessa_producao RP ON (RP.id_remessa = IR.id_remessa' +
        ')'
      '   JOIN tb_cliente C ON (C.id_cliente = RP.id_cliente)'
      '   JOIN tb_fornecedor F ON (F.id_fornecedor = RP.id_forn_tint)'
      '   WHERE C.id_grupo_empresa = 3'
      '   AND F.id_fornecedor =7'
      ' )AS QUANT,'
      ' ST.valor *  ('
      '   SELECT IR.quant_faturado FROM tb_item_remessa_producao IR'
      
        '   JOIN tb_remessa_producao RP ON (RP.id_remessa = IR.id_remessa' +
        ')'
      '   JOIN tb_cliente C ON (C.id_cliente = RP.id_cliente)'
      '   JOIN tb_fornecedor F ON (F.id_fornecedor = RP.id_forn_tint)'
      '   WHERE C.id_grupo_empresa = 3'
      '   AND F.id_fornecedor =7'
      ' ) AS TOTAL'
      'FROM tb_servico_tinturaria ST'
      'JOIN tb_servico S ON (S.id_servico = ST.id_servico)'
      
        'JOIN tb_preco_tinturaria P ON (P.id_preco_tinturaria = ST.id_pre' +
        'co_tinturaria)'
      
        'JOIN tb_grupo_empresa G ON (G.id_grupo_empresa = P.id_grupo_empr' +
        'esa)'
      'where G.id_grupo_empresa =3'
      'AND P.id_fornecedor = 7')
    SQLConnection = DM.conexao
    Left = 496
    Top = 50
    object qServicosID_SERVICO_TINTURARIA: TIntegerField
      FieldName = 'ID_SERVICO_TINTURARIA'
      Required = True
    end
    object qServicosID_SERVICO: TIntegerField
      FieldName = 'ID_SERVICO'
    end
    object qServicosID_PRECO_TINTURARIA: TIntegerField
      FieldName = 'ID_PRECO_TINTURARIA'
    end
    object qServicosID_ARTIGO_TINTURARIA: TIntegerField
      FieldName = 'ID_ARTIGO_TINTURARIA'
    end
    object qServicosID_ARTIGO: TIntegerField
      FieldName = 'ID_ARTIGO'
    end
    object qServicosVALOR: TFMTBCDField
      FieldName = 'VALOR'
      Precision = 18
      Size = 4
    end
    object qServicosDESCRICAO: TStringField
      FieldName = 'DESCRICAO'
      Size = 50
    end
    object qServicosQUANT: TFMTBCDField
      FieldName = 'QUANT'
      Precision = 18
      Size = 4
    end
    object qServicosTOTAL: TFMTBCDField
      FieldName = 'TOTAL'
      Precision = 18
    end
  end
  object dsNotas: TDataSource
    DataSet = cdsNotas
    Left = 512
    Top = 107
  end
  object cdsNotas: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'dspNotas'
    Left = 480
    Top = 107
    object cdsNotasID_ITEM_PRODUCAO: TIntegerField
      FieldName = 'ID_ITEM_PRODUCAO'
      Required = True
    end
    object cdsNotasID_PRODUCAO: TIntegerField
      FieldName = 'ID_PRODUCAO'
    end
    object cdsNotasID_MATERIA_PRIMA: TIntegerField
      FieldName = 'ID_MATERIA_PRIMA'
    end
    object cdsNotasQUANT_FATURADO: TFMTBCDField
      DisplayLabel = 'Quant.'
      FieldName = 'QUANT_FATURADO'
      Precision = 18
      Size = 4
    end
    object cdsNotasVL_UNIT: TFMTBCDField
      DisplayLabel = 'Valor Unit'
      FieldName = 'VL_UNIT'
      DisplayFormat = '###,###0.00'
      EditFormat = '###,###0.00'
      Precision = 18
      Size = 2
    end
    object cdsNotasVL_TOTAL: TFMTBCDField
      DisplayLabel = 'Vl Total'
      FieldName = 'VL_TOTAL'
      DisplayFormat = '###,###0.00'
      EditFormat = '###,###0.00'
      Precision = 18
      Size = 2
    end
    object cdsNotasVL_UNIT_REAL: TFMTBCDField
      FieldName = 'VL_UNIT_REAL'
      DisplayFormat = '###,###0.000'
      EditFormat = '###,###0.00'
      Precision = 18
      Size = 2
    end
    object cdsNotasVL_TOTAL_REAL: TFMTBCDField
      FieldName = 'VL_TOTAL_REAL'
      Precision = 18
      Size = 2
    end
    object cdsNotasQUANT_PROD: TFMTBCDField
      FieldName = 'QUANT_PROD'
      Precision = 18
      Size = 2
    end
    object cdsNotasCLIENTE: TStringField
      DisplayLabel = 'Cliente'
      FieldName = 'CLIENTE'
      Size = 50
    end
    object cdsNotasFORNECEDOR: TStringField
      DisplayLabel = 'Fornecedor'
      FieldName = 'FORNECEDOR'
      Size = 50
    end
    object cdsNotasNR_NOTA_FISCAL: TIntegerField
      DisplayLabel = 'N'#186' Nota Fiscal'
      FieldName = 'NR_NOTA_FISCAL'
    end
    object cdsNotasNR_NF_REMESSA: TIntegerField
      FieldName = 'NR_NF_REMESSA'
    end
    object cdsNotasNOME_MATERIA: TStringField
      DisplayLabel = 'Mat'#233'ria-Prima'
      FieldName = 'NOME_MATERIA'
      Size = 30
    end
    object cdsNotasDT_EMISSAO: TDateField
      DisplayLabel = 'Data Emiss'#227'o'
      FieldName = 'DT_EMISSAO'
    end
    object cdsNotasID_FORNECEDOR: TIntegerField
      FieldName = 'ID_FORNECEDOR'
      Required = True
    end
    object cdsNotasID_FORN_REM: TIntegerField
      FieldName = 'ID_FORN_REM'
      Required = True
    end
  end
  object dspNotas: TDataSetProvider
    DataSet = qNotas
    Options = [poIncFieldProps, poAllowMultiRecordUpdates, poAutoRefresh, poAllowCommandText, poUseQuoteChar]
    Left = 448
    Top = 107
  end
  object qNotas: TSQLQuery
    MaxBlobSize = -1
    Params = <>
    SQL.Strings = (
      'SELECT'
      ' IR.id_item_producao,'
      ' ir.id_producao,'
      ' ir.id_materia_prima,'
      ' ir.quant_faturado,'
      ' ir.vl_unit,'
      ' ir.vl_total,'
      ' ir.vl_unit_real,'
      ' ir.vl_total_real,'
      ' ir.quant_prod,'
      ' rp.nr_nota_fiscal,'
      ' rp.nr_nf_remessa,'
      ' rp.dt_emissao,'
      ' C.r_social AS CLIENTE,'
      ' F.nome AS FORNECEDOR,'
      ' rp.id_forn_rem,'
      ' f.id_fornecedor,'
      ' MP.nome_materia'
      'FROM tb_item_producao IR'
      'JOIN tb_producao RP ON (RP.id_producao = IR.id_producao)'
      'JOIN tb_cliente C ON (C.id_cliente = RP.id_cliente)'
      'JOIN tb_fornecedor F ON (F.id_fornecedor = RP.id_forncedor)'
      
        'JOIN tb_materia_prima MP ON (MP.id_materia = IR.id_materia_prima' +
        ')'
      'WHERE RP.id_cliente = 3'
      'AND RP.dt_emissao between '#39'07/24/2020'#39' and '#39'08/31/2020'#39)
    SQLConnection = DM.conexao
    Left = 416
    Top = 107
    object qNotasID_ITEM_PRODUCAO: TIntegerField
      FieldName = 'ID_ITEM_PRODUCAO'
      Required = True
    end
    object qNotasID_PRODUCAO: TIntegerField
      FieldName = 'ID_PRODUCAO'
    end
    object qNotasID_MATERIA_PRIMA: TIntegerField
      FieldName = 'ID_MATERIA_PRIMA'
    end
    object qNotasQUANT_FATURADO: TFMTBCDField
      FieldName = 'QUANT_FATURADO'
      Precision = 18
      Size = 4
    end
    object qNotasVL_UNIT: TFMTBCDField
      FieldName = 'VL_UNIT'
      Precision = 18
      Size = 2
    end
    object qNotasVL_TOTAL: TFMTBCDField
      FieldName = 'VL_TOTAL'
      Precision = 18
      Size = 2
    end
    object qNotasVL_UNIT_REAL: TFMTBCDField
      FieldName = 'VL_UNIT_REAL'
      Precision = 18
      Size = 2
    end
    object qNotasVL_TOTAL_REAL: TFMTBCDField
      FieldName = 'VL_TOTAL_REAL'
      Precision = 18
      Size = 2
    end
    object qNotasQUANT_PROD: TFMTBCDField
      FieldName = 'QUANT_PROD'
      Precision = 18
      Size = 2
    end
    object qNotasCLIENTE: TStringField
      FieldName = 'CLIENTE'
      Size = 50
    end
    object qNotasFORNECEDOR: TStringField
      FieldName = 'FORNECEDOR'
      Size = 50
    end
    object qNotasNR_NOTA_FISCAL: TIntegerField
      FieldName = 'NR_NOTA_FISCAL'
    end
    object qNotasNR_NF_REMESSA: TIntegerField
      FieldName = 'NR_NF_REMESSA'
    end
    object qNotasNOME_MATERIA: TStringField
      FieldName = 'NOME_MATERIA'
      Size = 30
    end
    object qNotasDT_EMISSAO: TDateField
      FieldName = 'DT_EMISSAO'
    end
    object qNotasID_FORNECEDOR: TIntegerField
      FieldName = 'ID_FORNECEDOR'
      Required = True
    end
    object qNotasID_FORN_REM: TIntegerField
      FieldName = 'ID_FORN_REM'
      Required = True
    end
  end
  object dsFaturamento: TDataSource
    DataSet = cdsFaturamento
    Left = 272
    Top = 139
  end
  object cdsFaturamento: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'dspFaturamento'
    Left = 240
    Top = 139
    object cdsFaturamentoMyColumn1: TStringField
      FieldName = 'MyColumn1'
      Required = True
      FixedChar = True
      Size = 10
    end
    object cdsFaturamentoFORNECEDOR: TStringField
      DisplayLabel = 'Fornecedor'
      FieldName = 'FORNECEDOR'
      Size = 50
    end
    object cdsFaturamentoNR_NOTA_FISCAL: TIntegerField
      DisplayLabel = 'N'#186' Nota Fiscal'
      FieldName = 'NR_NOTA_FISCAL'
    end
    object cdsFaturamentoDT_EMISSAO: TDateField
      DisplayLabel = 'Data Emiss'#227'o'
      FieldName = 'DT_EMISSAO'
    end
    object cdsFaturamentoVL_NOTA: TFMTBCDField
      DisplayLabel = 'Valor Nota'
      FieldName = 'VL_NOTA'
      DisplayFormat = '###,###0.00'
      EditFormat = '###,###0.00'
      Precision = 18
      Size = 3
    end
  end
  object dspFaturamento: TDataSetProvider
    DataSet = qFaturamento
    Options = [poIncFieldProps, poAllowMultiRecordUpdates, poAutoRefresh, poAllowCommandText, poUseQuoteChar]
    Left = 208
    Top = 139
  end
  object qFaturamento: TSQLQuery
    MaxBlobSize = -1
    Params = <>
    SQL.Strings = (
      'SELECT'
      ' '#39'FIO'#39','
      ' p.nr_nota_fiscal,'
      ' p.dt_emissao,'
      ' p.vl_nota,'
      ' F.nome AS FORNECEDOR'
      'FROM tb_producao p'
      'JOIN tb_fornecedor F ON (F.id_fornecedor = P.id_forncedor)'
      'WHERE P.id_cliente = 3'
      'AND P.dt_emissao between '#39'07/24/2020'#39' and '#39'08/31/2020'#39
      'union'
      'SELECT'
      ' '#39'MALHARIA'#39','
      ' rp.nr_nota_fiscal,'
      ' rp.dt_emissao,'
      ' rp.vl_nota,'
      ' F.nome AS FORNECEDOR'
      'FROM tb_remessa_producao Rp'
      'JOIN tb_fornecedor F ON (F.id_fornecedor = RP.id_for_remessa)'
      'WHERE RP.id_cliente = 3'
      'AND RP.dt_emissao between '#39'07/24/2020'#39' and '#39'08/31/2020'#39
      'union'
      'SELECT'
      ' '#39'TINTURARIA'#39','
      ' t.nr_nota_fiscal,'
      ' t.dt_emissao,'
      ' t.vl_nota,'
      ' F.nome AS FORNECEDOR'
      'FROM tb_tingimento t'
      'JOIN tb_fornecedor F ON (F.id_fornecedor = t.id_for_tingimento)'
      'WHERE t.id_cliente = 3'
      'AND t.dt_emissao between '#39'07/24/2020'#39' and '#39'08/31/2020'#39)
    SQLConnection = DM.conexao
    Left = 176
    Top = 139
    object qFaturamentoMyColumn1: TStringField
      FieldName = 'MyColumn1'
      Required = True
      FixedChar = True
      Size = 10
    end
    object qFaturamentoNR_NOTA_FISCAL: TIntegerField
      FieldName = 'NR_NOTA_FISCAL'
    end
    object qFaturamentoDT_EMISSAO: TDateField
      FieldName = 'DT_EMISSAO'
    end
    object qFaturamentoVL_NOTA: TFMTBCDField
      FieldName = 'VL_NOTA'
      Precision = 18
      Size = 3
    end
    object qFaturamentoFORNECEDOR: TStringField
      FieldName = 'FORNECEDOR'
      Size = 50
    end
  end
  object dsNotaQuebra: TDataSource
    DataSet = cdsNotaQuebra
    Left = 272
    Top = 172
  end
  object cdsNotaQuebra: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'dspNotaQuebra'
    Left = 240
    Top = 172
    object cdsNotaQuebraID_NOTA_QUEBRA: TIntegerField
      FieldName = 'ID_NOTA_QUEBRA'
      Required = True
    end
    object cdsNotaQuebraID_TINGIMENTO: TIntegerField
      FieldName = 'ID_TINGIMENTO'
    end
    object cdsNotaQuebraID_FOR_TINGIMENTO: TIntegerField
      FieldName = 'ID_FOR_TINGIMENTO'
    end
    object cdsNotaQuebraID_FOR_TINT: TIntegerField
      FieldName = 'ID_FOR_TINT'
    end
    object cdsNotaQuebraID_CLIENTE: TIntegerField
      FieldName = 'ID_CLIENTE'
    end
    object cdsNotaQuebraQUANT: TFMTBCDField
      DisplayLabel = 'Quant'
      FieldName = 'QUANT'
      DisplayFormat = '###,###0.000'
      EditFormat = '###,###0.000'
      Precision = 18
      Size = 3
    end
    object cdsNotaQuebraVALOR: TFMTBCDField
      DisplayLabel = 'Valor'
      FieldName = 'VALOR'
      DisplayFormat = '###,###0.00'
      EditFormat = '###,###0.00'
      Precision = 18
      Size = 3
    end
    object cdsNotaQuebraNR_NOTA_QUEBRA: TIntegerField
      DisplayLabel = 'N'#186' Nota  Fiscal'
      FieldName = 'NR_NOTA_QUEBRA'
    end
    object cdsNotaQuebraMALHARIA: TStringField
      DisplayLabel = 'Malharia'
      FieldName = 'MALHARIA'
      Size = 50
    end
    object cdsNotaQuebraTITURARIA: TStringField
      DisplayLabel = 'Tinturaria'
      FieldName = 'TITURARIA'
      Size = 50
    end
    object cdsNotaQuebraDT_EMISSAO: TDateField
      DisplayLabel = 'Data Emiss'#227'o'
      FieldName = 'DT_EMISSAO'
    end
  end
  object dspNotaQuebra: TDataSetProvider
    DataSet = qNotaQuebra
    Options = [poIncFieldProps, poAllowMultiRecordUpdates, poAutoRefresh, poAllowCommandText, poUseQuoteChar]
    Left = 208
    Top = 172
  end
  object qNotaQuebra: TSQLQuery
    MaxBlobSize = -1
    Params = <>
    SQL.Strings = (
      'SELECT'
      'Q.id_nota_quebra,'
      'q.id_tingimento,'
      'q.id_for_tingimento,'
      'q.id_for_tint,'
      'q.id_cliente,'
      'q.quant,'
      'q.valor,'
      'q.nr_nota_quebra,'
      't.dt_emissao,'
      'F.nome AS MALHARIA,'
      'FF.nome AS TITURARIA'
      'FROM tb_nota_quebra Q'
      'join tb_fornecedor F ON (F.id_fornecedor = Q.id_for_tingimento)'
      'join tb_fornecedor FF ON (FF.id_fornecedor = Q.id_for_tint)'
      'JOIN tb_tingimento T ON (T.id_tingimento = Q.id_tingimento)'
      'WHERE T.id_cliente = 3'
      'AND T.dt_emissao BETWEEN '#39'07/24/2020'#39' AND '#39'08/31/2020'#39)
    SQLConnection = DM.conexao
    Left = 176
    Top = 172
    object qNotaQuebraID_NOTA_QUEBRA: TIntegerField
      FieldName = 'ID_NOTA_QUEBRA'
      Required = True
    end
    object qNotaQuebraID_TINGIMENTO: TIntegerField
      FieldName = 'ID_TINGIMENTO'
    end
    object qNotaQuebraID_FOR_TINGIMENTO: TIntegerField
      FieldName = 'ID_FOR_TINGIMENTO'
    end
    object qNotaQuebraID_FOR_TINT: TIntegerField
      FieldName = 'ID_FOR_TINT'
    end
    object qNotaQuebraID_CLIENTE: TIntegerField
      FieldName = 'ID_CLIENTE'
    end
    object qNotaQuebraQUANT: TFMTBCDField
      FieldName = 'QUANT'
      Precision = 18
      Size = 3
    end
    object qNotaQuebraVALOR: TFMTBCDField
      FieldName = 'VALOR'
      Precision = 18
      Size = 3
    end
    object qNotaQuebraNR_NOTA_QUEBRA: TIntegerField
      FieldName = 'NR_NOTA_QUEBRA'
    end
    object qNotaQuebraMALHARIA: TStringField
      FieldName = 'MALHARIA'
      Size = 50
    end
    object qNotaQuebraTITURARIA: TStringField
      FieldName = 'TITURARIA'
      Size = 50
    end
    object qNotaQuebraDT_EMISSAO: TDateField
      FieldName = 'DT_EMISSAO'
    end
  end
  object RLPDFFilter1: TRLPDFFilter
    DocumentInfo.Title = 'Geprot'
    DocumentInfo.Creator = 
      'FortesReport Community Edition v4.0 \251 Copyright '#169' 1999-2016 F' +
      'ortes Inform'#225'tica'
    FileName = 'C:\Users\Marcio\Desktop\fechamento.pdf'
    DisplayName = 'Documento PDF'
    Left = 751
    Top = 116
  end
  object dsQuebraEstamparia: TDataSource
    DataSet = cdsQuebraEstamparia
    Left = 712
    Top = 150
  end
  object cdsQuebraEstamparia: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'dspQuebraEstamparia'
    Left = 680
    Top = 150
    object cdsQuebraEstampariaID_NOTA_QUEBRA: TIntegerField
      FieldName = 'ID_NOTA_QUEBRA'
      Required = True
    end
    object cdsQuebraEstampariaID_TINGIMENTO: TIntegerField
      FieldName = 'ID_TINGIMENTO'
    end
    object cdsQuebraEstampariaID_FOR_TINGIMENTO: TIntegerField
      FieldName = 'ID_FOR_TINGIMENTO'
    end
    object cdsQuebraEstampariaID_FOR_TINT: TIntegerField
      FieldName = 'ID_FOR_TINT'
    end
    object cdsQuebraEstampariaID_CLIENTE: TIntegerField
      FieldName = 'ID_CLIENTE'
    end
    object cdsQuebraEstampariaQUANT: TFMTBCDField
      FieldName = 'QUANT'
      Precision = 18
      Size = 3
    end
    object cdsQuebraEstampariaVALOR: TFMTBCDField
      FieldName = 'VALOR'
      DisplayFormat = '###,###0.00'
      EditFormat = '###,###0.00'
      Precision = 18
      Size = 3
    end
    object cdsQuebraEstampariaNR_NOTA_QUEBRA: TIntegerField
      FieldName = 'NR_NOTA_QUEBRA'
    end
  end
  object dspQuebraEstamparia: TDataSetProvider
    DataSet = qQuebraEstamparia
    Options = [poIncFieldProps, poAllowMultiRecordUpdates, poAutoRefresh, poAllowCommandText, poUseQuoteChar]
    Left = 648
    Top = 150
  end
  object qQuebraEstamparia: TSQLQuery
    MaxBlobSize = -1
    Params = <>
    SQL.Strings = (
      'SELECT'
      ' NQ.id_nota_quebra,'
      ' nq.id_tingimento,'
      ' nq.id_for_tingimento,'
      ' nq.id_for_tint,'
      ' nq.id_cliente,'
      ' nq.quant,'
      ' nq.valor,'
      ' nq.nr_nota_quebra'
      'FROM tb_nota_quebra NQ'
      'JOIN tb_tingimento T ON (T.id_tingimento = NQ.id_tingimento)'
      'WHERE T.id_cliente = 3'
      'AND T.id_forn_tint = 7'
      'AND NQ.quant <> 18')
    SQLConnection = DM.conexao
    Left = 616
    Top = 150
    object qQuebraEstampariaID_NOTA_QUEBRA: TIntegerField
      FieldName = 'ID_NOTA_QUEBRA'
      Required = True
    end
    object qQuebraEstampariaID_TINGIMENTO: TIntegerField
      FieldName = 'ID_TINGIMENTO'
    end
    object qQuebraEstampariaID_FOR_TINGIMENTO: TIntegerField
      FieldName = 'ID_FOR_TINGIMENTO'
    end
    object qQuebraEstampariaID_FOR_TINT: TIntegerField
      FieldName = 'ID_FOR_TINT'
    end
    object qQuebraEstampariaID_CLIENTE: TIntegerField
      FieldName = 'ID_CLIENTE'
    end
    object qQuebraEstampariaQUANT: TFMTBCDField
      FieldName = 'QUANT'
      Precision = 18
      Size = 3
    end
    object qQuebraEstampariaVALOR: TFMTBCDField
      FieldName = 'VALOR'
      Precision = 18
      Size = 3
    end
    object qQuebraEstampariaNR_NOTA_QUEBRA: TIntegerField
      FieldName = 'NR_NOTA_QUEBRA'
    end
  end
end
