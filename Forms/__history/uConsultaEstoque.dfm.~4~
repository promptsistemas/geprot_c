object frmConsultaEstoque: TfrmConsultaEstoque
  Left = 334
  Top = 124
  BorderIcons = [biSystemMenu]
  BorderStyle = bsSingle
  Caption = 'Consultar Estoque'
  ClientHeight = 255
  ClientWidth = 707
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  OldCreateOrder = False
  Position = poScreenCenter
  OnClose = FormClose
  OnKeyPress = FormKeyPress
  PixelsPerInch = 96
  TextHeight = 13
  object Panel1: TPanel
    Left = 1
    Top = 44
    Width = 705
    Height = 177
    Enabled = False
    TabOrder = 0
    object Label1: TLabel
      Left = 3
      Top = 2
      Width = 29
      Height = 13
      Caption = 'Grupo'
      FocusControl = DBEdit1
    end
    object Label2: TLabel
      Left = 3
      Top = 40
      Width = 33
      Height = 13
      Caption = 'C'#243'digo'
      FocusControl = DBEdit2
    end
    object Label3: TLabel
      Left = 99
      Top = 40
      Width = 47
      Height = 13
      Caption = 'C'#243'd.Barra'
      FocusControl = DBEdit3
    end
    object Label4: TLabel
      Left = 273
      Top = 40
      Width = 48
      Height = 13
      Caption = 'Descri'#231#227'o'
      FocusControl = DBEdit4
    end
    object Label5: TLabel
      Left = 3
      Top = 80
      Width = 40
      Height = 13
      Caption = 'Unidade'
      FocusControl = DBEdit5
    end
    object Label6: TLabel
      Left = 139
      Top = 80
      Width = 55
      Height = 13
      Caption = 'Embalagem'
      FocusControl = DBEdit6
    end
    object Label7: TLabel
      Left = 275
      Top = 80
      Width = 39
      Height = 13
      Caption = 'Estoque'
      FocusControl = DBEdit7
    end
    object Label8: TLabel
      Left = 415
      Top = 80
      Width = 54
      Height = 13
      Caption = 'Valor Custo'
      FocusControl = DBEdit8
    end
    object Label9: TLabel
      Left = 555
      Top = 80
      Width = 42
      Height = 13
      Caption = 'Entradas'
      FocusControl = DBEdit9
    end
    object Label10: TLabel
      Left = 3
      Top = 120
      Width = 32
      Height = 13
      Caption = 'Saidas'
      FocusControl = DBEdit10
    end
    object Label11: TLabel
      Left = 144
      Top = 120
      Width = 41
      Height = 13
      Caption = 'Total R$'
      FocusControl = DBEdit11
    end
    object DBEdit1: TDBEdit
      Left = 3
      Top = 16
      Width = 394
      Height = 19
      BevelInner = bvNone
      BorderStyle = bsNone
      Ctl3D = False
      DataField = 'GRUPO'
      DataSource = dsConsultaEstoque
      ParentCtl3D = False
      ReadOnly = True
      TabOrder = 0
    end
    object DBEdit2: TDBEdit
      Left = 3
      Top = 56
      Width = 95
      Height = 19
      BevelInner = bvNone
      BorderStyle = bsNone
      Ctl3D = False
      DataField = 'CODIGO'
      DataSource = dsConsultaEstoque
      ParentCtl3D = False
      ReadOnly = True
      TabOrder = 1
    end
    object DBEdit3: TDBEdit
      Left = 99
      Top = 56
      Width = 173
      Height = 19
      BevelInner = bvNone
      BorderStyle = bsNone
      Ctl3D = False
      DataField = 'CD_BARRA'
      DataSource = dsConsultaEstoque
      ParentCtl3D = False
      ReadOnly = True
      TabOrder = 2
    end
    object DBEdit4: TDBEdit
      Left = 273
      Top = 56
      Width = 431
      Height = 19
      BevelInner = bvNone
      BorderStyle = bsNone
      Ctl3D = False
      DataField = 'DESCRICAO'
      DataSource = dsConsultaEstoque
      ParentCtl3D = False
      ReadOnly = True
      TabOrder = 3
    end
    object DBEdit5: TDBEdit
      Left = 3
      Top = 96
      Width = 134
      Height = 19
      BevelInner = bvNone
      BorderStyle = bsNone
      Ctl3D = False
      DataField = 'UNIDADE'
      DataSource = dsConsultaEstoque
      ParentCtl3D = False
      ReadOnly = True
      TabOrder = 4
    end
    object DBEdit6: TDBEdit
      Left = 139
      Top = 96
      Width = 134
      Height = 19
      BevelInner = bvNone
      BorderStyle = bsNone
      Ctl3D = False
      DataField = 'EMBALGEM'
      DataSource = dsConsultaEstoque
      ParentCtl3D = False
      ReadOnly = True
      TabOrder = 5
    end
    object DBEdit7: TDBEdit
      Left = 275
      Top = 96
      Width = 138
      Height = 19
      BevelInner = bvNone
      BorderStyle = bsNone
      Color = clMoneyGreen
      Ctl3D = False
      DataField = 'ESTOQUE'
      DataSource = dsConsultaEstoque
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlue
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentCtl3D = False
      ParentFont = False
      ReadOnly = True
      TabOrder = 6
    end
    object DBEdit8: TDBEdit
      Left = 415
      Top = 96
      Width = 138
      Height = 19
      BevelInner = bvNone
      BorderStyle = bsNone
      Color = clInfoBk
      Ctl3D = False
      DataField = 'VL_CUSTO'
      DataSource = dsConsultaEstoque
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clNavy
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentCtl3D = False
      ParentFont = False
      ReadOnly = True
      TabOrder = 7
    end
    object DBEdit9: TDBEdit
      Left = 555
      Top = 96
      Width = 138
      Height = 19
      BevelInner = bvNone
      BorderStyle = bsNone
      Ctl3D = False
      DataField = 'ENTRADA'
      DataSource = dsConsultaEstoque
      ParentCtl3D = False
      ReadOnly = True
      TabOrder = 8
    end
    object DBEdit10: TDBEdit
      Left = 3
      Top = 134
      Width = 138
      Height = 19
      BevelInner = bvNone
      BorderStyle = bsNone
      Ctl3D = False
      DataField = 'SAIDAS'
      DataSource = dsConsultaEstoque
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clRed
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentCtl3D = False
      ParentFont = False
      ReadOnly = True
      TabOrder = 9
    end
    object DBEdit11: TDBEdit
      Left = 144
      Top = 134
      Width = 138
      Height = 19
      BevelInner = bvNone
      BorderStyle = bsNone
      Ctl3D = False
      DataField = 'TOTAL'
      DataSource = dsConsultaEstoque
      ParentCtl3D = False
      ReadOnly = True
      TabOrder = 10
    end
  end
  object BitBtn1: TBitBtn
    Left = 626
    Top = 222
    Width = 75
    Height = 25
    Caption = '&Fechar'
    Glyph.Data = {
      
      4B4B4E1E1FFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
    TabOrder = 1
    OnClick = BitBtn1Click
  end
  object BB_PESQUISAR: TBitBtn
    Left = 3
    Top = 2
    Width = 100
    Height = 39
    Caption = '&Pesquisar'
    Glyph.Data = {
      
      DB93A7B8B7AE9999DBDBDBDBDBDBDBDBDBDBDB9D9E9ADBDBDBDB}
    Layout = blGlyphTop
    TabOrder = 2
    OnClick = BB_PESQUISARClick
  end
  object P_PESQUISA: TPanel
    Left = 0
    Top = 1
    Width = 706
    Height = 253
    TabOrder = 3
    Visible = False
    object Label12: TLabel
      Left = 8
      Top = 1
      Width = 83
      Height = 13
      Caption = 'Pesquisa Produto'
    end
    object DBGrid1: TDBGrid
      Left = 8
      Top = 42
      Width = 689
      Height = 177
      Color = 14155775
      Ctl3D = False
      DataSource = dsConsultaEstoque
      Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit]
      ParentCtl3D = False
      ReadOnly = True
      TabOrder = 0
      TitleFont.Charset = DEFAULT_CHARSET
      TitleFont.Color = clWindowText
      TitleFont.Height = -11
      TitleFont.Name = 'MS Sans Serif'
      TitleFont.Style = []
      OnDrawColumnCell = DBGrid1DrawColumnCell
      Columns = <
        item
          Expanded = False
          FieldName = 'CODIGO'
          Width = 70
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'DESCRICAO'
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'UNIDADE'
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'EMBALGEM'
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'ESTOQUE'
          Width = 65
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'GRUPO'
          Visible = True
        end>
    end
    object BB_OK: TBitBtn
      Left = 507
      Top = 222
      Width = 75
      Height = 25
      Caption = 'OK'
      Glyph.Data = {
        
        0000}
      NumGlyphs = 2
      TabOrder = 1
      OnClick = BB_OKClick
    end
    object edtProduto: TEdit
      Left = 8
      Top = 20
      Width = 393
      Height = 19
      CharCase = ecUpperCase
      Ctl3D = False
      ParentCtl3D = False
      TabOrder = 2
      OnChange = edtProdutoChange
      OnExit = edtProdutoExit
      OnKeyPress = FormKeyPress
    end
    object BB_CANCELAR: TBitBtn
      Left = 584
      Top = 222
      Width = 75
      Height = 25
      Cancel = True
      Caption = '&Cancelar'
      Glyph.Data = {
        
        0000}
      NumGlyphs = 2
      TabOrder = 3
      OnClick = BB_CANCELARClick
    end
  end
  object dspConsultaEstoque: TDataSetProvider
    DataSet = qConsultaEstoque
    Options = [poIncFieldProps, poReadOnly, poAllowMultiRecordUpdates, poAutoRefresh, poAllowCommandText]
    Left = 595
    Top = 8
  end
  object cdsConsultaEstoque: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'dspConsultaEstoque'
    Left = 627
    Top = 8
    object cdsConsultaEstoqueGRUPO: TStringField
      DisplayLabel = 'Grupo'
      FieldName = 'GRUPO'
      Size = 30
    end
    object cdsConsultaEstoqueCODIGO: TStringField
      DisplayLabel = 'C'#243'digo'
      FieldName = 'CODIGO'
      Size = 7
    end
    object cdsConsultaEstoqueCD_BARRA: TStringField
      DisplayLabel = 'C'#243'd.Barra'
      FieldName = 'CD_BARRA'
      Size = 13
    end
    object cdsConsultaEstoqueDESCRICAO: TStringField
      DisplayLabel = 'Descri'#231#227'o'
      FieldName = 'DESCRICAO'
      Size = 50
    end
    object cdsConsultaEstoqueUNIDADE: TStringField
      DisplayLabel = 'Unidade'
      FieldName = 'UNIDADE'
      Size = 10
    end
    object cdsConsultaEstoqueEMBALGEM: TStringField
      DisplayLabel = 'Embalagem'
      FieldName = 'EMBALGEM'
      Size = 10
    end
    object cdsConsultaEstoqueESTOQUE: TFMTBCDField
      DisplayLabel = 'Estoque'
      FieldName = 'ESTOQUE'
      DisplayFormat = '###,###0.000'
      EditFormat = '###,###0.000'
      Precision = 18
      Size = 3
    end
    object cdsConsultaEstoqueVL_CUSTO: TFMTBCDField
      DisplayLabel = 'Vl.Custo'
      FieldName = 'VL_CUSTO'
      DisplayFormat = '###,###0.00'
      EditFormat = '###,###0.00'
      Precision = 18
      Size = 2
    end
    object cdsConsultaEstoqueENTRADA: TFMTBCDField
      DisplayLabel = 'Entradas'
      FieldName = 'ENTRADA'
      ReadOnly = True
      DisplayFormat = '###,###0.000'
      EditFormat = '###,###0.000'
      Precision = 18
      Size = 2
    end
    object cdsConsultaEstoqueSAIDAS: TFMTBCDField
      DisplayLabel = 'Saidas'
      FieldName = 'SAIDAS'
      ReadOnly = True
      DisplayFormat = '###,###0.000'
      EditFormat = '###,###0.000'
      Precision = 18
      Size = 3
    end
    object cdsConsultaEstoqueTOTAL: TFMTBCDField
      DisplayLabel = 'Total (R$)'
      FieldName = 'TOTAL'
      ReadOnly = True
      DisplayFormat = '###,###0.00'
      EditFormat = '###,###0.00'
      Precision = 18
      Size = 5
    end
  end
  object dsConsultaEstoque: TDataSource
    DataSet = cdsConsultaEstoque
    Left = 659
    Top = 8
  end
  object qConsultaEstoque: TSQLQuery
    MaxBlobSize = -1
    Params = <>
    SQL.Strings = (
      'SELECT'
      ' GP.DESCRICAO AS GRUPO,'
      ' p.CODIGO,'
      ' p.CD_BARRA,'
      ' p.DESCRICAO,'
      ' P.UNIDADE,'
      ' P.EMBALGEM,'
      ' P.ESTOQUE,'
      ' P.VL_CUSTO,'
      '  (select SUM(N.QUANTIDADE) FROM TB_ITEM_ENTRADA_NF N'
      '   WHERE N.ID_GRUPO_PRODUTO = P.ID_GRUPO_PRODUTO'
      '   AND N.ID_PRODUTO = P.ID_PRODUTO) AS ENTRADA,'
      '  (select SUM(I.QTDVENDA) FROM TB_ITEMVENDADTO I'
      '   WHERE I.ID_GRUPO_PRODUTO = P.ID_GRUPO_PRODUTO'
      '   AND I.ID_PRODUTO = P.ID_PRODUTO) as saidas,'
      ''
      '   (select SUM(N.QUANTIDADE) FROM TB_ITEM_ENTRADA_NF N'
      '   WHERE N.ID_GRUPO_PRODUTO = P.ID_GRUPO_PRODUTO'
      '   AND N.ID_PRODUTO = P.ID_PRODUTO)-'
      '   (select SUM(I.QTDVENDA) FROM TB_ITEMVENDADTO I'
      '   WHERE I.ID_GRUPO_PRODUTO = P.ID_GRUPO_PRODUTO'
      '   AND I.ID_PRODUTO = P.ID_PRODUTO) as saldo,'
      '   P.ESTOQUE * P.VL_CUSTO AS TOTAL'
      'FROM TB_PRODUTO P'
      
        'LEFT OUTER JOIN TB_GRUPO_PRODUTO GP ON (GP.ID_GRUPO_PRODUTO = P.' +
        'ID_GRUPO_PRODUTO)'
      'ORDER BY P.DESCRICAO')
    SQLConnection = DM.conexao
    Left = 563
    Top = 8
    object qConsultaEstoqueGRUPO: TStringField
      FieldName = 'GRUPO'
      Size = 30
    end
    object qConsultaEstoqueCODIGO: TStringField
      FieldName = 'CODIGO'
      Size = 7
    end
    object qConsultaEstoqueCD_BARRA: TStringField
      FieldName = 'CD_BARRA'
      Size = 13
    end
    object qConsultaEstoqueDESCRICAO: TStringField
      FieldName = 'DESCRICAO'
      Size = 50
    end
    object qConsultaEstoqueUNIDADE: TStringField
      FieldName = 'UNIDADE'
      Size = 10
    end
    object qConsultaEstoqueEMBALGEM: TStringField
      FieldName = 'EMBALGEM'
      Size = 10
    end
    object qConsultaEstoqueESTOQUE: TFMTBCDField
      FieldName = 'ESTOQUE'
      Precision = 18
      Size = 3
    end
    object qConsultaEstoqueVL_CUSTO: TFMTBCDField
      FieldName = 'VL_CUSTO'
      Precision = 18
      Size = 2
    end
    object qConsultaEstoqueENTRADA: TFMTBCDField
      FieldName = 'ENTRADA'
      ReadOnly = True
      Precision = 18
      Size = 2
    end
    object qConsultaEstoqueSAIDAS: TFMTBCDField
      FieldName = 'SAIDAS'
      ReadOnly = True
      Precision = 18
      Size = 3
    end
    object qConsultaEstoqueTOTAL: TFMTBCDField
      FieldName = 'TOTAL'
      ReadOnly = True
      Precision = 18
      Size = 5
    end
  end
end
