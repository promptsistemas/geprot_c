...
begin
  ...
  begin
    ShowMessage('Nota Fiscal Já Autorizada. Exclusão Negada');
    Exit;
  end;
...
begin
  ...
  if DM.cdsItemNotaFiscal.RecordCount > 0 then
  begin
    ShowMessage('Nota Fiscal Já Foi Gerada.');
    exit;
  end;
  if DM.cdsNotaFiscal.IsEmpty then
  begin
    ShowMessage('Não Exite Nota Para Gerar.');
    Exit;
  end
  else
  begin
...
begin
  ...
  begin
    ShowMessage('    A T E N Ç Ã O  !!!!!! '+#13+
                '                          '+#13+
                'Campo "Cód.Cliente" não informado');
    DBEdit1.SetFocus;
    Exit;
  end;
    if Tipo ='Insert' then
...
//  DBMemoFisco.Lines.Add(' Interesse Ao Fisco : EMPRESA OPTANTE PELO SIMPLES NACIONAL LEI COMPLEMENTAR Nº 123, DE 14/12/2016. NÃO GERA DIREITO A CRÉDITO FISCAL DE ICMS.');
  GerenciaBotoes;
  GerenciaBotoes2;
  DBLookupComboBox2.SetFocus;
end;
...
begin
  Pesquisa(DM.cdsNotaFiscal,['NR_NOTA_FISCAL','DT_EMISSAO','R_SOCIAL','VL_TOTAL_NOTA'],
                        ['Nº Nota','Data','Nome do Cliente','Total da Nota'],
...
begin
  if DM.cdsNotaFiscalSITUACAO.Value = 'Autorizada' then
  begin
    ShowMessage('Nota Fiscal Já Autorizada. Operação Negada');
    Exit;
  end;
...
begin
  ...
  if Application.MessageBox('Deseja Excluir Todos os Itens?','Confirmação',MB_YESNO+MB_ICONQUESTION ) = idyes then
  begin
    ...
    ShowMessage('Regsistros Excluídos com Sucesso');
  end;
...
begin
  ...
  ShowMessage('Informação Cancelada.');
  TabSheet3.Enabled := False;
end;
...
begin
  ...
  begin
    ShowMessage('Nota Fiscal Não Foi Autorizada, Impossivél Realizar o Cancelamento. Por Favor Verifique');
    Exit;
  end;
  ...
  ShowMessage('Por Favor Conferir os Dada Nota Para Cancelamento '+#13+
              'Nº da NF-e  '+DadosNota);

  if Application.MessageBox(' Deseja Cancelar a NF-e ','confirmação',MB_YESNO+MB_ICONQUESTION) = idyes then
  begin
  {  OpenDialog1.Title := 'Selecione a NFE';
    OpenDialog1.DefaultExt := '*-nfe.XML';
    OpenDialog1.Filter := 'Arquivos NFE (*-nfe.XML)|*-nfe.XML|Arquivos XML (*.XML)|*.XML|Todos os Arquivos (*.*)|*.*';
...
//      DM.cdsNotaFiscalCHAVE_NFE.AsString := ACBrNFe1.WebServices.EnvEvento.EventoRetorno.retEvento.Items[0].RetInfEvento.nProt;
  ...
  begin
    ShowMessage('Cancelamento Não Realizado!');
  end;
...
begin
 ...
 begin
   ShowMessage('Nota Fiscal Já Assinada e Enviada');
   Exit;
 end;
...
//      DM.cdsNotaFiscalR_SOCIAL_EMPRESA.AsString := cdsPDVLOJA.AsString;
     ...
     //   LBLTPPESSOA.Caption :='Pessoa Jurídica'
     // else
     //   LBLTPPESSOA.Caption :='Pessoa Física';
...
Var
 vRecNo : Integer;
begin
  if Application.MessageBox('Confirma Acrescimo Para o Item?','Confirmação',MB_YESNO+MB_ICONQUESTION) = idyes then
  begin
    DM.cdsItemNotaFiscal.ApplyUpdates(0);
    BB_CALCULARClick(Self);
...
Var
 vRecNo : Integer;
begin
  if Application.MessageBox('Confirma Desconto Para o Item?','Confirmação',MB_YESNO+MB_ICONQUESTION) = idyes then
  begin
    DM.cdsItemNotaFiscal.ApplyUpdates(0);
    BB_CALCULARClick(Self);
...
begin
  If DM.cdsNotaFiscalSITUACAO.AsString = 'Cancelada'  then // condição
  begin
  ...
  if DM.cdsNotaFiscalSITUACAO.AsString = 'Autorizada'  then // condição
  begin
  ...
  if DM.cdsNotaFiscalSITUACAO.AsString = 'Em digitacao'  then // condição
  begin
  ...
  if DM.cdsNotaFiscalSITUACAO.AsString = 'Assinada'  then // condição
  begin
...
//                ICMS.pRedBCST := 20;
  ...
  with autXML.Add do // Lista contendo até 10
  begin
...
//  DM.cdsNotaFiscal.ApplyUpdates(0);
  ...
  MemoResp.Lines.Add('       STATUS DO SERVIÇOS   ');
  MemoResp.Lines.Add('');
...
begin
   if Pesquisa(cdsPDV,['ID_PDV','ID_CLIENTE','R_SOCIAL','CNPJ_CPF_CLIENTE'],
    ['','','',''],[8,6,28,20],'','0','','Orçamento','Orçamento','Orçamento') Then
   begin
      DM.cdsNotaFiscalID_PDV.AsInteger          := cdsPDVID_PDV.AsInteger;
      DM.cdsNotaFiscalID_CLIENTE.AsInteger      := cdsPDVID_CLIENTE.AsInteger;
      DM.cdsNotaFiscalR_SOCIAL.AsString         := cdsPDVR_SOCIAL.AsString;
