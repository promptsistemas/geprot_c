...
begin
  ...
  cdsComplementar.Params[3].AsInteger := StrToInt(DM.cdsRemessaProducaoNR_NOTA_FISCAL.AsString);
  cdsComplementar.Open;
...
begin
  ...
  DM.cdsDinamica.CommandText :=' SELECT SUM(IP.VL_TOTAL_REAL),SUM(IP.VL_TOTAL),SUM(IP.VL_TOTAL_REAL)+SUM(IP.VL_TOTAL) '+
                               ...
                               ' WHERE IP.ID_REMESSA = '+DM.cdsRemessaProducaoID_REMESSA.AsString;
  DM.cdsDinamica.Open;


  DM.cdsRemessaProducao.Edit;
  DM.cdsRemessaProducaoVL_POR_FORA.AsFloat  := DM.cdsDinamica.Fields[0].AsFloat;
  DM.cdsRemessaProducaoVL_NOTA.AsFloat      := DM.cdsDinamica.Fields[1].AsFloat;
  DM.cdsRemessaProducaoVL_TOTAL_NF.AsFloat  := DM.cdsDinamica.Fields[2].AsFloat;;
