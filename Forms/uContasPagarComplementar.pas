...
begin
  ...
  begin
    ...
    cdsComplementarORIGEM.AsString              := 'TECELAGEM';
    ...
    if cdscomplementar.RecordCount = 0 then
      cdsComplementarNR_DOC.Value                 := DM.cdsProducaoNR_NF_REMESSA.AsString +'/'+IntToStr(1)+'-'+cdsContasPagarQT_PARCELA.AsString+'/R'
    else
      cdsComplementarNR_DOC.Value                 := DM.cdsProducaoNR_NF_REMESSA.AsString +'/'+IntToStr(Parcelas+1)+'-'+cdsContasPagarQT_PARCELA.AsString+'/R';
...
{    if cdsContasPagar.RecordCount > 0 then
    ...
    if not DM.cdsPlanoPagamentoQT_PARCELAS.IsNull then
        ...
        (DM.cdsProducaoVL_POR_FORA.AsFloat / cdsContasPagarQT_PARCELA.AsInteger);
...
begin
  ...
  begin
    if iCont = cdsContasPagarQT_PARCELA.AsInteger -1 then
      ...
      cdsComplementarVL_PARCELA.AsFloat := DM.cdsProducaoVL_POR_FORA.AsFloat/cdsContasPagarQT_PARCELA.AsInteger;
  end;
  if Tipo = 'Edit' then
  begin
    vlParcelaEdit := cdsComplementarVL_PARCELA.AsFloat;
