...
begin
  ...
  begin
    ...
    begin
      ...
      Pesquisa(cdsReativaProducao,['NR_NOTA_FISCAL','FORNECEDOR'],['Nº Nota Fiscal','Fornecedor'],[16,40],'','','','','Reativar Nota','Reativar Nota');
...
procedure TfrmReativaProducao.edtPesquisaKeyPress(Sender: TObject;
  var Key: Char);
begin
  if Key = #13 then
  begin
...
begin
  if Application.MessageBox('Deseja Realmente Reativar Nota?','Confirmação',MB_ICONQUESTION+MB_YESNO) = idyes then
  begin
...
procedure TfrmReativaProducao.SpeedButton1Click(Sender: TObject);
Var
 sSql : string;
begin
