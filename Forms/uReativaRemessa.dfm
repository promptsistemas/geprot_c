object frmReativaRemessa: TfrmReativaRemessa
  Left = 0
  Top = 0
  BorderStyle = bsNone
  Caption = 'frmReativaRemessa'
  ClientHeight = 484
  ClientWidth = 813
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  OldCreateOrder = False
  Position = poScreenCenter
  OnClose = FormClose
  OnKeyPress = FormKeyPress
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object SB_REATIVAR: TSpeedButton
    Left = 4
    Top = 443
    Width = 149
    Height = 40
    Caption = 'Reativar Nota Fiscal'
    Enabled = False
    Glyph.Data = {
      
      FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
    NumGlyphs = 2
    OnClick = SB_REATIVARClick
  end
  object Panel1: TPanel
    Left = 0
    Top = 0
    Width = 813
    Height = 41
    Align = alTop
    Color = clBlue
    Font.Charset = ANSI_CHARSET
    Font.Color = clBlue
    Font.Height = -21
    Font.Name = 'Segoe UI Semibold'
    Font.Style = [fsBold]
    ParentBackground = False
    ParentFont = False
    TabOrder = 0
    object Label17: TLabel
      Left = 8
      Top = 4
      Width = 456
      Height = 37
      Caption = 'REATIVAR NOTA FISCAL MALHARIA'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWhite
      Font.Height = -27
      Font.Name = 'Segoe UI'
      Font.Style = [fsBold, fsItalic]
      ParentFont = False
    end
    object SpeedButton3: TSpeedButton
      Left = 774
      Top = 1
      Width = 38
      Height = 39
      Align = alRight
      Flat = True
      Glyph.Data = {
        
        000000000000}
      OnClick = SpeedButton3Click
      ExplicitLeft = 1102
      ExplicitTop = 5
      ExplicitHeight = 36
    end
  end
  object Panel2: TPanel
    Left = 0
    Top = 41
    Width = 813
    Height = 152
    Align = alTop
    Color = clWhite
    ParentBackground = False
    TabOrder = 1
    object Label1: TLabel
      Left = 4
      Top = 1
      Width = 67
      Height = 13
      Caption = 'N'#186' Nota Fiscal'
    end
    object Label2: TLabel
      Left = 4
      Top = 36
      Width = 33
      Height = 13
      Caption = 'C'#243'digo'
      FocusControl = DBEdit1
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlue
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
    end
    object Label4: TLabel
      Left = 4
      Top = 73
      Width = 64
      Height = 13
      Caption = 'Data Emiss'#227'o'
      FocusControl = DBEdit4
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlue
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
    end
    object Label21: TLabel
      Left = 4
      Top = 113
      Width = 41
      Height = 13
      Caption = 'C'#243'd.Tint'
      FocusControl = DBEdit11
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlue
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
    end
    object Label10: TLabel
      Left = 77
      Top = 113
      Width = 76
      Height = 13
      Caption = 'Nome Tinturaria'
      FocusControl = DBEdit10
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlue
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
    end
    object Label22: TLabel
      Left = 77
      Top = 73
      Width = 56
      Height = 13
      Caption = 'C'#243'd.Cliente'
      FocusControl = DBEdit11
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlue
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
    end
    object Label3: TLabel
      Left = 78
      Top = 36
      Width = 67
      Height = 13
      Caption = 'N'#186' Nota Fiscal'
      FocusControl = DBEdit2
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlue
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
    end
    object Label5: TLabel
      Left = 161
      Top = 36
      Width = 63
      Height = 13
      Caption = 'C'#243'd.Malharia'
      FocusControl = DBEdit11
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlue
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
    end
    object Label6: TLabel
      Left = 230
      Top = 36
      Width = 40
      Height = 13
      Caption = 'Malharia'
      FocusControl = DBEdit11
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlue
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
    end
    object Label11: TLabel
      Left = 161
      Top = 73
      Width = 63
      Height = 13
      Caption = 'Nome Cliente'
      FocusControl = DBEdit11
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlue
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
    end
    object Label9: TLabel
      Left = 612
      Top = 113
      Width = 74
      Height = 13
      Caption = 'N'#186' NF Remessa'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlue
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
    end
    object Label7: TLabel
      Left = 690
      Top = 73
      Width = 67
      Height = 13
      Caption = 'Vl. Nota Fiscal'
      FocusControl = DBEdit7
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlue
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
    end
    object Label8: TLabel
      Left = 721
      Top = 113
      Width = 53
      Height = 13
      Caption = 'Total Rolos'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlue
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
    end
    object SpeedButton1: TSpeedButton
      Left = 384
      Top = 16
      Width = 23
      Height = 22
      Visible = False
      OnClick = SpeedButton1Click
    end
    object edtPesquisa: TEdit
      Left = 4
      Top = 15
      Width = 93
      Height = 19
      Ctl3D = False
      ParentCtl3D = False
      TabOrder = 0
      OnExit = edtPesquisaExit
      OnKeyPress = edtPesquisaKeyPress
    end
    object BB_PESQUISAR: TBitBtn
      Left = 98
      Top = 14
      Width = 81
      Height = 23
      Caption = '&Pesquisar'
      Glyph.Data = {
        
        DB93A7B8B7AE9999DBDBDBDBDBDBDBDBDBDBDB9D9E9ADBDBDBDB}
      TabOrder = 1
      OnClick = BB_PESQUISARClick
    end
    object DBEdit1: TDBEdit
      Left = 4
      Top = 49
      Width = 72
      Height = 19
      CharCase = ecUpperCase
      Ctl3D = False
      DataField = 'ID_REMESSA'
      DataSource = dsReativaRemessa
      ParentCtl3D = False
      ReadOnly = True
      TabOrder = 2
      OnKeyPress = FormKeyPress
    end
    object DBEdit4: TDBEdit
      Left = 4
      Top = 89
      Width = 72
      Height = 19
      CharCase = ecUpperCase
      Ctl3D = False
      DataField = 'DT_EMISSAO'
      DataSource = dsReativaRemessa
      ParentCtl3D = False
      TabOrder = 3
      OnKeyPress = FormKeyPress
    end
    object DBEdit19: TDBEdit
      Left = 4
      Top = 129
      Width = 72
      Height = 19
      CharCase = ecUpperCase
      Ctl3D = False
      DataField = 'ID_FORN_TINT'
      DataSource = dsReativaRemessa
      ParentCtl3D = False
      TabOrder = 4
    end
    object DBEdit10: TDBEdit
      Left = 77
      Top = 129
      Width = 533
      Height = 19
      CharCase = ecUpperCase
      Ctl3D = False
      DataField = 'TITURARIA'
      DataSource = dsReativaRemessa
      ParentCtl3D = False
      ReadOnly = True
      TabOrder = 5
    end
    object DBEdit20: TDBEdit
      Left = 77
      Top = 89
      Width = 82
      Height = 19
      CharCase = ecUpperCase
      Ctl3D = False
      DataField = 'ID_CLIENTE'
      DataSource = dsReativaRemessa
      ParentCtl3D = False
      TabOrder = 6
    end
    object DBEdit2: TDBEdit
      Left = 77
      Top = 49
      Width = 82
      Height = 19
      CharCase = ecUpperCase
      Ctl3D = False
      DataField = 'NR_NOTA_FISCAL'
      DataSource = dsReativaRemessa
      ParentCtl3D = False
      TabOrder = 7
    end
    object DBEdit3: TDBEdit
      Left = 161
      Top = 49
      Width = 68
      Height = 19
      CharCase = ecUpperCase
      Ctl3D = False
      DataField = 'ID_FOR_REMESSA'
      DataSource = dsReativaRemessa
      ParentCtl3D = False
      TabOrder = 8
    end
    object DBEdit5: TDBEdit
      Left = 230
      Top = 49
      Width = 568
      Height = 19
      CharCase = ecUpperCase
      Ctl3D = False
      DataField = 'NOME_MALHARIA'
      DataSource = dsReativaRemessa
      ParentCtl3D = False
      ReadOnly = True
      TabOrder = 9
    end
    object DBEdit11: TDBEdit
      Left = 161
      Top = 89
      Width = 527
      Height = 19
      CharCase = ecUpperCase
      Ctl3D = False
      DataField = 'CLIENTE'
      DataSource = dsReativaRemessa
      ParentCtl3D = False
      ReadOnly = True
      TabOrder = 10
    end
    object DBEdit16: TDBEdit
      Left = 612
      Top = 129
      Width = 108
      Height = 19
      CharCase = ecUpperCase
      Ctl3D = False
      DataField = 'NR_NF_REMESSA'
      DataSource = dsReativaRemessa
      ParentCtl3D = False
      TabOrder = 11
      OnKeyPress = FormKeyPress
    end
    object DBEdit7: TDBEdit
      Left = 690
      Top = 89
      Width = 108
      Height = 19
      CharCase = ecUpperCase
      Color = clInfoBk
      Ctl3D = False
      DataField = 'VL_NOTA'
      DataSource = dsReativaRemessa
      ParentCtl3D = False
      TabOrder = 12
      OnKeyPress = FormKeyPress
    end
    object DBEdit8: TDBEdit
      Left = 721
      Top = 129
      Width = 77
      Height = 19
      CharCase = ecUpperCase
      Ctl3D = False
      DataField = 'TOTAL_ROLOS'
      DataSource = dsReativaRemessa
      ParentCtl3D = False
      ReadOnly = True
      TabOrder = 13
      OnKeyPress = FormKeyPress
    end
  end
  object Panel3: TPanel
    Left = 0
    Top = 193
    Width = 813
    Height = 41
    Align = alTop
    Color = clBlue
    Font.Charset = ANSI_CHARSET
    Font.Color = clBlue
    Font.Height = -21
    Font.Name = 'Segoe UI Semibold'
    Font.Style = [fsBold]
    ParentBackground = False
    ParentFont = False
    TabOrder = 2
    object Label12: TLabel
      Left = 8
      Top = 4
      Width = 301
      Height = 37
      Caption = 'ITENS DA NOTA FISCAL'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWhite
      Font.Height = -27
      Font.Name = 'Segoe UI'
      Font.Style = [fsBold, fsItalic]
      ParentFont = False
    end
  end
  object PageControl2: TPageControl
    Left = 0
    Top = 234
    Width = 813
    Height = 209
    ActivePage = TabSheet3
    Align = alTop
    TabOrder = 3
    object TabSheet3: TTabSheet
      Caption = 'Itens'
      object DBGrid2: TDBGrid
        Left = 0
        Top = 0
        Width = 805
        Height = 181
        Align = alClient
        Color = clBtnFace
        Ctl3D = False
        DataSource = dsItemProducao
        FixedColor = cl3DLight
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = []
        Options = [dgTitles, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit, dgTitleClick, dgTitleHotTrack]
        ParentCtl3D = False
        ParentFont = False
        ParentShowHint = False
        ShowHint = False
        TabOrder = 0
        TitleFont.Charset = DEFAULT_CHARSET
        TitleFont.Color = clBlue
        TitleFont.Height = -11
        TitleFont.Name = 'Tahoma'
        TitleFont.Style = []
        Columns = <
          item
            Expanded = False
            FieldName = 'ID_ARTIGO'
            Width = 81
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'NOME_ARTIGO'
            Width = 145
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'QUANT_FATURADO'
            Width = 71
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'VL_UNIT'
            Width = 92
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'VL_TOTAL'
            Width = 105
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'NR_NOTA_FISCAL'
            Title.Caption = 'N'#186' Nota Fiscal'
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'NR_NF_REMESSA'
            Title.Caption = 'N'#186' NF Remessa'
            Visible = True
          end>
      end
    end
  end
  object dsReativaRemessa: TDataSource
    DataSet = cdsReativaRemessa
    Left = 712
    Top = 78
  end
  object cdsReativaRemessa: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'dspReativaRemessa'
    Left = 680
    Top = 78
    object cdsReativaRemessaID_REMESSA: TIntegerField
      FieldName = 'ID_REMESSA'
      Required = True
    end
    object cdsReativaRemessaID_FOR_REMESSA: TIntegerField
      FieldName = 'ID_FOR_REMESSA'
      Required = True
    end
    object cdsReativaRemessaSTATUS: TStringField
      FieldName = 'STATUS'
      Size = 1
    end
    object cdsReativaRemessaQUANT_NF: TFMTBCDField
      FieldName = 'QUANT_NF'
      Precision = 18
      Size = 3
    end
    object cdsReativaRemessaQUANT_FATURADO: TFMTBCDField
      FieldName = 'QUANT_FATURADO'
      Precision = 18
      Size = 3
    end
    object cdsReativaRemessaQUANT_SALDO: TFMTBCDField
      FieldName = 'QUANT_SALDO'
      Precision = 18
      Size = 3
    end
    object cdsReativaRemessaID_CLIENTE: TIntegerField
      FieldName = 'ID_CLIENTE'
    end
    object cdsReativaRemessaID_FORNECEDOR: TIntegerField
      FieldName = 'ID_FORNECEDOR'
    end
    object cdsReativaRemessaDT_EMISSAO: TDateField
      FieldName = 'DT_EMISSAO'
    end
    object cdsReativaRemessaNR_NOTA_FISCAL: TIntegerField
      DisplayLabel = 'N'#186' Nota Fiscal'
      FieldName = 'NR_NOTA_FISCAL'
    end
    object cdsReativaRemessaVL_NOTA: TFMTBCDField
      FieldName = 'VL_NOTA'
      Precision = 18
      Size = 3
    end
    object cdsReativaRemessaVL_POR_FORA: TFMTBCDField
      FieldName = 'VL_POR_FORA'
      Precision = 18
      Size = 3
    end
    object cdsReativaRemessaVL_TOTAL_NF: TFMTBCDField
      FieldName = 'VL_TOTAL_NF'
      Precision = 18
      Size = 3
    end
    object cdsReativaRemessaID_FORN_TINT: TIntegerField
      FieldName = 'ID_FORN_TINT'
      Required = True
    end
    object cdsReativaRemessaNR_NF_REMESSA: TIntegerField
      FieldName = 'NR_NF_REMESSA'
    end
    object cdsReativaRemessaTOTAL_ROLOS: TIntegerField
      FieldName = 'TOTAL_ROLOS'
    end
    object cdsReativaRemessaNF_QUEBRA: TStringField
      FieldName = 'NF_QUEBRA'
      Size = 1
    end
    object cdsReativaRemessaCLIENTE: TStringField
      FieldName = 'CLIENTE'
      ProviderFlags = []
      Size = 50
    end
    object cdsReativaRemessaFORNECEDOR: TStringField
      FieldName = 'FORNECEDOR'
      ProviderFlags = []
      Size = 50
    end
    object cdsReativaRemessaNOME_MALHARIA: TStringField
      FieldName = 'NOME_MALHARIA'
      ProviderFlags = []
      Size = 50
    end
    object cdsReativaRemessaTITURARIA: TStringField
      DisplayLabel = 'Tinturaria'
      FieldName = 'TITURARIA'
      ProviderFlags = []
      Size = 50
    end
  end
  object dspReativaRemessa: TDataSetProvider
    DataSet = qReativaRemessa
    Options = [poIncFieldProps, poAllowMultiRecordUpdates, poAutoRefresh, poAllowCommandText, poUseQuoteChar]
    Left = 648
    Top = 78
  end
  object qReativaRemessa: TSQLQuery
    MaxBlobSize = -1
    Params = <>
    SQL.Strings = (
      'SELECT'
      ' RP.ID_REMESSA,'
      ' RP.ID_FOR_REMESSA,'
      ' RP.STATUS,'
      ' RP.QUANT_NF,'
      ' RP.QUANT_FATURADO,'
      ' RP.QUANT_SALDO,'
      ' RP.ID_CLIENTE,'
      ' RP.ID_FORNECEDOR,'
      ' RP.DT_EMISSAO,'
      ' RP.NR_NOTA_FISCAL,'
      ' RP.VL_NOTA,'
      ' RP.VL_POR_FORA,'
      ' RP.VL_TOTAL_NF,'
      ' RP.ID_FORN_TINT,'
      ' RP.NR_NF_REMESSA,'
      ' RP.TOTAL_ROLOS,'
      ' RP.NF_QUEBRA,'
      ' C.R_SOCIAL AS CLIENTE,'
      ' F.NOME AS FORNECEDOR,'
      ' FF.NOME AS NOME_MALHARIA,'
      ' FT.NOME AS TITURARIA'
      'FROM TB_REMESSA_PRODUCAO RP'
      
        'LEFT OUTER JOIN TB_FORNECEDOR F ON (F.ID_FORNECEDOR = RP.ID_FORN' +
        'ECEDOR)'
      'JOIN TB_FORNECEDOR FF ON (FF.ID_FORNECEDOR = RP.ID_FOR_REMESSA)'
      'JOIN TB_FORNECEDOR FT ON (FT.ID_FORNECEDOR = RP.ID_FORN_TINT)'
      'JOIN TB_CLIENTE C ON (C.ID_CLIENTE = RP.ID_CLIENTE)'
      'WHERE RP.NF_QUEBRA = '#39'N'#39
      'AND RP.status = '#39'F'#39)
    SQLConnection = DM.conexao
    Left = 616
    Top = 78
    object qReativaRemessaID_REMESSA: TIntegerField
      FieldName = 'ID_REMESSA'
      Required = True
    end
    object qReativaRemessaID_FOR_REMESSA: TIntegerField
      FieldName = 'ID_FOR_REMESSA'
      Required = True
    end
    object qReativaRemessaSTATUS: TStringField
      FieldName = 'STATUS'
      Size = 1
    end
    object qReativaRemessaQUANT_NF: TFMTBCDField
      FieldName = 'QUANT_NF'
      Precision = 18
      Size = 3
    end
    object qReativaRemessaQUANT_FATURADO: TFMTBCDField
      FieldName = 'QUANT_FATURADO'
      Precision = 18
      Size = 3
    end
    object qReativaRemessaQUANT_SALDO: TFMTBCDField
      FieldName = 'QUANT_SALDO'
      Precision = 18
      Size = 3
    end
    object qReativaRemessaID_CLIENTE: TIntegerField
      FieldName = 'ID_CLIENTE'
    end
    object qReativaRemessaID_FORNECEDOR: TIntegerField
      FieldName = 'ID_FORNECEDOR'
    end
    object qReativaRemessaDT_EMISSAO: TDateField
      FieldName = 'DT_EMISSAO'
    end
    object qReativaRemessaNR_NOTA_FISCAL: TIntegerField
      FieldName = 'NR_NOTA_FISCAL'
    end
    object qReativaRemessaVL_NOTA: TFMTBCDField
      FieldName = 'VL_NOTA'
      Precision = 18
      Size = 3
    end
    object qReativaRemessaVL_POR_FORA: TFMTBCDField
      FieldName = 'VL_POR_FORA'
      Precision = 18
      Size = 3
    end
    object qReativaRemessaVL_TOTAL_NF: TFMTBCDField
      FieldName = 'VL_TOTAL_NF'
      Precision = 18
      Size = 3
    end
    object qReativaRemessaID_FORN_TINT: TIntegerField
      FieldName = 'ID_FORN_TINT'
      Required = True
    end
    object qReativaRemessaNR_NF_REMESSA: TIntegerField
      FieldName = 'NR_NF_REMESSA'
    end
    object qReativaRemessaTOTAL_ROLOS: TIntegerField
      FieldName = 'TOTAL_ROLOS'
    end
    object qReativaRemessaNF_QUEBRA: TStringField
      FieldName = 'NF_QUEBRA'
      Size = 1
    end
    object qReativaRemessaCLIENTE: TStringField
      FieldName = 'CLIENTE'
      ProviderFlags = []
      Size = 50
    end
    object qReativaRemessaFORNECEDOR: TStringField
      FieldName = 'FORNECEDOR'
      ProviderFlags = []
      Size = 50
    end
    object qReativaRemessaNOME_MALHARIA: TStringField
      FieldName = 'NOME_MALHARIA'
      ProviderFlags = []
      Size = 50
    end
    object qReativaRemessaTITURARIA: TStringField
      FieldName = 'TITURARIA'
      ProviderFlags = []
      Size = 50
    end
  end
  object dsItemProducao: TDataSource
    DataSet = cdsItemProducao
    Left = 712
    Top = 126
  end
  object cdsItemProducao: TClientDataSet
    Aggregates = <>
    IndexFieldNames = 'ID_REMESSA'
    MasterFields = 'ID_REMESSA'
    MasterSource = dsReativaRemessa
    PacketRecords = 0
    Params = <
      item
        DataType = ftInteger
        Name = 'ID_REMESSA'
        ParamType = ptInput
      end>
    ProviderName = 'dspItemProducao'
    Left = 680
    Top = 126
    object cdsItemProducaoID_ITEM_REMESSA: TIntegerField
      FieldName = 'ID_ITEM_REMESSA'
      Required = True
    end
    object cdsItemProducaoID_REMESSA: TIntegerField
      FieldName = 'ID_REMESSA'
    end
    object cdsItemProducaoID_ARTIGO: TIntegerField
      DisplayLabel = 'C'#243'd. Artigo'
      FieldName = 'ID_ARTIGO'
    end
    object cdsItemProducaoQUANT_FATURADO: TFMTBCDField
      DisplayLabel = 'Quant. Fat'
      FieldName = 'QUANT_FATURADO'
      DisplayFormat = '###,###0.000'
      EditFormat = '###,###0.000'
      Precision = 18
      Size = 4
    end
    object cdsItemProducaoVL_UNIT: TFMTBCDField
      DisplayLabel = 'Vl. Unit'
      FieldName = 'VL_UNIT'
      DisplayFormat = '###,###0.00'
      EditFormat = '###,###0.00'
      Precision = 18
      Size = 3
    end
    object cdsItemProducaoVL_TOTAL: TFMTBCDField
      DisplayLabel = 'Vl. Total'
      FieldName = 'VL_TOTAL'
      DisplayFormat = '###,###0.00'
      EditFormat = '###,###0.00'
      Precision = 18
      Size = 3
    end
    object cdsItemProducaoID_CLIENTE: TIntegerField
      FieldName = 'ID_CLIENTE'
    end
    object cdsItemProducaoID_FORNECEDOR: TIntegerField
      FieldName = 'ID_FORNECEDOR'
    end
    object cdsItemProducaoID_MATERIA: TIntegerField
      FieldName = 'ID_MATERIA'
    end
    object cdsItemProducaoID_FORN_MALHARIA: TIntegerField
      FieldName = 'ID_FORN_MALHARIA'
    end
    object cdsItemProducaoNR_NOTA_FISCAL: TIntegerField
      DisplayLabel = 'N'#186' Nota Fiscal'
      FieldName = 'NR_NOTA_FISCAL'
    end
    object cdsItemProducaoNR_NF_REMESSA: TIntegerField
      DisplayLabel = 'N'#186' Nota Fiscal Remessa'
      FieldName = 'NR_NF_REMESSA'
    end
    object cdsItemProducaoID_PRODUCAO: TIntegerField
      FieldName = 'ID_PRODUCAO'
    end
    object cdsItemProducaoVL_UNIT_REAL: TFMTBCDField
      DisplayLabel = 'Vl. Unit'
      FieldName = 'VL_UNIT_REAL'
      DisplayFormat = '###,###0.00'
      EditFormat = '###,###0.00'
      Precision = 18
      Size = 3
    end
    object cdsItemProducaoVL_TOTAL_REAL: TFMTBCDField
      FieldName = 'VL_TOTAL_REAL'
      Precision = 18
      Size = 3
    end
    object cdsItemProducaoQT_ROLOS: TIntegerField
      FieldName = 'QT_ROLOS'
    end
    object cdsItemProducaoNOME_ARTIGO: TStringField
      DisplayLabel = 'Artigo'
      FieldName = 'NOME_ARTIGO'
      Size = 30
    end
    object cdsItemProducaoR_SOCIAL: TStringField
      FieldName = 'R_SOCIAL'
      Size = 50
    end
    object cdsItemProducaoFORNECEDOR: TStringField
      FieldName = 'FORNECEDOR'
      Size = 50
    end
    object cdsItemProducaoMALHARIA: TStringField
      FieldName = 'MALHARIA'
      Size = 50
    end
    object cdsItemProducaoNOME_MATERIA: TStringField
      FieldName = 'NOME_MATERIA'
      Size = 30
    end
  end
  object dspItemProducao: TDataSetProvider
    DataSet = qItemProducao
    Options = [poIncFieldProps, poAllowMultiRecordUpdates, poAutoRefresh, poAllowCommandText, poUseQuoteChar]
    Left = 648
    Top = 126
  end
  object qItemProducao: TSQLQuery
    MaxBlobSize = -1
    Params = <
      item
        DataType = ftInteger
        Name = 'ID_REMESSA'
        ParamType = ptInput
      end>
    SQL.Strings = (
      'SELECT'
      ' IR.ID_ITEM_REMESSA,'
      ' IR.ID_REMESSA,'
      ' IR.ID_ARTIGO,'
      ' IR.QUANT_FATURADO,'
      ' IR.VL_UNIT,'
      ' IR.VL_TOTAL,'
      ' IR.ID_CLIENTE,'
      ' IR.ID_FORNECEDOR,'
      ' IR.ID_MATERIA,'
      ' IR.ID_FORN_MALHARIA,'
      ' IR.NR_NOTA_FISCAL,'
      ' IR.NR_NF_REMESSA,'
      ' IR.ID_PRODUCAO,'
      ' IR.VL_UNIT_REAL,'
      ' IR.VL_TOTAL_REAL,'
      ' IR.QT_ROLOS,'
      ' A.NOME_ARTIGO,'
      ' C.R_SOCIAL,'
      ' F.NOME AS FORNECEDOR,'
      ' M.NOME AS MALHARIA,'
      ' MP.NOME_MATERIA'
      'FROM TB_ITEM_REMESSA_PRODUCAO IR'
      'LEFT OUTER JOIN TB_ARTIGO A ON (A.ID_ARTIGO = IR.ID_ARTIGO)'
      'LEFT OUTER JOIN TB_CLIENTE C ON (C.ID_CLIENTE = IR.ID_CLIENTE)'
      
        'LEFT OUTER JOIN TB_FORNECEDOR F ON (F.ID_FORNECEDOR = IR.ID_FORN' +
        'ECEDOR)'
      
        'LEFT OUTER JOIN TB_FORNECEDOR M ON (M.ID_FORNECEDOR = IR.ID_FORN' +
        '_MALHARIA)'
      
        'LEFT OUTER JOIN TB_MATERIA_PRIMA MP ON (MP.ID_MATERIA = IR.ID_MA' +
        'TERIA)'
      'WHERE IR.ID_REMESSA = :ID_REMESSA')
    SQLConnection = DM.conexao
    Left = 616
    Top = 126
    object qItemProducaoID_ITEM_REMESSA: TIntegerField
      FieldName = 'ID_ITEM_REMESSA'
      Required = True
    end
    object qItemProducaoID_REMESSA: TIntegerField
      FieldName = 'ID_REMESSA'
    end
    object qItemProducaoID_ARTIGO: TIntegerField
      FieldName = 'ID_ARTIGO'
    end
    object qItemProducaoQUANT_FATURADO: TFMTBCDField
      FieldName = 'QUANT_FATURADO'
      Precision = 18
      Size = 4
    end
    object qItemProducaoVL_UNIT: TFMTBCDField
      FieldName = 'VL_UNIT'
      Precision = 18
      Size = 3
    end
    object qItemProducaoVL_TOTAL: TFMTBCDField
      FieldName = 'VL_TOTAL'
      Precision = 18
      Size = 3
    end
    object qItemProducaoID_CLIENTE: TIntegerField
      FieldName = 'ID_CLIENTE'
    end
    object qItemProducaoID_FORNECEDOR: TIntegerField
      FieldName = 'ID_FORNECEDOR'
    end
    object qItemProducaoID_MATERIA: TIntegerField
      FieldName = 'ID_MATERIA'
    end
    object qItemProducaoID_FORN_MALHARIA: TIntegerField
      FieldName = 'ID_FORN_MALHARIA'
    end
    object qItemProducaoNR_NOTA_FISCAL: TIntegerField
      FieldName = 'NR_NOTA_FISCAL'
    end
    object qItemProducaoNR_NF_REMESSA: TIntegerField
      FieldName = 'NR_NF_REMESSA'
    end
    object qItemProducaoID_PRODUCAO: TIntegerField
      FieldName = 'ID_PRODUCAO'
    end
    object qItemProducaoVL_UNIT_REAL: TFMTBCDField
      FieldName = 'VL_UNIT_REAL'
      Precision = 18
      Size = 3
    end
    object qItemProducaoVL_TOTAL_REAL: TFMTBCDField
      FieldName = 'VL_TOTAL_REAL'
      Precision = 18
      Size = 3
    end
    object qItemProducaoQT_ROLOS: TIntegerField
      FieldName = 'QT_ROLOS'
    end
    object qItemProducaoNOME_ARTIGO: TStringField
      FieldName = 'NOME_ARTIGO'
      Size = 30
    end
    object qItemProducaoR_SOCIAL: TStringField
      FieldName = 'R_SOCIAL'
      Size = 50
    end
    object qItemProducaoFORNECEDOR: TStringField
      FieldName = 'FORNECEDOR'
      Size = 50
    end
    object qItemProducaoMALHARIA: TStringField
      FieldName = 'MALHARIA'
      Size = 50
    end
    object qItemProducaoNOME_MATERIA: TStringField
      FieldName = 'NOME_MATERIA'
      Size = 30
    end
  end
end
